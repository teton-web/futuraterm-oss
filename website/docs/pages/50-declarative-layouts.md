...
Describe a project's tabs, split layout, and the process each pane runs in a YAML file, and FuturaTerm builds the workspace from it. Project files live in `~/.config/futuraterm/projects/`, one per project — they're matched to a project by their `path`, not their filename, so the filename is just cosmetic.

```yaml title="~/.config/futuraterm/projects/myapp.yaml"
name: "MyApp"
path: "~/dev/myapp"
tabs:
  - run: "npm run dev"
...
> The older in-project layout file was removed in v1.22.0, after being deprecated since v1.20.0. If you still have one, recreate the layout with **Save layout** — it writes the central file for you.
