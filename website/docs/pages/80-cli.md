...
FuturaTerm bundles a control CLI that drives the **running app** — projects, tabs, panes, and their zmx sessions — over a local Unix socket. It's how scripts, AI agents, and other apps orchestrate the terminal: spawn a grid of panes, run a command in each, focus one, tear it down.

Shells that FuturaTerm spawns already have `futuraterm` on their `PATH`, so inside any pane it just works:

```console
$ futuraterm status
FuturaTerm 1.4.0 (pid 4242) — active project: api

$ futuraterm tab new --run "npm run dev"
tab:3  *  npm  1 pane

$ futuraterm grid 2x2 --run "tail -f log/dev.log"
$ futuraterm session list
futuraterm-api-8f327ce4a3f8  clients:1  pid:4310  attached-pane
```

From a shell FuturaTerm *didn't* spawn, invoke it by bundle path (or symlink it onto your `PATH`):

```sh
/Applications/FuturaTerm.app/Contents/Resources/bin/futuraterm status
```

Every command takes `--json` for a stable, scriptable payload instead of the human table, `--socket <path>` to target a specific app instance, and `--no-launch` to skip auto-starting FuturaTerm. `--help` works at every level of the command tree.

## Commands

The grammar is `futuraterm <noun> <verb> [options]`. A bare noun defaults to its `list` verb, so `futuraterm project` is `futuraterm project list`.
...
| `project create <path> [--name N] [--select]` | Add a project for a local directory or an scp-style `[user@]host:dir` [remote spec](/docs/remote-projects) (stored verbatim — a wrong host or dir surfaces in the pane). **Not idempotent** — each run adds a distinct project, even for a directory that already has one. Use `project open` for create-or-select. `--select` activates it — and, on first open, applies a matching [layout file](/docs/declarative-layouts). |
| `project open <path> [--name N]` | Find the first project matching `path`, or create one, then select it and front the window. `--name` is used only when creating. Same path rules as `project create` (local dir must exist; remote spec stored verbatim). |
...
1. `--session <name>` — the zmx session name (`futuraterm-<slug>-<hex12>`). This is the **restart-stable** address: pane UUIDs regenerate every launch, session names persist verbatim.
2. `--pane <uuid|index>` — a pane UUID (searched project-wide), or its index within the tab scope.
3. `FUTURATERM_SESSION` — inside a pane, FuturaTerm sets this to that pane's own session, so a bare `futuraterm pane split` splits *the pane you're in*. An explicit `--tab` disables this fallback.
4. Otherwise, the focused pane of the active tab.

`pane close` never uses the `FUTURATERM_SESSION` fallback — destroying "whatever pane I'm in" because no target was given is a footgun, so it always demands an explicit `--pane` or `--session`.

`pane focus --direction` treats the resolved target as the **origin**, not the destination — so a bare `futuraterm pane focus --direction left` run inside a pane moves left from *that* pane. It reports the pane that ended up focused, and at the outermost edge it reports the origin unchanged rather than failing, so a caller can tell whether it moved by comparing the reported session to its own `$FUTURATERM_SESSION`.
...
```console
$ futuraterm pane inspect
session             futuraterm-api-8f327ce4a3f8
grid                132×65
cell px             16×40
...
```console
$ futuraterm pane dump --scrollback | wc -l
360
```
...
- `FUTURATERM_SOCKET` — the control socket path. A discovery *hint*: if it doesn't answer (the app restarted since this shell spawned), the CLI falls back to the well-known locations. Only `--socket` pins hard.
- `FUTURATERM_SESSION` — the pane's own session name, for self-targeting.
- `PATH` — prepended with the bundle's `Resources/bin`, so `futuraterm` resolves.
...
- `2` — no running FuturaTerm could be reached (stderr lists every socket path tried).

By default the CLI launches the companion `.app` when no control socket answers, then retries until the app is ready (or 20s elapses). `--no-launch` restores fail-fast exit 2 for scripts that start the app themselves. `--socket` never auto-launches — the pin may not be this app.

Scripts that already launch FuturaTerm can pass `--no-launch` and keep polling:

```sh
until futuraterm status --no-launch >/dev/null 2>&1; do sleep 0.2; done
```
...
#!/bin/sh
set -e
mac=/Applications/FuturaTerm.app/Contents/Resources/bin/futuraterm

# status launches the app if needed and waits until the socket is ready.
"$mac" status >/dev/null
...
The `futuraterm` binary is a thin client over a documented protocol — any same-user process can speak it directly. One request per connection to the socket at `~/Library/Application Support/FuturaTerm/control.sock`: write one newline-terminated JSON line, half-close your write end, and read one line back.
...
```json title="response"
{"v":1,"id":"<echoed>","ok":true,"data":{"panes":[{"id":"…","session":"futuraterm-api-1a2b3c4d5e6f","index":2}]}}
```
...
```sh
echo '{"v":1,"id":"x","command":"status"}' | nc -U ~/Library/Application\ Support/FuturaTerm/control.sock
```
