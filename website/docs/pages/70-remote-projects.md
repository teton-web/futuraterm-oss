...
A remote project is a directory on another machine. Every pane in it is a persistent [zmx](https://zmx.sh) session running **on that host** over SSH — so your shells, running processes, and scrollback live on the server, not on your Mac. Quit FuturaTerm and the sessions keep running; relaunch and every pane reattaches. Because the host's session daemon outlives your laptop, remote panes even survive a local reboot or a dropped connection — reconnecting is just reattaching.
...
Port, identity file, and other connection settings come from your `~/.ssh/config`, never from FuturaTerm — define a `Host` alias there and use the alias as the project's host:
...
Repeating the same host and directory via the sheet **selects** the existing sidebar project rather than adding a duplicate. The existing name and zmx path are left unchanged. To add a second project for the same spec, use the command palette's **Add another project** row or `futuraterm project create`.
...
| Quit FuturaTerm | SSH disconnects; sessions **detach and keep running** on the host |
...
```yaml title="~/.config/futuraterm/projects/api.yaml"
name: "API (devbox)"
path: "devbox:~/dev/api"
...
- **`futuraterm: zmx not found in PATH on this host (…)`** in the pane — zmx isn't in a directory the non-interactive ssh PATH covers. Either move it to `~/bin` or `~/.local/bin`, or set the project's **zmx path** to its absolute location. The pane stays open as a plain shell so you can investigate.
- **`futuraterm: cannot cd to …`** — the project directory doesn't exist on the host. The pane drops to a shell in the home directory.
