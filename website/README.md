...
public/            Served as static files
  ...
  assets/          Symlink to the repo-root assets/ (icon, schema)
src/
...
`ghcr.io/davidsolheim/futuraterm/website`. Pull requests build both architectures without
...
cluster runs the site at [futuraterm.com](https://futuraterm.com) and
...
timestamp does. The deploy-side ImagePolicy `filterTags` pattern must match
this format — the two move together.

```sh
docker run --rm -p 3000:3000 ghcr.io/davidsolheim/futuraterm/website:latest
```
...
```sh
bun run docker:build   # docker build -f Dockerfile -t futuraterm-website ..
bun run docker:run     # docker run --rm -p 3000:3000 futuraterm-website
```
