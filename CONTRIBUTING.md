# Contributing to FuturaTerm

Thanks for your interest in contributing! FuturaTerm is a macOS terminal emulator built with SwiftUI and libghostty.

## Setup

Clone the public snapshot (Teton Web Ventures LLC). Do not clone the private working tree unless you already have access:
...
Building requires macOS 26+ and a full install of Xcode 26 (the Command Line Tools alone aren't enough — `xcodebuild` needs the Xcode app to build the macOS app target, and the code compiles against macOS 26 SDK APIs behind `#available` checks). The shipped app itself runs on macOS 14+. Linux contributors can skip Xcode and use `mise run linux-test` / `./scripts/build-linux.sh` on Omarchy.
...
| `clean` | Remove `build/` and the generated `FuturaTerm.xcodeproj` |
...
- **Unit tests** (`FuturaTermTests/`) — `mise run test`. One `XxxTests.swift` per production type, mirroring the source path. They run hosted inside the Debug app and cover the model, persistence, palette, and hotkey logic; SwiftUI views and libghostty bindings aren't unit-tested.
- **End-to-end tests** (`e2e/`) — `mise run e2e`. A pytest suite that launches the real GUI app hermetically and drives it through the bundled `futuraterm` CLI. Needs `python3` (the script bootstraps its own venv under `build/`). Slower than the unit suite — the app takes ~15–30s to come up — so it's not part of the pre-commit loop, but run it when you touch panes, sessions, layouts, or the control socket.
...
- The Xcode project is generated by xcodegen from [`project.yml`](project.yml). Don't edit `FuturaTerm.xcodeproj/` directly — it's gitignored and your changes will be wiped on the next regeneration.
...
File issues at https://github.com/teton-web/futuraterm-oss/issues with steps to reproduce and your macOS version. `mise run logs --last 30m` is a good way to grab relevant log output.
