...
struct CommandSourceTests {
    ...
    func applyLayout_is_muted_with_hint_when_no_project_file_exists() throws {
        ...
        #expect(
            item.subtitle
                == "No project file for this project — use “Save Layout” to create one"
        )
        #expect(item.subtitle != AppCommand.applyLayout.help)
    }

    @Test
    func viewDesktop_is_muted_on_local_and_enabled_on_remote() throws {
        let (localCtx, _) = makeContext()
        let muted = try #require(findItem(title: AppCommand.viewDesktop.title, in: localCtx))
        #expect(!muted.isEnabled)
        #expect(muted.subtitle == DesktopView.notRemoteReason)
        #expect(muted.category == AppCommand.viewDesktop.category.rawValue)
        ...
        let remoteCtx = PaletteContext(appState: state, projectStore: store)
        let enabled = try #require(findItem(title: AppCommand.viewDesktop.title, in: remoteCtx))
        #expect(enabled.isEnabled)
        #expect(enabled.subtitle == AppCommand.viewDesktop.help)
    }

    @Test
    func newTab_palette_subtitle_is_help() throws {
        let (ctx, _) = makeContext()
        let item = try #require(findItem(title: AppCommand.newTab.title, in: ctx))
        #expect(item.isEnabled)
        #expect(item.subtitle == AppCommand.newTab.help)
    }
    ...
    func applyLayout_is_muted_when_project_file_has_no_tabs() throws {
        ...
        let item = try #require(findItem(title: AppCommand.applyLayout.title, in: ctx))
        #expect(!item.isEnabled)
        #expect(item.subtitle == "The project file declares no tabs")
        #expect(item.subtitle != AppCommand.applyLayout.help)
    }
    ...
    func applyLayout_subtitle_names_duplicate_project_files() throws {
        ...
        #expect(subtitle != AppCommand.applyLayout.help)
    }
    ...
    @Test
    func query_newTab_carries_help_subtitle() throws {
        let (ctx, _) = makeContext()
        let item = try #require(queryItem(title: AppCommand.newTab.title, query: "new t", in: ctx))
        #expect(item.subtitle == AppCommand.newTab.help)
    }
