...
struct ControlHandlerTests {
    ...
    private func makeAppState() -> AppState {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-control-tests-\(UUID().uuidString).json")
        let projectsDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-control-tests-projects-\(UUID().uuidString)", isDirectory: true)
        return AppState(
    ...
    private func makeHandler(
        ...
        let projectStore = store ?? ProjectStore(fileURL: FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-control-tests-store-\(UUID().uuidString).json"))
    ...
    private func openRunFixtureDir(_ label: String) throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-open-run-\(label)-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }

    private func tabID(containing pane: Pane, in workspace: Workspace) -> UUID? {
        workspace.tabs.first { $0.splitRoot.findPane(id: pane.id) != nil }?.id
    }
    ...
    func pane_list_walks_splits_and_marks_focus() async {
        ...
        #expect(panes?.allSatisfy { $0.session.hasPrefix("futuraterm-") } == true)
    ...
    func session_list_maps_entries_and_live_panes() async throws {
        ...
        stubZmx(
            ...
            entries: [
                ...
                .init(name: "futuraterm-orphan-aaaabbbbcccc", clients: 0),
            ],
        ...
        let orphan = sessions?.first { $0.name == "futuraterm-orphan-aaaabbbbcccc" }
    ...
    func session_info_finds_by_name_or_404s() async {
        let (handler, appState, _) = makeHandler()
        stubZmx(appState, entries: [.init(name: "futuraterm-x-000011112222", clients: 0)])

        let hit = await handler.handle(request("session.info", args: ControlArgs(session: "futuraterm-x-000011112222")))
        #expect(hit.ok)
        #expect(hit.data?.sessions?.count == 1)

        let miss = await handler.handle(request("session.info", args: ControlArgs(session: "futuraterm-y-000011112222")))
    ...
    // MARK: - project.create / project.open / project.select

    @Test
    func project_create_adds_and_optionally_selects() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-create-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    @Test
    func project_open_reuses_existing_and_selects() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("macterm-open-existing-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let existing = seedProject(appState, projectStore, name: "already", path: dir.path, select: false)
        #expect(appState.activeProjectID != existing.id)

        let opened = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, name: "ignored-on-reuse")
        ))
        #expect(opened.ok)
        let info = try #require(opened.data?.projects?.first)
        #expect(info.id == existing.id.uuidString)
        #expect(info.name == "already")
        #expect(info.active == true)
        #expect(info.loaded == true)
        #expect(projectStore.projects.count == 1)
        #expect(appState.activeProjectID == existing.id)
        #expect(projectStore.projects.first?.name == "already")
    }

    @Test
    func project_open_creates_then_reuses_the_same_id() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("macterm-open-new-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)

        let first = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, name: "fresh-open")
        ))
        #expect(first.ok)
        let firstInfo = try #require(first.data?.projects?.first)
        #expect(firstInfo.name == "fresh-open")
        #expect(firstInfo.active == true)
        #expect(firstInfo.loaded == true)
        #expect(projectStore.projects.count == 1)
        #expect(appState.activeProjectID?.uuidString == firstInfo.id)

        let second = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, select: false)
        ))
        #expect(second.ok)
        #expect(second.data?.projects?.first?.id == firstInfo.id)
        #expect(second.data?.projects?.first?.active == true)
        #expect(projectStore.projects.count == 1)
        #expect(appState.activeProjectID?.uuidString == firstInfo.id)
    }

    @Test
    func project_open_picks_the_first_of_several_same_path_projects() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("macterm-open-first-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        let first = seedProject(appState, projectStore, name: "one", path: dir.path, select: false)
        _ = seedProject(appState, projectStore, name: "two", path: dir.path, select: true)
        #expect(projectStore.projects.count == 2)
        #expect(appState.activeProjectID != first.id)

        let opened = await handler.handle(request("project.open", args: ControlArgs(path: dir.path)))
        #expect(opened.ok)
        #expect(opened.data?.projects?.first?.id == first.id.uuidString)
        #expect(opened.data?.projects?.first?.active == true)
        #expect(projectStore.projects.count == 2)
        #expect(appState.activeProjectID == first.id)
    }

    @Test
    func project_create_still_duplicates_after_open() async throws {
        let (handler, _, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("macterm-open-then-create-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)

        let opened = await handler.handle(request("project.open", args: ControlArgs(path: dir.path)))
        let openedID = try #require(opened.data?.projects?.first?.id)
        #expect(projectStore.projects.count == 1)

        let created = await handler.handle(request("project.create", args: ControlArgs(path: dir.path)))
        #expect(created.data?.projects?.first?.id != openedID)
        #expect(projectStore.projects.count == 2)
    }

    @Test
    func project_open_rejects_bad_paths() async {
        let (handler, _, projectStore) = makeHandler()
        let relative = await handler.handle(request("project.open", args: ControlArgs(path: "dev/api")))
        #expect(relative.error?.code == .badRequest)
        let missing = await handler.handle(request(
            "project.open", args: ControlArgs(path: "/nonexistent-\(UUID().uuidString)")
        ))
        #expect(missing.error?.code == .notFound)
        let empty = await handler.handle(request("project.open"))
        #expect(empty.error?.code == .badRequest)
        #expect(projectStore.projects.isEmpty)
    }

    @Test
    func project_open_accepts_a_remote_spec_verbatim_and_reuses() async {
        let (handler, appState, projectStore) = makeHandler()
        let spec = "dev@host:~/dev/api"
        let first = await handler.handle(request("project.open", args: ControlArgs(path: spec)))
        #expect(first.ok)
        #expect(projectStore.projects.first?.path == spec)
        #expect(projectStore.projects.first?.isRemote == true)
        #expect(first.data?.projects?.first?.active == true)
        #expect(appState.activeProjectID?.uuidString == first.data?.projects?.first?.id)

        let second = await handler.handle(request("project.open", args: ControlArgs(path: spec)))
        #expect(second.data?.projects?.first?.id == first.data?.projects?.first?.id)
        #expect(projectStore.projects.count == 1)
    }

    // MARK: - project.open --run (POR-346)

    @Test
    func project_open_run_reuses_existing_grok_pane() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = try openRunFixtureDir("reuse-grok")
        let project = seedProject(appState, projectStore, path: dir.path)
        let created = await handler.handle(request("tab.new", args: ControlArgs(run: "grok")))
        #expect(created.ok)
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 2)
        let grokPane = try #require(workspace.tabs.last?.splitRoot.allPanes().first)
        #expect(grokPane.command == "grok")

        let opened = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "grok")
        ))
        #expect(opened.ok)
        #expect(workspace.tabs.count == 2)
        let panes = try #require(opened.data?.panes)
        #expect(panes.count == 1)
        #expect(panes[0].session == grokPane.sessionName)
        #expect(!panes[0].session.isEmpty)
        #expect(panes[0].focused == true)
        #expect(workspace.activeTabID == workspace.tabs.last?.id)
        #expect(opened.data?.projects?.count == 1)
        #expect(opened.data?.projects?[0].id == project.id.uuidString)
    }

    @Test
    func project_open_run_creates_grok_tab_when_none_matches() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = try openRunFixtureDir("create-grok")
        let project = seedProject(appState, projectStore, path: dir.path)
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 1)
        #expect(workspace.tabs[0].splitRoot.allPanes().first?.command == nil)
        ...
        #expect(opened.ok)
        #expect(workspace.tabs.count == 2)
        let pane = try #require(opened.data?.panes?.first)
        #expect(!pane.session.isEmpty)
        let live = try #require(
            workspace.tabs.flatMap { $0.splitRoot.allPanes() }
                .first { $0.sessionName == pane.session }
        )
        #expect(live.command == "grok")
        #expect(pane.focused == true)
        #expect(workspace.activeTabID == tabID(containing: live, in: workspace))
    }

    @Test
    func project_open_no_reuse_always_creates() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = try openRunFixtureDir("no-reuse")
        let project = seedProject(appState, projectStore, path: dir.path)
        _ = await handler.handle(request("tab.new", args: ControlArgs(run: "grok")))
        let workspace = try #require(appState.workspaces[project.id])
        let existingSession = try #require(workspace.tabs.last?.splitRoot.allPanes().first?.sessionName)
        #expect(workspace.tabs.count == 2)

        let opened = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "grok", reuse: false)
        ))
        #expect(opened.ok)
        #expect(workspace.tabs.count == 3)
        let pane = try #require(opened.data?.panes?.first)
        #expect(pane.session != existingSession)
        #expect(!pane.session.isEmpty)
        let live = try #require(
            workspace.tabs.flatMap { $0.splitRoot.allPanes() }
                .first { $0.sessionName == pane.session }
    ...
    @Test
    func project_open_without_run_does_not_add_a_tab() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = try openRunFixtureDir("omit-run")
        let project = seedProject(appState, projectStore, path: dir.path)
        _ = await handler.handle(request("tab.new", args: ControlArgs(run: "grok")))
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 2)
        ...
        #expect(opened.ok)
        #expect(workspace.tabs.count == 2)
        #expect(opened.data?.panes == nil)
        #expect(opened.data?.projects?.first?.id == project.id.uuidString)

        let emptyRun = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "")
        ))
        #expect(emptyRun.ok)
        #expect(workspace.tabs.count == 2)
        #expect(emptyRun.data?.panes == nil)
    }

    @Test
    func project_open_run_matches_agent_icon_and_process_basename() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = try openRunFixtureDir("match-process")
        let project = seedProject(appState, projectStore, path: dir.path)
        let workspace = try #require(appState.workspaces[project.id])
        let firstPane = try #require(workspace.tabs[0].splitRoot.allPanes().first)
        firstPane.foregroundProcessName = "grok-aarch64-a"
        #expect(firstPane.command == nil)

        let grokOpen = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "grok")
        ))
        #expect(grokOpen.ok)
        #expect(workspace.tabs.count == 1)
        #expect(try #require(grokOpen.data?.panes?.first).session == firstPane.sessionName)

        firstPane.foregroundProcessName = "btop"
        let btopOpen = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "btop")
        ))
        #expect(btopOpen.ok)
        #expect(workspace.tabs.count == 1)
        #expect(try #require(btopOpen.data?.panes?.first).session == firstPane.sessionName)

        firstPane.foregroundProcessName = "grokify"
        let grokifyOpen = await handler.handle(request(
            "project.open", args: ControlArgs(path: dir.path, run: "grok")
        ))
        #expect(grokifyOpen.ok)
        #expect(workspace.tabs.count == 2)
        let created = try #require(grokifyOpen.data?.panes?.first)
        #expect(created.session != firstPane.sessionName)
        #expect(!created.session.isEmpty)
    }

    @Test
    func tab_new_run_always_creates_even_when_grok_exists() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let project = seedProject(appState, projectStore)
        _ = await handler.handle(request("tab.new", args: ControlArgs(run: "grok")))
        let again = await handler.handle(request("tab.new", args: ControlArgs(run: "grok")))
        #expect(again.ok)
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 3)
        #expect(workspace.tabs.suffix(2).allSatisfy { $0.splitRoot.allPanes().first?.command == "grok" })
    }
    ...
    @Test
    func tab_new_without_run_starts_grok_when_pref_on() async throws {
        ...
        let (handler, appState, projectStore) = makeHandler()
        let project = seedProject(appState, projectStore)
        let response = await handler.handle(request("tab.new"))
        #expect(response.ok)
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 2)
        #expect(workspace.tabs.last?.splitRoot.allPanes().first?.command == "grok")
    }

    @Test
    func tab_new_without_run_leaves_command_nil_when_pref_off() async throws {
        let prior = Preferences.shared.startGrokInNewTabs
        defer { Preferences.shared.startGrokInNewTabs = prior }
        ...
        let workspace = try #require(appState.workspaces[project.id])
        #expect(workspace.tabs.count == 2)
        #expect(workspace.tabs.last?.splitRoot.allPanes().first?.command == nil)
    }
    ...
    func pane_split_targets_session_selector() async throws {
        ...
        let unknown = await handler.handle(request(
            "pane.split", args: ControlArgs(session: "futuraterm-nope-000000000000", direction: "down")
        ))
        #expect(unknown.error?.code == .notFound)
    }
    ...
    @Test
    func pane_click_unknown_session_is_not_found() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        // Selector resolution runs before missing-cell / surface checks.
        let unknown = await handler.handle(request(
            "pane.click",
            args: ControlArgs(session: "futuraterm-nope-000000000000")
        ))
        #expect(unknown.error?.code == .notFound)

        let unknownWithCell = await handler.handle(request(
            "pane.click",
            args: ControlArgs(session: "futuraterm-nope-000000000000", cellCol: 1, cellRow: 1)
        ))
        #expect(unknownWithCell.error?.code == .notFound)
    }

    @Test
    func pane_click_without_surface_is_no_surface() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let response = await handler.handle(request(
            "pane.click", args: ControlArgs(cellCol: 1, cellRow: 1)
        ))
        #expect(response.error?.code == .noSurface)
        #expect(response.error?.message == "the pane's terminal isn't live yet")
    }

    @Test
    func pane_click_missing_or_out_of_range_cell_is_bad_request() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)

        let missing = await handler.handle(request("pane.click"))
        #expect(missing.error?.code == .badRequest)

        let missingRow = await handler.handle(request(
            "pane.click", args: ControlArgs(cellCol: 1)
        ))
        #expect(missingRow.error?.code == .badRequest)

        let missingCol = await handler.handle(request(
            "pane.click", args: ControlArgs(cellRow: 1)
        ))
        #expect(missingCol.error?.code == .badRequest)

        // Lower bound is knowable without a live grid; 0 is never a cell.
        let zeroCol = await handler.handle(request(
            "pane.click", args: ControlArgs(cellCol: 0, cellRow: 1)
        ))
        #expect(zeroCol.error?.code == .badRequest)
        #expect(zeroCol.error?.action?.contains("1-based") == true)

        let zeroRow = await handler.handle(request(
            "pane.click", args: ControlArgs(cellCol: 1, cellRow: 0)
        ))
        #expect(zeroRow.error?.code == .badRequest)
    }
    ...
    func pane_inspect_resolves_target_before_surface_check() async {
        ...
        let unknown = await handler.handle(request(
            "pane.inspect", args: ControlArgs(session: "futuraterm-nope-000000000000")
        ))
        #expect(unknown.error?.code == .notFound)
    }
    ...
    @Test
    func pane_dump_quiet_ms_out_of_range_is_bad_request() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let tooSmall = await handler.handle(request(
            "pane.dump", args: ControlArgs(quietMs: 10)
        ))
        #expect(tooSmall.error?.code == .badRequest)
        let tooBig = await handler.handle(request(
            "pane.dump", args: ControlArgs(quietMs: 4000)
        ))
        #expect(tooBig.error?.code == .badRequest)
        let timeoutSmall = await handler.handle(request(
            "pane.dump", args: ControlArgs(quietMs: 50, timeoutMs: 50)
        ))
        #expect(timeoutSmall.error?.code == .badRequest)
        let timeoutBig = await handler.handle(request(
            "pane.dump", args: ControlArgs(quietMs: 50, timeoutMs: 40000)
        ))
        #expect(timeoutBig.error?.code == .badRequest)
    }

    @Test
    func pane_dump_quiet_ms_headless_fails_fast() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let start = ContinuousClock.now
        let response = await handler.handle(request(
            "pane.dump", args: ControlArgs(quietMs: 200, timeoutMs: 5000)
        ))
    ...
    @Test
    func pane_choices_without_surface_is_no_surface() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let response = await handler.handle(request("pane.choices"))
        #expect(response.error?.code == .noSurface)
    }

    @Test
    func pane_choices_unknown_session_is_not_found() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let unknown = await handler.handle(request(
            "pane.choices",
    ...
    @Test
    func pane_choose_requires_choice_arg() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let missing = await handler.handle(request("pane.choose"))
        #expect(missing.error?.code == .badRequest)
        #expect(missing.error?.message.contains("choice") == true)
    }

    @Test
    func pane_choose_unknown_session_is_not_found() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let unknown = await handler.handle(request(
            "pane.choose",
            args: ControlArgs(session: "futuraterm-nope-000000000000", choice: 1)
        ))
        #expect(unknown.error?.code == .notFound)
    }

    @Test
    func pane_choose_without_surface_is_no_surface() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let response = await handler.handle(request("pane.choose", args: ControlArgs(choice: 1)))
        #expect(response.error?.code == .noSurface)
    }

    @Test
    func pane_selection_without_surface_is_no_surface() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let response = await handler.handle(request("pane.selection"))
        #expect(response.error?.code == .noSurface)
        #expect(response.error?.message == "the pane's terminal isn't live yet")
    }

    @Test
    func pane_selection_unknown_session_is_not_found() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let unknown = await handler.handle(request(
            "pane.selection",
    ...
    @Test
    func pane_select_unknown_session_is_not_found() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let unknown = await handler.handle(request(
            "pane.select",
            args: ControlArgs(session: "futuraterm-nope-000000000000", line: 1)
        ))
        #expect(unknown.error?.code == .notFound)
    }

    @Test
    func pane_select_without_surface_is_no_surface() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)
        let response = await handler.handle(request(
            "pane.select", args: ControlArgs(line: 1)
        ))
        #expect(response.error?.code == .noSurface)
        #expect(response.error?.message == "the pane's terminal isn't live yet")
    }
    ...
    @Test
    func pane_select_missing_or_mixed_mode_is_bad_request() async {
        let (handler, appState, projectStore) = makeHandler()
        _ = seedProject(appState, projectStore)

        let missing = await handler.handle(request("pane.select"))
        #expect(missing.error?.code == .badRequest)

        let mixed = await handler.handle(request(
            "pane.select", args: ControlArgs(start: 0, length: 4, line: 1)
        ))
        #expect(mixed.error?.code == .badRequest)

        let mixedCell = await handler.handle(request(
            "pane.select", args: ControlArgs(cellCol: 1, cellRow: 1, line: 2)
        ))
        #expect(mixedCell.error?.code == .badRequest)

        let incompleteRange = await handler.handle(request(
            "pane.select", args: ControlArgs(start: 0)
        ))
        #expect(incompleteRange.error?.code == .badRequest)

        let incompleteCell = await handler.handle(request(
            "pane.select", args: ControlArgs(cellCol: 1)
        ))
        #expect(incompleteCell.error?.code == .badRequest)

        let zeroLine = await handler.handle(request(
            "pane.select", args: ControlArgs(line: 0)
        ))
        #expect(zeroLine.error?.code == .badRequest)
    }
    ...
    func session_kill_verifies_then_kills() async {
        ...
        appState.zmx = ZmxClient(
            ...
            remoteForegrounds: { _, _ in .unreachable },
            sweepRemoteOrphans: { _, _, _, _ in nil },
            listSessionsWithClients: { [.init(name: "futuraterm-x-000011112222", clients: 0)] },
            sessionLeaderPIDs: { [:] },
            sessionListSnapshot: { (entries: [.init(name: "futuraterm-x-000011112222", clients: 0)], leaders: [:]) }
        )

        let miss = await handler.handle(request("session.kill", args: ControlArgs(session: "futuraterm-y-000011112222")))
        #expect(miss.error?.code == .notFound)
        #expect(await killed.names.isEmpty)

        let hit = await handler.handle(request("session.kill", args: ControlArgs(session: "futuraterm-x-000011112222")))
        #expect(hit.ok)
        #expect(await killed.names == ["futuraterm-x-000011112222"])
    }
    ...
    func layout_save_then_apply_round_trips() async throws {
        let (handler, appState, projectStore) = makeHandler()
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-layout-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
