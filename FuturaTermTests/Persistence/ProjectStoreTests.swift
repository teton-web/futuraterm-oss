...
struct ProjectStoreTests {
    ...
    @Test
    func setPath_keeps_bookmark_when_new_path_is_inside_old_folder() {
        let store = makeStore()
        let blob = Data("bookmark-bytes".utf8)
        let created = store.create(
            name: "x",
            path: "/tmp/proj",
            securityScopedBookmark: blob
        )
        store.setPath(id: created.id, to: "/tmp/proj/src")
        let updated = store.projects.first { $0.id == created.id }
        #expect(updated?.path == "/tmp/proj/src")
        #expect(updated?.securityScopedBookmark == blob)
    }

    @Test
    func setPath_clears_bookmark_when_new_path_leaves_granted_folder() {
        let store = makeStore()
        let blob = Data("bookmark-bytes".utf8)
        let created = store.create(
            name: "x",
        ...
        store.setPath(id: created.id, to: "/tmp/other")
        let updated = store.projects.first { $0.id == created.id }
        #expect(updated?.path == "/tmp/other")
        #expect(updated?.securityScopedBookmark == nil)
    }

    @Test
    func setPath_persists_path_and_bookmark_together() {
        let store = makeStore()
        let old = Data("old-bookmark".utf8)
        let fresh = Data("new-bookmark".utf8)
        let created = store.create(
            name: "x",
            path: "/tmp/proj",
            securityScopedBookmark: old
        )
        store.setPath(id: created.id, to: "/tmp/other", bookmark: fresh)
        let updated = store.projects.first { $0.id == created.id }
        #expect(updated?.path == "/tmp/other")
        #expect(updated?.securityScopedBookmark == fresh)
    }
    ...
    func decode_missing_groupID_is_nil() throws {
        ...
        #expect(store.projects.first?.securityScopedBookmark == nil)
    }

    @Test
    func decode_missing_securityScopedBookmark_is_nil() throws {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-project-store-nobookmark-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let json = """
        [{"id":"11111111-1111-1111-1111-111111111111","name":"alpha","path":"/tmp/alpha","sortOrder":0,"createdAt":0}]
        """
        ...
        #expect(store.projects.count == 1)
        #expect(store.projects.first?.securityScopedBookmark == nil)
        #expect(store.projects.first?.path == "/tmp/alpha")
    }

    @Test
    func remote_encode_omits_bookmark() throws {
        let remote = Project(name: "api", path: "devbox:~/dev/api", securityScopedBookmark: Data([1, 2, 3]))
        #expect(remote.securityScopedBookmark == nil)
        let data = try JSONEncoder().encode(remote)
        let object = try JSONSerialization.jsonObject(with: data) as? [String: Any]
        #expect(object?["securityScopedBookmark"] == nil)
        #expect(object?["path"] as? String == "devbox:~/dev/api")
    }

    @Test
    func local_bookmark_round_trips_through_the_file() {
        let fileURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-project-store-bookmark-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: fileURL) }
        let blob = Data("bookmark-bytes".utf8)
        let writer = makeStore(fileURL: fileURL)
        let created = writer.create(name: "alpha", path: "/tmp/alpha", securityScopedBookmark: blob)
        #expect(created.securityScopedBookmark == blob)

        let reader = makeStore(fileURL: fileURL)
        #expect(reader.projects.first?.securityScopedBookmark == blob)
    }
    ...
    @Test
    func dropFolder_same_parent_roots_reorders() throws {
        let store = makeStore()
        let work = try #require(store.createGroup(name: "Work"))
        let home = try #require(store.createGroup(name: "Home"))
        #expect(store.dropFolder(work.id, ontoFolder: home.id) == .reordered)
        #expect(store.childGroups(in: nil).map(\.id) == [home.id, work.id])
        #expect(store.groups.first { $0.id == work.id }?.parentID == nil)
        #expect(store.groups.first { $0.id == home.id }?.parentID == nil)
    }

    @Test
    func dropFolder_nested_siblings_reorders_without_changing_parent() throws {
        let store = makeStore()
        let clients = try #require(store.createGroup(name: "Clients"))
        let alpha = try #require(store.createGroup(name: "Alpha", parentID: clients.id))
        let beta = try #require(store.createGroup(name: "Beta", parentID: clients.id))
        #expect(store.dropFolder(beta.id, ontoFolder: alpha.id) == .reordered)
        #expect(store.childGroups(in: clients.id).map(\.id) == [beta.id, alpha.id])
        #expect(store.groups.first { $0.id == beta.id }?.parentID == clients.id)
        #expect(store.groups.first { $0.id == alpha.id }?.parentID == clients.id)
    }
    ...
    @Test
    func dropFolder_different_parent_nests() throws {
        let store = makeStore()
        let work = try #require(store.createGroup(name: "Work"))
        let home = try #require(store.createGroup(name: "Home"))
        let nested = try #require(store.createGroup(name: "Nested", parentID: work.id))
        #expect(store.dropFolder(nested.id, ontoFolder: home.id) == .nested)
        #expect(store.groups.first { $0.id == nested.id }?.parentID == home.id)
        #expect(store.childGroups(in: work.id).isEmpty)
    }

    @Test
    func dropFolder_cycle_and_self_are_ignored() throws {
        let store = makeStore()
        let clients = try #require(store.createGroup(name: "Clients"))
        let kectil = try #require(store.createGroup(name: "Kectil", parentID: clients.id))
        #expect(store.dropFolder(clients.id, ontoFolder: clients.id) == .ignored)
        #expect(store.dropFolder(clients.id, ontoFolder: kectil.id) == .ignored)
        #expect(store.groups.first { $0.id == clients.id }?.parentID == nil)
        #expect(store.groups.first { $0.id == kectil.id }?.parentID == clients.id)
    }
}
