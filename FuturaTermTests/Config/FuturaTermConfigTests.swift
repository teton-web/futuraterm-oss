...
/// Pins the `futuraterm-overrides.conf` wire contract with libghostty — most
...
@MainActor
struct FuturaTermConfigTests {
    // MARK: - overridesBody: the translucency contract

    @Test
    func never_paints_default_background() {
        let body = FuturaTermConfig.overridesBody(windowOpacity: 1.0, userConfigText: nil, shimDirectory: nil)
        #expect(body.contains("background-default-transparent = true"))
    }

    @Test
    func window_opacity_is_the_real_background_opacity() {
        let body = FuturaTermConfig.overridesBody(windowOpacity: 0.85, userConfigText: nil, shimDirectory: nil)
    ...
    func ghostty_blur_stays_off() {
        // FuturaTerm calls the CGS blur SPI itself; ghostty's blur would
        // compose on top of it.
        let body = FuturaTermConfig.overridesBody(windowOpacity: 0.5, userConfigText: nil, shimDirectory: nil)
    ...
    func shim_directory_feeds_ghostty_bin_dir_and_keeps_ssh_features() {
        let body = FuturaTermConfig.overridesBody(windowOpacity: 1.0, userConfigText: nil, shimDirectory: "/tmp/shim")
    ...
    func user_features_survive_the_merge() {
        ...
        let body = FuturaTermConfig.overridesBody(windowOpacity: 1.0, userConfigText: user, shimDirectory: "/tmp/shim")
