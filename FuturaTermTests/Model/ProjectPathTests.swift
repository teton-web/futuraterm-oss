...
struct ProjectPathTests {
    ...
    @Test
    func resolved_local_folds_darwin_private_aliases() {
        #expect(ProjectPath.resolvedLocal("/private/var/folders/xx/T")
            == ProjectPath.resolvedLocal("/var/folders/xx/T"))
        #expect(ProjectPath.resolvedLocal("/private/tmp/f44")
            == ProjectPath.resolvedLocal("/tmp/f44"))
        #expect(ProjectPath.resolvedLocal("/private/etc/hosts")
            == ProjectPath.resolvedLocal("/etc/hosts"))
        #expect(ProjectPath.resolvedLocal("/private/var") == ProjectPath.resolvedLocal("/var"))
        // Prefix only — `/private/variable` is not `/var`.
        #expect(ProjectPath.resolvedLocal("/private/variable") == "/private/variable")
    }
    ...
    func canonical_does_not_resolve_symlinks() throws {
        ...
        #expect(ProjectPath.canonicalLocal(link.path) != ProjectPath.canonicalLocal(real.path))
        #expect(!ProjectPath.matches(link.path, real.path))
    }

    @Test
    func resolved_local_follows_existing_symlinks() throws {
        let fm = FileManager.default
        let base = fm.temporaryDirectory
            .appendingPathComponent("projectpath-\(UUID().uuidString)")
        let real = base.appendingPathComponent("real")
        ...
        #expect(ProjectPath.resolvedLocal(link.path) == ProjectPath.resolvedLocal(real.path))
        #expect(ProjectPath.canonicalLocal(link.path) != ProjectPath.canonicalLocal(real.path))
    }

    @Test
    func resolved_local_follows_longest_existing_prefix() throws {
        let fm = FileManager.default
        let base = fm.temporaryDirectory
            .appendingPathComponent("projectpath-prefix-\(UUID().uuidString)")
        let real = base.appendingPathComponent("real")
        let link = base.appendingPathComponent("link")
        try fm.createDirectory(at: real, withIntermediateDirectories: true)
        try fm.createSymbolicLink(at: link, withDestinationURL: real)
        ...
        let missingOnLink = link.appendingPathComponent("src", isDirectory: true).path
        let missingOnReal = real.appendingPathComponent("src", isDirectory: true).path
        #expect(ProjectPath.resolvedLocal(missingOnLink) == ProjectPath.resolvedLocal(missingOnReal))
        #expect(ProjectPath.canonicalLocal(missingOnLink).hasSuffix("/link/src"))
        #expect(ProjectPath.canonicalLocal(missingOnLink) != ProjectPath.canonicalLocal(missingOnReal))
    }
    ...
    func compose_remote_validates_through_the_parser() {
        ...
        #expect(ProjectPath.remoteHost(from: "me@box.tailnet.ts.net:~/dev") == "box.tailnet.ts.net")
        #expect(ProjectPath.remoteHost(from: "box:~/dev") == "box")
        #expect(ProjectPath.remoteHost(from: "/Users/me/dev") == nil)
    }
