...
struct TerminalExecutionTrackerTests {
    ...
    @Test
    func nestedAgents_zeroRefreshDoesNotBlockActivityQuietSettle() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        let start = Date(timeIntervalSince1970: 100)
        var state = tracker.markTerminalActivity(at: start, currentState: .idle)
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        state = tracker.settleIfQuiet(now: start.addingTimeInterval(3), quietInterval: 3, currentState: state)
        #expect(state == .done)
    }

    @Test
    func nestedAgents_rawTUI_survivesQuietSettleThenCompletes() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        let rawAt = Date(timeIntervalSince1970: 100)
        var state = tracker.refreshForeground(
        ...
        state = tracker.refreshNestedAgents(count: 2, currentState: state)
        #expect(state == .running)
        state = tracker.settleIfQuiet(now: rawAt.addingTimeInterval(4), quietInterval: 3, currentState: state)
        #expect(state == .running)
    ...
    @Test
    func nestedAgents_zeroFromIdle_neverStartsARun() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        var state = tracker.refreshForeground(
            name: "grok", pid: 42, foregroundIsShell: false, terminalInputIsRaw: true, currentState: .idle
        )
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        state = tracker.settleIfQuiet(
            now: Date(timeIntervalSince1970: 10), quietInterval: 3, currentState: state
        )
        #expect(state == .idle)
    }

    @Test
    func nestedAgents_withoutInteraction_stayIdle() {
        var tracker = TerminalExecutionTracker()
        let state = tracker.refreshNestedAgents(count: 2, currentState: .idle)
        #expect(state == .idle)
    }

    @Test
    func nestedAgents_doNotOverrideProgress() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        var state = tracker.markProgressStarted(currentState: .idle)
        state = tracker.refreshNestedAgents(count: 2, currentState: state)
        #expect(state == .running)
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        #expect(state == .running)
        state = tracker.settleIfQuiet(
            now: Date(timeIntervalSince1970: 100), quietInterval: 3, currentState: state
        )
        #expect(state == .running)
        state = tracker.markProgressFinished(currentState: state)
        #expect(state == .done)
    ...
    @Test
    func nestedAgents_doNotOverrideCanonicalForeground() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        var state = tracker.refreshForeground(
            name: "sleep", pid: 42, foregroundIsShell: false, terminalInputIsRaw: false, currentState: .idle
        ...
        state = tracker.refreshNestedAgents(count: 2, currentState: state)
        state = tracker.settleIfQuiet(
            now: Date(timeIntervalSince1970: 30), quietInterval: 3, currentState: state
        )
        #expect(state == .running)
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        state = tracker.settleIfQuiet(
            now: Date(timeIntervalSince1970: 60), quietInterval: 3, currentState: state
        )
        #expect(state == .running)
        state = tracker.refreshForeground(
            name: "zsh", pid: 43, foregroundIsShell: true, terminalInputIsRaw: false, currentState: state
    ...
    @Test
    func nestedAgents_activityHeartbeatDoesNotQuietSettle() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        let start = Date(timeIntervalSince1970: 100)
        var state = tracker.refreshNestedAgents(count: 2, currentState: .idle)
        state = tracker.markTerminalActivity(at: start, currentState: state)
        state = tracker.settleIfQuiet(now: start.addingTimeInterval(5), quietInterval: 3, currentState: state)
        #expect(state == .running)
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        #expect(state == .done)
    ...
    @Test
    func nestedAgents_overwriteStaleActivityThenComplete() {
        var tracker = TerminalExecutionTracker()
        tracker.recordUserInteraction()
        let start = Date(timeIntervalSince1970: 100)
        var state = tracker.refreshForeground(
        ...
        state = tracker.refreshNestedAgents(count: 2, currentState: state)
        state = tracker.settleIfQuiet(now: start.addingTimeInterval(10), quietInterval: 3, currentState: state)
        #expect(state == .running)
        state = tracker.refreshNestedAgents(count: 0, currentState: state)
        #expect(state == .done)
    }
