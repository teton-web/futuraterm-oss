...
struct TerminalTabTests {
    ...
    @Test
    func hasNestedAgents_is_false_when_every_pane_is_zero() {
        let (tab, _) = makeTab(H(pane("a"), pane("b")), focused: "a")
        #expect(!tab.hasNestedAgents)
    }

    @Test
    func hasNestedAgents_is_true_when_any_pane_has_nested_count() throws {
        let (tab, ids) = makeTab(H(pane("a"), pane("b")), focused: "a")
        let bID = try #require(ids["b"])
        let aID = try #require(ids["a"])
        let b = try #require(tab.splitRoot.findPane(id: bID))
        b.applyForegroundRefresh(
            name: "grok",
            foregroundPID: 1,
        ...
        #expect(b.nestedAgentCount == 2)
        #expect(tab.hasNestedAgents)
        let a = try #require(tab.splitRoot.findPane(id: aID))
        #expect(a.nestedAgentCount == 0)
    }
