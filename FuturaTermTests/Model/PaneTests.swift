...
struct PaneTests {
    ...
    @Test
    func resetSurfaceForFirstSpawn_clears_hasBuiltSurface_after_destroy() {
        let p = Pane(projectPath: "/tmp", projectID: UUID(), command: "echo hi")
        _ = p.ensureNSView()
        #expect(p.hasBuiltSurface)
        p.destroySurface()
        #expect(p.hasBuiltSurface)
        p.resetSurfaceForFirstSpawn()
        #expect(!p.hasBuiltSurface)
    }
    ...
    @Test
    func applyForegroundRefresh_skipsExecutionState_whenIndicatorDisabled_evenWithNestedAgents() {
        let p = Pane(projectPath: "/", projectID: UUID())
        ...
        p.applyForegroundRefresh(
            ...
            applyExecutionState: false,
            nestedChildrenOf: { pid in pid == 1 ? [2, 3] : [] },
            nestedCommOf: { _ in "grok" }
        )
        #expect(p.agentIcon == .grok)
        #expect(p.nestedAgentCount == 2)
        #expect(p.executionState == .idle)
    }
    ...
    func session_name_slugs_from_project_path_basename() throws {
        ...
        let p = Pane(projectPath: "/Users/me/dev/FuturaTerm", projectID: UUID(), sessionID: id)
        #expect(p.sessionName == "futuraterm-futuraterm-aaaaaaaabbbb")
        #expect(p.sessionSlug == "FuturaTerm")
    }
    ...
    func explicit_session_slug_wins_over_path_basename() {
        ...
        #expect(p.sessionName.hasPrefix("futuraterm-quick-"))
    }
    ...
    func split_inherits_the_source_panes_session_slug() throws {
        ...
        #expect(newPane.sessionName.hasPrefix("futuraterm-myproj-"))
    ...
    func pane_has_no_agent_without_a_foreground_pid() {
        ...
        #expect(pane.agentIcon == nil)
        #expect(pane.nestedAgentCount == 0)
    }

    @Test
    func nestedAgentCount_stays_zero_until_agentIcon_is_set() {
        let pane = Pane(projectPath: "/", projectID: UUID())
        #expect(pane.nestedAgentCount == 0)
        var walked = 0
        ...
        #expect(pane.nestedAgentCount == 2)

        pane.applyForegroundRefresh(name: "zsh", foregroundPID: 7)
        #expect(pane.agentIcon == nil)
        #expect(pane.nestedAgentCount == 0)
    }

    @Test
    func nestedAgentCount_remote_pane_stays_zero() {
    ...
    @Test
    func nestedAgents_rawGrok_keepsRunningThroughQuietSettleThenCompletes() {
        let pane = Pane(projectPath: "/", projectID: UUID())
        ...
        pane.settleTerminalActivityIfQuiet(now: Date(timeIntervalSince1970: 10), quietInterval: 3)
        #expect(pane.executionState == .running)
    ...
    @Test
    func nestedAgents_idleGrokWithZeroChildren_staysIdle() {
        let pane = Pane(projectPath: "/", projectID: UUID())
        pane.recordUserInteraction()
        pane.applyForegroundRefresh(
            name: "grok",
    ...
    @Test
    func nestedAgents_withoutInteraction_doNotStartExecution() {
        let pane = Pane(projectPath: "/", projectID: UUID())
        pane.applyForegroundRefresh(
            name: "grok",
            foregroundPID: 1,
    ...
    @Test
    func nestedAgents_promptHookAfterMarkCommandFinished_doesNotResurrectRunning() {
        let pane = Pane(projectPath: "/", projectID: UUID())
        pane.recordUserInteraction()
        pane.applyForegroundRefresh(
            name: "grok",
        ...
        #expect(pane.nestedAgentCount == 0)
        #expect(walked == 0)
        #expect(pane.foregroundProcessName == "grok")
        #expect(pane.agentIcon == .grok)
        ...
        #expect(pane.nestedAgentCount == 0)
        #expect(walked == 0)
        #expect(pane.foregroundProcessName != "mise")
