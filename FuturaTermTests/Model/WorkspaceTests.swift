...
struct WorkspaceTests {
    ...
    @Test
    func adoptTab_activate_false_preserves_selection() {
        let ws = makeWorkspace()
        let original = ws.tabs[0].id
        let incoming = TerminalTab(projectPath: "/elsewhere", projectID: UUID())
        ws.adoptTab(incoming, activate: false)
        #expect(ws.activeTabID == original)
        #expect(ws.tabs.last?.id == incoming.id)
    }

    @Test
    func adoptTab_activate_false_selects_when_empty() {
        let ws = Workspace(projectID: UUID(), tabs: [], activeTabID: nil)
        let incoming = TerminalTab(projectPath: "/elsewhere", projectID: UUID())
        ws.adoptTab(incoming, activate: false)
        #expect(ws.activeTabID == incoming.id)
        #expect(ws.tabs.map(\.id) == [incoming.id])
    }
