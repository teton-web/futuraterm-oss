...
struct AgentIconTests {
    ...
    @Test
    func spinnerTint_nested_running_is_copilot_purple() {
        let tint = TabStatusSpinnerTint.resolve(running: true, nested: true)
        #expect(tint == .nestedAgents)
        #expect(tint.color == AgentIcon.nestedAgentSpinnerTint)
        #expect(AgentIcon.nestedAgentSpinnerTint == AgentIcon.copilot.brandColor)
    }

    @Test
    func spinnerTint_running_only_is_secondary() {
        let tint = TabStatusSpinnerTint.resolve(running: true, nested: false)
        #expect(tint == .secondary)
        #expect(tint.color == Color.secondary)
    }

    @Test
    func spinnerTint_not_running_has_no_spinner_even_if_nested() {
        #expect(TabStatusSpinnerTint.resolve(running: false, nested: true) == .none)
        #expect(TabStatusSpinnerTint.resolve(running: false, nested: false) == .none)
        #expect(TabStatusSpinnerTint.resolve(running: false, nested: true).color == nil)
        #expect(TabStatusSpinnerTint.resolve(running: false, nested: false).color == nil)
    }
}
