...
/// Verifies the ghostty resources FuturaTerm ships in its bundle are complete and
...
///
/// These assert against `FuturaTerm/Resources/` in the repo (where `setup.sh`
...
struct BundledResourcesTests {
    /// Repo `FuturaTerm/Resources` dir, located relative to this source file.
    private static let resourcesDir: URL? = {
        // …/FuturaTermTests/Ghostty/BundledResourcesTests.swift → repo root is 3 up.
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // Ghostty
            .deletingLastPathComponent() // FuturaTermTests
            .deletingLastPathComponent() // repo root
        let dir = root.appendingPathComponent("FuturaTerm/Resources")
    ...
    func terminfo_is_a_sibling_of_the_ghostty_resources_dir() throws {
        try #require(resourcesPresent, "run `mise run setup` to populate FuturaTerm/Resources")
