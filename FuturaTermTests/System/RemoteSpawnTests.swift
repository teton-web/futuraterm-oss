...
struct RemoteSpawnTests {
    ...
    func pane_command_execs_zmx_after_guarding_zmx_and_cd() {
        ...
        #expect(cmd?.contains("exec zmx attach \"futuraterm-api-abc123\"") == true)
    }
    ...
    func pane_command_with_explicit_zmx_path_skips_the_presence_guard() {
        ...
        let cmd = RemoteSpawn.paneCommand(
            remote: remote, sessionName: "futuraterm-api-abc123", zmxPath: "~/bin/zmx"
        )
        #expect(cmd?.contains("command -v zmx") == false)
        #expect(cmd?.contains("exec \"~/bin/zmx\" attach \"futuraterm-api-abc123\"") == true)
    }
    ...
    func orphan_sweep_stamps_only_the_names_passed_then_lists() {
        ...
        let script = RemoteSpawn.orphanSweepScript(
            sessionNames: ["futuraterm-api-aa11", "futuraterm-api-bb22"],
            ownerID: "cafe01",
            zmxPath: nil
        )
        #expect(script.contains("for n in \"futuraterm-api-aa11\" \"futuraterm-api-bb22\""))
        #expect(script.contains("zmx set \"$n\" \"futuraterm.owner=cafe01\""))
    ...
    func orphan_sweep_argv_uses_the_background_ssh_profile() {
        ...
        let argv = RemoteSpawn.orphanSweepArgv(
            ...
            sessionNames: ["futuraterm-api-aa11"], ownerID: "cafe01"
        )
    ...
    func remote_scripts_never_source_profiles() {
        ...
        let sweep = RemoteSpawn.orphanSweepArgv(
            remote: remote, zmxPath: nil, sessionNames: ["futuraterm-a-1"], ownerID: "x"
        )?.joined(separator: " ") ?? ""
    ...
    func pane_command_includes_user_in_destination() {
        let cmd = RemoteSpawn.paneCommand(
            ...
            sessionName: "futuraterm-app-ff00"
        )
    ...
    func op_argv_uses_batchmode_and_connect_timeout() {
        let argv = RemoteSpawn.opArgv(remote: remote, zmxArguments: ["kill", "futuraterm-api-abc123"])
        #expect(argv == [
            "-o", "BatchMode=yes",
            "-o", "ConnectTimeout=5",
            "devbox",
            "\(RemoteSpawn.remoteShell) " + RemoteSpawn.shellQuote(
                RemoteSpawn.remoteEnvPreamble + "exec zmx \"kill\" \"futuraterm-api-abc123\""
            ),
        ])
    ...
    @Test
    func sunshine_pair_argv_is_batchmode_localhost_helper() {
        let argv = RemoteSpawn.sunshinePairArgv(remote: remote, pin: "4821", clientName: "dts-0")
        #expect(argv?.prefix(4) == ["-o", "BatchMode=yes", "-o", "ConnectTimeout=5"])
        #expect(argv?.dropFirst(4).first == "devbox")
        let script = argv?.last ?? ""
        #expect(script.contains("futuraterm-sunshine-pair"))
        #expect(script.contains("--pin 4821"))
        #expect(script.contains("--name \"dts-0\""))
        #expect(script.hasPrefix("sh -c '"))
        #expect(RemoteSpawn.sunshinePairArgv(remote: .local("/a"), pin: "4821", clientName: "x") == nil)
        #expect(RemoteSpawn.sunshinePairArgv(remote: remote, pin: "12", clientName: "x") == nil)
        #expect(RemoteSpawn.sunshinePairClientName("bad name!") == "badname")
    }

    @Test
    func sunshine_probe_argv_is_batchmode_localhost() {
        let argv = RemoteSpawn.sunshineProbeArgv(remote: remote)
        #expect(argv?.prefix(4) == ["-o", "BatchMode=yes", "-o", "ConnectTimeout=5"])
        let script = argv?.last ?? ""
        #expect(script.contains("127.0.0.1:47990"))
        #expect(script.contains("curl"))
        #expect(script.hasPrefix("sh -c '"))
        #expect(RemoteSpawn.sunshineProbeArgv(remote: .local("/a")) == nil)
    }
    ...
    func assert_single_quote_free_passes_clean_scripts_through() {
        let clean = "PATH=$PATH; exec \"zmx\" attach \"futuraterm-x-abc\""
    ...
    func pane_command_with_single_quote_in_directory_stays_quote_free() {
        ...
        let cmd = RemoteSpawn.paneCommand(remote: remoteWithQuote, sessionName: "futuraterm-x-abc123")
    ...
    func op_argv_with_single_quote_in_zmx_path_stays_quote_free() {
        let cmd = RemoteSpawn.opArgv(
            remote: remote, zmxArguments: ["kill", "futuraterm-x-abc123"], zmxPath: "~/it's/zmx"
        )
