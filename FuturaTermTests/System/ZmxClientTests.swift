...
struct ZmxSessionListParserTests {
    ...
    func parsesHealthySessionsWithClientCounts() {
        let stdout = """
        → name=futuraterm-abc\tclients=1\tcreated=123
          name=futuraterm-def\tclients=0\tcreated=456
        """
        let entries = ZmxSessionListParser.parse(stdout)
        #expect(entries == [
            .init(name: "futuraterm-abc", clients: 1),
            .init(name: "futuraterm-def", clients: 0),
        ])
    }
    ...
    func absentClientCountIsUnknownNotZero() {
        ...
        let entries = ZmxSessionListParser.parse("  name=futuraterm-xyz\terr=unreachable")
        #expect(entries == [.init(name: "futuraterm-xyz", clients: nil)])
    }

    @Test
    func ignoresForeignSessionsWithoutPrefix() {
        // Only `futuraterm-` sessions are ours; a co-resident tmux/other-app zmx
    ...
    func extractsTheOwnerLabelWhenPresent() {
        ...
        let stdout = """
        name=futuraterm-abc\tpid=42\tclients=0\tcreated=123\tcwd=/home/u\tfuturaterm.owner=cafe01
          name=futuraterm-def\tclients=0\tcreated=456
        """
        let entries = ZmxSessionListParser.parse(stdout)
        #expect(entries == [
            .init(name: "futuraterm-abc", clients: 0, owner: "cafe01"),
            .init(name: "futuraterm-def", clients: 0, owner: nil),
        ])
    }
}
...
struct ZmxSocketBudgetTests {
    ...
    func defaultTmpDirIsUnderBudget() {
        // `/tmp/zmx-<uid>` (~13 chars) + `/futuraterm-<UUID>` (45) is well under
...
struct ZmxSessionNameTests {
    ...
    func formats_project_slug_and_short_hex() {
        #expect(ZmxSessionName.make(projectName: "FuturaTerm", paneSessionID: id)
            == "futuraterm-futuraterm-aaaaaaaabbbb")
    }
    ...
    func slug_recovery_rejects_foreign_or_corrupt_names() {
        ...
        #expect(ZmxSessionName.slug(fromName: "futuraterm-nohex") == nil)
        #expect(ZmxSessionName.slug(fromName: "futuraterm--aaaaaaaabbbb") == nil)
        // Slugs may themselves contain hex-looking segments; only a trailing
        // 12-hex-digit component counts as the id.
        #expect(ZmxSessionName.slug(fromName: "futuraterm-abc-def-aaaaaaaabbbb") == "abc-def")
    }
}
...
struct ZmxReaperTests {
    ...
    @Test
    func reaps_unclaimed_detached_futuraterm_sessions() {
        let entries = [
            entry("futuraterm-a", clients: 0), // orphan → reap
            entry("futuraterm-b", clients: 0), // claimed → spare
        ]
        let orphans = ZmxReaper.orphans(in: entries, known: ["futuraterm-b"])
        #expect(orphans == ["futuraterm-a"])
    }
    ...
    func spares_attached_sessions_even_if_unclaimed() {
        ...
        let orphans = ZmxReaper.orphans(in: [entry("futuraterm-x", clients: 1)], known: [])
    ...
    func spares_unknown_client_count() {
        ...
        let orphans = ZmxReaper.orphans(in: [entry("futuraterm-x", clients: nil)], known: [])
        #expect(orphans.isEmpty)
    }

    @Test
    func never_touches_foreign_prefix_sessions() {
        // A co-resident Supacode session must never be reaped by FuturaTerm.
        let entries = [
            entry("supa-abc", clients: 0),
            entry("futuraterm-y", clients: 0),
        ]
        let orphans = ZmxReaper.orphans(in: entries, known: [])
        #expect(orphans == ["futuraterm-y"])
    }

    @Test
    func empty_listing_reaps_nothing() {
        #expect(ZmxReaper.orphans(in: [], known: ["futuraterm-a"]).isEmpty)
    }
    ...
    func remote_reaps_only_our_labelled_unclaimed_detached_sessions() {
        let entries = [
            entry("futuraterm-a", clients: 0, owner: "us"), // ours, orphan → reap
            entry("futuraterm-b", clients: 0, owner: "them"), // another machine → spare
            entry("futuraterm-c", clients: 0, owner: nil), // unstamped → spare
            entry("futuraterm-d", clients: 0, owner: "us"), // claimed → spare
            entry("futuraterm-e", clients: 1, owner: "us"), // attached → spare
            entry("futuraterm-f", clients: nil, owner: "us"), // unknown count → spare
            entry("supa-g", clients: 0, owner: "us"), // foreign prefix → spare
        ]
        let orphans = ZmxReaper.orphans(in: entries, known: ["futuraterm-d"], owner: "us")
        #expect(orphans == ["futuraterm-a"])
    }
...
struct ZmxAttachTests {
    ...
    func wrapperArgvWrapsTheShellWhenExecutablePresent() {
        let argv = ZmxAttach.wrapperArgv(executablePath: "/path/to/zmx", sessionID: "futuraterm-1")
        #expect(argv == ["/path/to/zmx", "attach", "futuraterm-1"])
    }

    @Test
    func noExecutableYieldsEmptyArgv() {
        // nil executable (zmx unbundled or over budget) → no wrapper, plain shell.
        #expect(ZmxAttach.wrapperArgv(executablePath: nil, sessionID: "futuraterm-1").isEmpty)
    }
}
...
struct ZmxEnvironmentTests {
    ...
    func scrub_removes_inherited_session_marker() {
        // An app launched from inside a FuturaTerm pane inherits the launcher's
        ...
        setenv("ZMX_SESSION", "futuraterm-test-deadbeefcafe", 1)
    ...
    func reapsOnlyUnclaimedDetachedSessions() async {
        ...
        let client = recordingClient(
            entries: [
                ...
                .init(name: "futuraterm-orphan", clients: 0), // unclaimed → reap
                .init(name: "futuraterm-attached", clients: 1), // attached → spare
                .init(name: "supa-foreign", clients: 0), // foreign prefix → spare
            ],
            killed: killed
        )
        await client.reapOrphans(knownSessionNames: [knownName])
        #expect(killed.value == ["futuraterm-orphan"])
    }
