...
struct ZmxForegroundResolverParseTests {
    ...
    func parses_session_name_to_leader_pid() {
        let stdout = """
          name=futuraterm-proj-abc123\tpid=46878\tclients=1\tcreated=123
          name=futuraterm-proj-def456\tpid=47353\tclients=0\tcreated=456
        """
        let map = ZmxForegroundResolver.parseLeaderPIDs(stdout)
        #expect(map == ["futuraterm-proj-abc123": 46878, "futuraterm-proj-def456": 47353])
    }
    ...
    func skips_foreign_prefix_and_pidless_lines() {
        let stdout = """
          ...
          name=futuraterm-nopid\tclients=0
          name=futuraterm-ok\tpid=42\tclients=1
        """
        let map = ZmxForegroundResolver.parseLeaderPIDs(stdout)
        #expect(map == ["futuraterm-ok": 42])
    }
}
