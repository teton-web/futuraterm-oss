...
struct RemoteForegroundResolverTests {
    ...
    func parses_session_tab_comm_tab_idleflag_tab_args_lines() {
        // Garbage and non-futuraterm sessions drop; an empty args field is a
        // nil command.
        let out = """
        futuraterm-api-abc123\tbtop\t0\tbtop --utf-force
        futuraterm-api-def456\t/usr/local/bin/hx\t0\t
        garbage line
        ...
        #expect(map == [
            "futuraterm-api-abc123": RemoteForeground(comm: "btop", isIdle: false, command: "btop --utf-force"),
            "futuraterm-api-def456": RemoteForeground(comm: "/usr/local/bin/hx", isIdle: false, command: nil),
        ])
    }
    ...
    func parses_the_host_idle_flag() {
        ...
        let out = """
        futuraterm-api-idle\t-bash\t1\t-bash
        futuraterm-api-busy\tbtop\t0\tbtop
        futuraterm-api-unknown\thx\t\thx
        futuraterm-api-garbage\tvim\tmaybe\tvim
        """
        let map = RemoteForegroundResolver.parseProbeOutput(out)
        #expect(map == [
            "futuraterm-api-idle": RemoteForeground(comm: "-bash", isIdle: true, command: "-bash"),
            "futuraterm-api-busy": RemoteForeground(comm: "btop", isIdle: false, command: "btop"),
            "futuraterm-api-unknown": RemoteForeground(comm: "hx", isIdle: nil, command: "hx"),
            "futuraterm-api-garbage": RemoteForeground(comm: "vim", isIdle: nil, command: "vim"),
        ])
    }
    ...
    func args_keep_embedded_tabs_and_short_lines_still_parse() {
        ...
        let out = """
        futuraterm-api-abc123\tnode\t0\tnode server.js\t--flag
        futuraterm-api-legacy\thx
        futuraterm-api-flagonly\thx\t0
        """
        let map = RemoteForegroundResolver.parseProbeOutput(out)
        #expect(map == [
            "futuraterm-api-abc123": RemoteForeground(comm: "node", isIdle: false, command: "node server.js\t--flag"),
            "futuraterm-api-legacy": RemoteForeground(comm: "hx", isIdle: nil, command: nil),
            "futuraterm-api-flagonly": RemoteForeground(comm: "hx", isIdle: false, command: nil),
        ])
    }
