...
/// `com.davidsolheim.futuraterm.debug` domain. `Preferences.shared` must be backed by
...
struct PreferencesTests {
    ...
    @Test
    func start_grok_in_new_tabs_defaults_on_and_round_trips() {
        let prior = Preferences.shared.startGrokInNewTabs
        defer { Preferences.shared.startGrokInNewTabs = prior }

        #expect(Preferences.shared.startGrokInNewTabs)

        let standardBefore = UserDefaults.standard.object(forKey: Preferences.Keys.startGrokInNewTabs)
        Preferences.shared.startGrokInNewTabs = false
        #expect(Preferences.defaults.object(forKey: Preferences.Keys.startGrokInNewTabs) as? Bool == false)
        #expect(Preferences.shared.startGrokInNewTabs == false)

        let standardAfter = UserDefaults.standard.object(forKey: Preferences.Keys.startGrokInNewTabs)
        #expect(standardAfter as? Bool == standardBefore as? Bool)
    ...
    @Test
    func show_agent_usage_defaults_on_and_round_trips() {
        let prior = Preferences.shared.showAgentUsage
        defer { Preferences.shared.showAgentUsage = prior }

        #expect(Preferences.shared.showAgentUsage)

        let standardBefore = UserDefaults.standard.object(forKey: Preferences.Keys.showAgentUsage)
        Preferences.shared.showAgentUsage = false
        #expect(Preferences.defaults.object(forKey: Preferences.Keys.showAgentUsage) as? Bool == false)
        #expect(Preferences.shared.showAgentUsage == false)

        let standardAfter = UserDefaults.standard.object(forKey: Preferences.Keys.showAgentUsage)
        #expect(standardAfter as? Bool == standardBefore as? Bool)

        Preferences.shared.showAgentUsage = true
        #expect(Preferences.shared.showAgentUsage)
    }
    ...
    func selectProject_does_not_persist_to_the_standard_defaults_domain() {
        ...
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-prefs-tests-\(UUID().uuidString).json")
        let filesDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-prefs-tests-projects-\(UUID().uuidString)", isDirectory: true)
        // Inject a tempdir ProjectFileStore too, so this never reads/writes the
        // developer's real ~/.config/futuraterm/projects/ (tempdir-injection rule).
        let state = AppState(
        ...
        let standardRecency = UserDefaults.standard.stringArray(forKey: "futuraterm.projectRecency") ?? []
    ...
    private func isolatedDefaults() throws -> UserDefaults {
        let suiteName = "com.davidsolheim.futuraterm.preferences-tests.\(UUID().uuidString)"
