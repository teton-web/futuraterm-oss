...
struct HotkeysTests {
    ...
    @Test
    func closeTab_defaults_to_cmd_shift_t() {
        // Distinct from Cmd+W Close Pane: this closes the whole tab. Busy
        // confirmation and pinned-tab unload stay on requestCloseTab.
        #expect(HotkeyAction.closeTab.defaultShortcut == "cmd+shift+t")
        #expect(HotkeyAction.closeTab.title == "Close Tab")
        #expect(AppCommand.closeTab.hotkeyAction == .closeTab)
        #expect(HotkeyAction.closePane.defaultShortcut == "cmd+w")
        #expect(HotkeyAction.newTab.defaultShortcut == "cmd+t")
    }
    ...
    @Test
    func defaultsKeys_use_the_futuraterm_prefix() {
        let action = HotkeyAction.splitRight
        #expect(action.defaultsKey == "futuraterm.hotkey.split_right")
        #expect(action.legacyDefaultsKey == "macterm.hotkey.split_right")
        #expect(action.passthroughDefaultsKey == "futuraterm.hotkey.split_right.passthrough")
        #expect(action.legacyPassthroughDefaultsKey == "macterm.hotkey.split_right.passthrough")
        for action in HotkeyAction.allCases {
            #expect(action.defaultsKey.hasPrefix("futuraterm.hotkey."))
            #expect(action.legacyDefaultsKey.hasPrefix("macterm.hotkey."))
        }
    }

    @Test
    func selectedShortcutString_migrates_legacy_macterm_hotkey_defaults() {
        let action = HotkeyAction.renameProject
        let priorNew = Preferences.defaults.string(forKey: action.defaultsKey)
        let priorLegacy = Preferences.defaults.string(forKey: action.legacyDefaultsKey)
        defer {
            if let priorLegacy {
                Preferences.defaults.set(priorLegacy, forKey: action.legacyDefaultsKey)
            } else {
                Preferences.defaults.removeObject(forKey: action.legacyDefaultsKey)
            }
            HotkeyRegistry.setShortcutString(priorNew ?? action.defaultShortcut, for: action)
        }

        Preferences.defaults.removeObject(forKey: action.defaultsKey)
        Preferences.defaults.set("cmd+e", forKey: action.legacyDefaultsKey)
        #expect(HotkeyRegistry.selectedShortcutString(for: action) == "cmd+e")
        #expect(Preferences.defaults.string(forKey: action.defaultsKey) == "cmd+e")

        Preferences.defaults.set("cmd+f", forKey: action.defaultsKey)
        Preferences.defaults.set("cmd+e", forKey: action.legacyDefaultsKey)
        #expect(HotkeyRegistry.selectedShortcutString(for: action) == "cmd+f")
    }

    @Test
    func passthrough_migrates_legacy_macterm_hotkey_defaults() {
        let action = HotkeyAction.focusPaneLeft
        let priorEnabled = Preferences.defaults.object(forKey: action.passthroughDefaultsKey) != nil
            && Preferences.defaults.bool(forKey: action.passthroughDefaultsKey)
        let priorLegacy = Preferences.defaults.object(forKey: action.legacyPassthroughDefaultsKey)
        defer {
            if let priorLegacy {
                Preferences.defaults.set(priorLegacy, forKey: action.legacyPassthroughDefaultsKey)
            } else {
                Preferences.defaults.removeObject(forKey: action.legacyPassthroughDefaultsKey)
            }
            HotkeyRegistry.setPassesThroughToPrograms(priorEnabled, for: action)
        }

        HotkeyRegistry.setPassesThroughToPrograms(false, for: action)
        Preferences.defaults.removeObject(forKey: action.passthroughDefaultsKey)
        Preferences.defaults.set(true, forKey: action.legacyPassthroughDefaultsKey)
        #expect(HotkeyRegistry.passesThroughToPrograms(for: action))
        #expect(Preferences.defaults.bool(forKey: action.passthroughDefaultsKey))
        #expect(HotkeyRegistry.passthroughActions().contains(action))
    }
