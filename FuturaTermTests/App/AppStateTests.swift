...
struct AppStateTests {
    ...
    @Test
    func viewDesktop_unpaired_does_not_open_moonlight_when_host_is_not_ready() async {
        let state = makeAppState()
        let remote = Project(name: "box", path: "box.tailnet.ts.net:~", sortOrder: 0)
        var launched = 0
        var pairStarts = 0
    ...
    @Test
    func viewDesktop_unpaired_does_not_stream_when_pin_submit_fails() async {
        let state = makeAppState()
        let remote = Project(name: "box", path: "box.tailnet.ts.net:~", sortOrder: 0)
        var launched = 0
        let result = await state.viewDesktop(
            ...
            pairing: .init(
                backgroundSSH: true,
                isPaired: { _ in false },
                startPair: { _, _ in true },
                submitPin: { _, _, _ in false }
            )
        )
        #expect(result == .missing(DesktopView.moonlightLaunchFailedReason))
        #expect(launched == 0)
