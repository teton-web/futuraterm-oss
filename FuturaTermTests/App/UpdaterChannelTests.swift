...
struct UpdaterChannelTests {
    ...
    func release_workflow_packages_linux_and_attaches_to_public_latest_without_dropping_the_dmg() throws {
        ...
        #expect(yml.contains("gh release upload \"$TAG\" \"$VERSIONED_LINUX\" \"$ALIAS_LINUX\" --repo davidsolheim/futuraterm --clobber"))
        #expect(yml.contains("gh release upload \"$TAG\" \"dmgs/${FILENAME}\" --repo davidsolheim/futuraterm --clobber"))
