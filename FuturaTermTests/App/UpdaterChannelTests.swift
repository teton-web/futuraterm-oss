...
struct UpdaterChannelTests {
    ...
    func appcast_publisher_runs_from_the_private_repo_and_writes_public_urls() throws {
        ...
        #expect(script.contains("teton-web/futuraterm-oss"))
        ...
        #expect(script.contains("https://github.com/${PUBLIC_REPO}/releases/download"))
    ...
    func release_workflow_publishes_the_appcast_and_public_dmg() throws {
        ...
        #expect(yml.contains("--repo teton-web/futuraterm-oss"))
    ...
    /// Public `releases/latest` is one document: a Linux-only attach hides the
    /// DMG and a Mac-only attach hides Omarchy. The Release job packages the
    /// ELF and uploads the versioned tarball plus the installer alias next to
    /// the DMG, never `delete-asset`.
    @Test
    func release_workflow_packages_linux_and_attaches_to_public_latest_without_dropping_the_dmg() throws {
        let yml = try String(
            contentsOf: repoRoot().appendingPathComponent(".github/workflows/release.yml"),
            encoding: .utf8
        )
        let packager = try String(
            contentsOf: repoRoot().appendingPathComponent("scripts/package-linux.sh"),
            encoding: .utf8
        )
        #expect(yml.contains("needs: [flag, build, linux]"))
        #expect(yml.contains("./scripts/package-linux.sh"))
        #expect(yml.contains("FuturaTerm-${VERSION}-linux-x86_64.tar.gz"))
        #expect(yml.contains("FuturaTerm-linux-x86_64.tar.gz"))
        #expect(yml.contains("gh release upload \"$TAG\" \"$VERSIONED_LINUX\" \"$ALIAS_LINUX\" --repo teton-web/futuraterm-oss --clobber"))
        #expect(yml.contains("gh release upload \"$TAG\" \"dmgs/${FILENAME}\" --repo teton-web/futuraterm-oss --clobber"))
        #expect(!yml.contains("gh release delete-asset"))
        #expect(!yml.contains("gh release delete "))
        #expect(packager.contains("FuturaTerm-linux-${ARCH}.tar.gz"))
        #expect(packager.contains("package-linux.sh must run on Linux"))
    }
