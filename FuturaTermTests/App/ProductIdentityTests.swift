...
struct ProductIdentityTests {
    ...
    @Test
    func source_info_plist_declares_non_exempt_encryption_false() throws {
        let url = try #require(Self.repoFile("FuturaTerm/Info.plist"))
        let data = try Data(contentsOf: url)
        let plist = try #require(PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
        #expect(plist["ITSAppUsesNonExemptEncryption"] as? Bool == false)
        #expect(plist["LSApplicationCategoryType"] as? String == "public.app-category.developer-tools")
        #expect(plist["NSAppleEventsUsageDescription"] as? String
            == "A process running in FuturaTerm's terminal would like to use AppleScript.")
        #expect(plist["NSCameraUsageDescription"] as? String
            == "A process running in FuturaTerm's terminal would like to use the camera.")
        #expect(plist["NSMicrophoneUsageDescription"] as? String
            == "A process running in FuturaTerm's terminal would like to use your microphone.")
    }

    @Test
    func privacy_manifest_declares_no_tracking_and_required_reason_apis() throws {
        let url = try #require(Self.repoFile("FuturaTerm/PrivacyInfo.xcprivacy"))
        let data = try Data(contentsOf: url)
        let plist = try #require(PropertyListSerialization.propertyList(from: data, format: nil) as? [String: Any])
        #expect(plist["NSPrivacyTracking"] as? Bool == false)
        let collected = try #require(plist["NSPrivacyCollectedDataTypes"] as? [Any])
        #expect(collected.isEmpty)
        let trackingDomains = try #require(plist["NSPrivacyTrackingDomains"] as? [Any])
        #expect(trackingDomains.isEmpty)
        let types = try #require(plist["NSPrivacyAccessedAPITypes"] as? [[String: Any]])
        let reasonsByType = Dictionary(uniqueKeysWithValues: types.compactMap { entry -> (String, [String])? in
    ...
    func readme_credits_macterm_without_origin_links() throws {
        ...
        #expect(readme.contains("https://github.com/teton-web/futuraterm-oss.git"))
        #expect(readme.contains("Teton Web Ventures LLC"))
    ...
    @Test
    func agents_and_contributing_name_the_llc_oss_snapshot() throws {
        let agents = try String(
            contentsOf: #require(Self.repoFile("AGENTS.md")),
            encoding: .utf8
        )
        let contributing = try String(
            contentsOf: #require(Self.repoFile("CONTRIBUTING.md")),
            encoding: .utf8
        )
        #expect(agents.contains("teton-web/futuraterm-oss"))
        #expect(agents.contains("Teton Web Ventures LLC"))
        #expect(agents.contains("owner/name"))
        #expect(!agents.contains("git push` this clone’s history to `davidsolheim/futuraterm`"))
        #expect(contributing.contains("https://github.com/teton-web/futuraterm-oss.git"))
        #expect(contributing.contains("https://github.com/teton-web/futuraterm-oss/issues"))
        let setup = try String(
            contentsOf: #require(Self.repoFile("scripts/setup.sh")),
            encoding: .utf8
        )
        #expect(setup.contains("FORK_REPO=\"teton-web/futuraterm-ghosttykit\""))
        #expect(setup.contains("ZMX_REPO=\"teton-web/futuraterm-zmx\""))
        #expect(!setup.contains("FORK_REPO=\"davidsolheim/futuraterm-ghosttykit\""))
        #expect(!setup.contains("ZMX_REPO=\"davidsolheim/futuraterm-zmx\""))
    }
