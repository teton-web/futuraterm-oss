...
int main(void) {
    ...
    expect(ft_desktop_vnc_argv(p.host, argv, 4) == 1, "vnc argv count");
    expect(strcmp(argv[0], "dts-0.tailnet.ts.net") == 0, "vnc argv host");
    expect(argv[1] == NULL, "vnc argv nil");
    expect(strstr(FT_DESKTOP_VNC_HOST_NOT_SHARING, "Screen Sharing") != NULL, "host not sharing");
    expect(strstr(FT_DESKTOP_VNC_HOST_NOT_SHARING, "ssh") == NULL, "host not sharing no ssh");
    ...
    {
        const char *json =
            "{\"Self\":{\"HostName\":\"dts-3\",\"DNSName\":\"dts-3.tailnet.ts.net.\","
            "\"OS\":\"linux\",\"TailscaleIPs\":[\"100.1.2.3\"]},"
            "\"Peer\":{\"nodekey:abc\":{\"HostName\":\"dts-0\","
            "\"DNSName\":\"dts-0.tailnet.ts.net.\",\"OS\":\"macOS\","
            "\"TailscaleIPs\":[\"100.9.8.7\"]}}}";
        char os[32];
        expect(ft_desktop_os_from_tailscale_json("dts-0", json, os, sizeof(os)) == 0, "peer hostname");
        expect(strcmp(os, "macOS") == 0, "peer is macOS");
        expect(
            ft_desktop_os_from_tailscale_json("dts-0.tailnet.ts.net", json, os, sizeof(os)) == 0,
            "peer dns"
        );
        expect(strcmp(os, "macOS") == 0, "dns is macOS");
        expect(ft_desktop_os_from_tailscale_json("100.9.8.7", json, os, sizeof(os)) == 0, "peer ip");
        expect(strcmp(os, "macOS") == 0, "ip is macOS");
        expect(ft_desktop_os_from_tailscale_json("dts-3", json, os, sizeof(os)) == 0, "self hostname");
        expect(strcmp(os, "linux") == 0, "self is linux");
        expect(ft_desktop_os_from_tailscale_json("other", json, os, sizeof(os)) != 0, "unknown host");
    }
