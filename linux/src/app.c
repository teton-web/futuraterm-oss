...
static void desktop_os_hint(const char *host, char *out, size_t n) {
    gchar *stdout_buf = NULL;
    gchar *argv[] = {(gchar *)"tailscale", (gchar *)"status", (gchar *)"--json", NULL};
    if (!out || n == 0) {
        return;
    }
    out[0] = 0;
    ...
    ft_desktop_os_from_tailscale_json(host, stdout_buf, out, n);
    g_free(stdout_buf);
}
...
static void view_desktop(void) {
    ...
    int vnc = have_on_path("vncviewer") || have_on_path("xtigervncviewer") || have_on_path("remmina")
        || have_on_path("xdg-open");
    char os_hint[64];
    FtDesktopPlan plan;
    os_hint[0] = 0;
    desktop_os_hint(parsed.host, os_hint, sizeof(os_hint));
    ft_desktop_plan(parsed.host, os_hint[0] ? os_hint : NULL, moon, vnc, &plan);
    ...
    if (plan.url[0]) {
        execlp("xdg-open", "xdg-open", plan.url, (char *)NULL);
    }
    {
        const char *vargs[2];
        if (ft_desktop_vnc_argv(plan.host, vargs, 2) == 1) {
            execlp("vncviewer", "vncviewer", vargs[0], (char *)NULL);
            execlp("xtigervncviewer", "xtigervncviewer", vargs[0], (char *)NULL);
            execlp("remmina", "remmina", vargs[0], (char *)NULL);
        }
    }
    _exit(127);
}
