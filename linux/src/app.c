...
static int run_gui(void) {
    ...
    gtk_window_set_title(GTK_WINDOW(app.window), "FuturaTerm");
    ...
    gtk_window_set_child(GTK_WINDOW(app.window), outer);
    ...
    gtk_window_set_titlebar(GTK_WINDOW(app.window), bar);
    ...
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(side_scroll), app.sidebar);
    g_signal_connect(app.sidebar, "row-activated", G_CALLBACK(on_project_row), NULL);
    g_signal_connect(app.sidebar, "row-selected", G_CALLBACK(on_project_row), NULL);
    gtk_box_append(GTK_BOX(body), side_scroll);

    app.stack = gtk_stack_new();
    gtk_widget_set_hexpand(app.stack, TRUE);
    gtk_widget_set_vexpand(app.stack, TRUE);
    gtk_box_append(GTK_BOX(body), app.stack);

    load_projects();
    if (app.projects->len) {
        select_project(g_ptr_array_index(app.projects, 0));
        gtk_list_box_select_row(GTK_LIST_BOX(app.sidebar), gtk_list_box_get_row_at_index(GTK_LIST_BOX(app.sidebar), 0));
    }

    add_shortcut(app.window, GDK_KEY_t, GDK_CONTROL_MASK, G_CALLBACK(shortcut_new_tab));
    add_shortcut(app.window, GDK_KEY_d, GDK_CONTROL_MASK, G_CALLBACK(shortcut_split_h));
    add_shortcut(app.window, GDK_KEY_d, GDK_CONTROL_MASK | GDK_SHIFT_MASK, G_CALLBACK(shortcut_split_v));
...
int main(int argc, char **argv) {
    g_set_prgname("futuraterm-linux");
