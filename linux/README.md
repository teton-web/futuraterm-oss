...
on the Mac). The installer (and each upgrade) sets Sunshine admin credentials
so nobody visits a login page. First View Desktop auto-pairs over the existing
remote SSH. From this Linux app, View Desktop on a Mac remote uses Screen
Sharing (`vnc://`). Super+Space then finds **FuturaTerm**.
