...
ensure_desktop_host() {
  if command -v omarchy-install-service-sunshine >/dev/null 2>&1; then
    ...
    omarchy-install-service-sunshine || true
  elif command -v pacman >/dev/null 2>&1 && pacman -Si sunshine >/dev/null 2>&1; then
...
enable_sunshine_user_unit() {
  ...
  # Real unit first. Alias sunshine.service can fail to enable (#7050).
...
has_sunshine_username() {
  f="$HOME/.config/sunshine/sunshine_state.json"
  [ -f "$f" ] || return 1
  grep -q '"username"[[:space:]]*:[[:space:]]*"[^"]' "$f"
}
...
configure_sunshine_first_run() {
  ...
  sunshine --creds "$user" "$pass" >/dev/null 2>&1 || true
  old_umask="$(umask)"
  umask 077
