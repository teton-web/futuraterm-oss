...
struct ControlClient {
    ...
    func send(command: String, args: ControlArgs? = nil) throws -> ControlResponse {
        ...
        let receiveSeconds = ControlProtocol.receiveTimeoutSeconds(timeoutMs: args?.timeoutMs)

        var attempts: [String] = []
        for path in candidatePaths() {
            switch tryPath(path, payload: payload, receiveTimeoutSeconds: receiveSeconds) {
        ...
        let hint = socketOverride == nil
            ? "\nIs FuturaTerm running? (launch it, or pass --socket for a non-default location)"
            : ""
        throw ClientError(
            description: "could not reach FuturaTerm's control socket:\n" + attempts.joined(separator: "\n") + hint,
            isConnectionFailure: true
        )
    }

    /// Discovery order: explicit `--socket` is a hard pin (tried alone);
    /// otherwise `FUTURATERM_SOCKET` env
    /// hint — but only as a *hint*: a stale exported path (app restarted
    /// since this shell spawned) falls through to the well-known per-flavor
    /// locations instead of bricking the CLI in that shell (cmux's documented
    /// sleep/wake trap).
    func candidatePaths(environment: [String: String] = ProcessInfo.processInfo.environment) -> [String] {
        let appSupport = FileManager.default
            .urls(for: .applicationSupportDirectory, in: .userDomainMask)
            .first ?? URL(fileURLWithPath: NSHomeDirectory() + "/Library/Application Support")
        return ControlProtocol.socketCandidatePaths(
            override: socketOverride,
    ...
    private func tryPath(_ path: String, payload: Data, receiveTimeoutSeconds: Int) -> Attempt {
        ...
        var timeout = timeval(tv_sec: time_t(receiveTimeoutSeconds), tv_usec: 0)
