...
enum Output {
    ...
    static func render(_ data: ControlData?, asJSON: Bool) throws {
        ...
        if let selection = data.selection { renderSelection(selection) }
        if let choices = data.choices { renderChoices(choices) }
    }

    private static func renderStatus(_ status: ControlStatusInfo) {
        var line = "FuturaTerm \(status.version) (pid \(status.pid))"
    ...
    /// `N  label  line:L col:C  *` — selected rows get a trailing `*`.
    private static func renderChoices(_ choices: [ControlPaneChoice]) {
    ...
    /// Selected text only — same pipeline-friendly contract as `pane dump`.
    private static func renderSelection(_ s: ControlPaneSelection) {
        if s.text.isEmpty {
            return
        }
        print(s.text, terminator: s.text.hasSuffix("\n") ? "" : "\n")
    }
    ...
    static func printError(_ message: String, action: String? = nil) {
        FileHandle.standardError.write(Data("futuraterm: \(message)\n".utf8))
        if let action {
