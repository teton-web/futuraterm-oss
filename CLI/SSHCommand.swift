...
/// `futuraterm ssh` — ghostty's `+ssh` action reimplemented natively (see
/// `SSHWrapper` for the full story). The one offline verb: it never touches
/// the control socket and works with no FuturaTerm running.
///
...
/// by hand or alias (`alias ssh='futuraterm ssh --'`).
///
...
struct SSHCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
        Installs FuturaTerm's bundled xterm-ghostty terminfo on the destination \
        ...
        real ssh. ssh's own flags and the destination pass through verbatim:
          ...
          futuraterm ssh user@example.com
          futuraterm ssh -p 2222 -i ~/.ssh/id_ed25519 user@example.com
          futuraterm ssh --terminfo=false user@example.com
    ...
    private func installTerminfo(source: Data) -> Bool {
        let controlPath = NSTemporaryDirectory()
            + "futuraterm-ssh-" + String(format: "%08x", UInt32.random(in: .min ... .max))
