...
The private working clone (`teton-web/futuraterm`) holds the Swift app at the root and [futuraterm.com](https://futuraterm.com) in `website/`. Vercel **Root Directory** must be `website/` (console setting; this README does not change Vercel). The public snapshot (`davidsolheim/futuraterm`) is history-free MIT source only: `scripts/publish-public-snapshot.sh` strips `website/` plus notarizing workflows. Do not `git push` private history to the public remote, and do not run Apple signing on the public Actions account.

| Repo | Visibility | Role |
| --- | --- | --- |
| [davidsolheim/futuraterm](https://github.com/davidsolheim/futuraterm) | public | OSS snapshot: MIT source, tests, `mise run build`. |
