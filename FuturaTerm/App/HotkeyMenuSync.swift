...
/// Every `AppCommand` menu item also registers its `help` string so `toolTip`
/// is pinned even when SwiftUI `.help()` is dropped on the live menu — including
/// Unload/Remove, which have no `HotkeyAction`.
@MainActor
enum HotkeyMenuSync {
    ...
    /// Rendered menu title → help tag. Includes nil-hotkey commands
    /// (Unload/Remove) so they still get `toolTip`.
