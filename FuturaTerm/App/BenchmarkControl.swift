...
///
/// When the app is launched with `FUTURATERM_BENCHMARK=1` it listens for Darwin
...
enum BenchmarkControl {
    static let isEnabled =
        ProcessInfo.processInfo.environment["FUTURATERM_BENCHMARK"] == "1"
    ...
    private static let notificationPrefix = "com.davidsolheim.futuraterm.bench."
    ...
    static func install() {
        ...
        logger.info("benchmark control listening (FUTURATERM_BENCHMARK=1)")
    }
    ...
    /// Select (creating if needed) a project at `FUTURATERM_BENCHMARK_DIR` (or
    /// `$HOME`) so the window hosts a
    /// live terminal pane. A fresh CI runner otherwise sits on the WelcomeView,
    /// and the benchmark would measure an app with no surface, no shell, and
    /// no foreground-process poll.
    private static func openProject() {
        ...
        let path = env["FUTURATERM_BENCHMARK_DIR"] ?? NSHomeDirectory()
