...
struct FuturaTermApp: App {
    ...
    @State
    private var grokSessions = GrokSessionStore()
    @State
    private var githubAvatar = GitHubAvatarStore()
    ...
    var body: some Scene {
        WindowGroup {
            MainWindow()
                ...
                .environment(agentUsage)
                .environment(grokSessions)
                .environment(githubAvatar)
                ...
                .alert(
                    "Couldn't View Desktop",
                    isPresented: Binding(
                        get: { appState.pendingViewDesktopError != nil },
                        set: {
                            if !$0 {
                                appState.pendingViewDesktopError = nil
                            }
                        }
                    )
                ) {
                    ...
                    if let pending = appState.pendingViewDesktopError {
                        Text(pending)
                    }
                }
                .onAppear {
                ...
                // AppDelegate is the source of truth for `futuraterm://`.
                // SwiftUI can also deliver the same URL here; the router
                // dedupes so we do not `project.open` twice.
                .onOpenURL { url in
                    appDelegate.openURLs([url])
                }
        }
        ...
        .commands {
            CommandGroup(replacing: .newItem) {
                ...
                // unhides the single FuturaTerm window after the user clicked
            ...
            CommandGroup(replacing: .sidebar) {
                ...
                AppCommandMenuItem(
                    command: .manageSessions,
                    appState: appState,
                    projectStore: projectStore,
                    titleOverride: "Sessions"
                )
                AppCommandMenuItem(
                    command: .manageEnvironments,
                    appState: appState,
                    projectStore: projectStore,
                    titleOverride: "Environments"
                )
                AppCommandMenuItem(
            ...
            CommandMenu("Project") {
                ...
                AppCommandMenuItem(command: .viewDesktop, appState: appState, projectStore: projectStore)
                AppCommandMenuItem(
...
/// dependency, so `.preferredColorScheme` — and every `FuturaTermTheme` color read
/// downstream — re-evaluates when the resolved theme changes. Without this the
/// chrome would freeze at its launch appearance, since `FuturaTermTheme.colorScheme`
...
private struct AppColorScheme: ViewModifier {
    func body(content: Content) -> some View {
        ...
        // `FuturaTermTheme.colorScheme`).
        _ = GhosttyApp.shared.configVersion
        return content.preferredColorScheme(FuturaTermTheme.colorScheme)
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// LaunchServices `futuraterm://` queue. AppDelegate is the source of
    /// truth; SwiftUI `.onOpenURL` forwards here so a double delivery is
    /// one `project.open`, not two.
    let urlRouter = FuturaTermURLRouter()
    ...
    func applicationDidFinishLaunching(_: Notification) {
        ...
        // FUTURATERM_SOCKET, PATH, and the zmx-session scrub moved to
        // EnvironmentSetup (run from FuturaTermApp.init): environ must not be
    ...
    /// Called from FuturaTermApp.onAppear once the state objects exist. Registers
    ...
    /// Shared with SwiftUI `.onOpenURL`. URLs arriving before
    /// `installResponders` stay queued on `urlRouter` until attach.
