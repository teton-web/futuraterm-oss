...
final class AppState {
    var activeProjectID: UUID? {
        didSet {
            ...
            if let activeProjectID {
                unloadedProjectIDs.remove(activeProjectID)
                beginFolderGrantForActiveProject()
            }
        }
    }
    ...
    private let recencyKey = "futuraterm.projectRecency"
    /// Legacy key; copied into `recencyKey` on first restore.
    private let legacyRecencyKey = "macterm.projectRecency"

    /// Which window presents a dialog `AppState` stages.
    ///
    /// Both scenes in `FuturaTermApp` — the main `WindowGroup` and `Settings` —
    ...
    /// `FuturaTermApp`). Fed by the explicit palette/menu commands and by the
    ...
    /// Missing Moonlight / VNC viewer after View Desktop. Main window only.
    var pendingViewDesktopError: String?
    ...
    /// the changes FuturaTerm makes, which the user does expect to see land.
    ...
    /// Sessions sheet (POR-444) — native list of local zmx sessions.
    var isSessionManagerPresented = false
    var sessionInventoryRows: [SessionInventory.Row] = []
    var isSessionInventoryLoading = false
    var sessionInventoryUnavailable = false

    /// Environments sheet (POR-556) — local TCP LISTEN development servers.
    var isEnvironmentManagerPresented = false
    var localListenerRows: [LocalListenerInventory.Row] = []
    var isLocalListenerLoading = false
    var localListenerUnavailable = false
    ...
    /// Injectable so tests can exercise the MAS "don't spawn until grant" gate
    /// on a Direct/Debug host (`usesSecurityScope` is false there).
    @ObservationIgnored
    var folderGrantUsesSecurityScope: Bool = SecurityScopedBookmark.usesSecurityScope
    /// Test seam for Replace Project Path: production presents NSOpenPanel.
    @ObservationIgnored
    var pickFolderForPathReplacement: ((String) -> URL?)?
    /// Bumped when a folder grant is held so SwiftUI retries `ensureNSView`
    /// / `createSurface` (holding lives on an unobserved access object).
    var folderGrantEpoch = 0
    /// Project needing a recoverable Open-panel re-grant after a stale/missing bookmark.
    var pendingSecurityScopeRegrantID: UUID?
    /// Additional projects waiting after the in-flight re-grant panel.
    @ObservationIgnored
    private var securityScopeRegrantQueue: [UUID] = []
    /// Last-seen folder bookmarks, so focus paths that only have a project
    /// UUID (`moveTab`, `navigateToPane`) can still start a MAS grant.
    @ObservationIgnored
    private var knownFolderBookmarks: [UUID: Data] = [:]
    @ObservationIgnored
    private var knownRemoteProjectIDs: Set<UUID> = []
    ...
    /// Sidebar projects last seen by restore/select. Adopt matches against
    /// the full list; `selectProject` only receives the destination.
    @ObservationIgnored
    private var knownProjectsForAdoption: [Project] = []
    ...
    /// Central project-file store (`~/.config/futuraterm/projects`). Injectable
    ...
    init(
        ...
        if Preferences.defaults.stringArray(forKey: recencyKey) == nil,
           let legacy = Preferences.defaults.stringArray(forKey: legacyRecencyKey)
        {
            Preferences.defaults.set(legacy, forKey: recencyKey)
        }
    ...
    private func reschedulePoll() {
        ...
        if let existing = pollTimer, existing.isValid, pollTimerDelay == delay {
            return
        }
        pollTimer?.invalidate()
    ...
    func restoreSelection(projects: [Project]) {
        ...
        rememberFolderBookmarks(from: projects)
        rememberProjectsForAdoption(projects)
        // Origin (and persisted covering) grants don't need live tabs.
        // The covering loop that matches pane cwds to ProjectStore runs
        // again after `materializeRestoredPinnedTabs` builds those panes.
        beginSecurityScopedAccessForPinnedOrigins(projects: projects)
        if let id = Preferences.shared.activeProjectID {
            if id == PinnedTabs.projectID {
                ...
                beginSecurityScopedAccess(for: project)
                acknowledgeActiveTab(projectID: id)
                warmFocusedProject()
            }
        }
        // Sweep crash/force-quit orphans: kill zero-client futuraterm-* sessions
        ...
        // Adopt unclaimed grok sessions in that same Task, after pinned
        // claims exist and before the local reap, so we don't kill the
        // chats we're about to reattach (POR-372).
        Task {
            await materializePinnedAdoptGrokAndReap(projects: projects)
        }
        guard !workspaceStore.loadFailed else { return }
        sweptDestinations[Self.localSweepKey] = Date()
        // Local reap runs in the Task above (after adopt). Mark the slot
        // swept so this call stays remote-only and cannot race adopt.
    ...
    /// Interactive Open / New / Settings add: reuse the first sidebar project
    /// whose path matches (`ProjectStore.findOrCreate`), else create one, then
    /// select it. Does not rewrite an existing project's name or `zmxPath`.
    /// Explicit second-project creation stays on `ProjectStore.create` (CLI
    /// `project create`, palette "Add another project").
    @discardableResult
    func addOrSelectProject(
        store: ProjectStore,
        name: String,
        path: String,
        zmxPath: String? = nil,
    ...
    func selectProject(_ project: Project) {
        ...
        rememberFolderBookmark(from: project)
        activeProjectID = project.id
        recordProjectVisit(project.id)
        rememberProjectForAdoption(project)
        autoApplyLayoutOnFirstOpen(project)
        ...
        Task {
            await adoptUnclaimedGrokSessionsThenSweep(project: project)
        }
        warmFocusedProject()
    ...
    func warmFocusedProject() {
        ...
        let panes = Self.panesToWarm(in: ws).filter { shouldSpawnPane($0, workspaceID: projectID) }
        warmStaggered(panes)
    }

    func beginSecurityScopedAccess(for project: Project) {
        guard !project.isRemote else { return }
        rememberFolderBookmark(from: project)
        beginFolderGrant(id: project.id, bookmark: project.securityScopedBookmark)
    }

    /// MAS folder grants used to start only from `selectProject` / restore.
    /// `activeProjectID`'s didSet is the choke point for tab cycle, move,
    /// merge, layout apply, and navigate — begin (or queue Grant Access) there.
    private func beginFolderGrantForActiveProject() {
        guard let id = activeProjectID else { return }
        if id == PinnedTabs.projectID {
            beginSecurityScopedAccessForPinnedOrigins(projects: knownProjectsForAdoption)
            return
        }
        if knownRemoteProjectIDs.contains(id) { return }
        if let panes = workspaces[id]?.tabs.flatMap({ $0.splitRoot.allPanes() }),
           !panes.isEmpty,
           panes.allSatisfy(\.isRemote)
        {
            return
        }
        beginFolderGrant(id: id, bookmark: knownFolderBookmarks[id])
    }

    private func rememberFolderBookmark(from project: Project) {
        if project.isRemote {
            knownRemoteProjectIDs.insert(project.id)
            return
        }
        knownRemoteProjectIDs.remove(project.id)
        if let data = project.securityScopedBookmark {
            knownFolderBookmarks[project.id] = data
        }
    }

    private func rememberFolderBookmarks(from projects: [Project]) {
        for project in projects {
            rememberFolderBookmark(from: project)
        }
    }

    /// Pinned panes spawn from origin-project folders. Start those grants
    /// (and queue a re-grant when missing/stale) before `warmPinnedTabs`.
    /// An origin that has left ProjectStore still grants against the record's
    /// stored folder/bookmark (or queues Open panel using the pane cwd).
    /// Origin-less records (Cmd+T / hand-added file entries) grant against
    /// the record id + pane cwd — a missing origin is not treated as granted.
    /// Extra covering grants (merged B, dest home leftover, pin of mixed
    /// cwds, split/grid outside the origin) start from the record so Grant
    /// Access still has a target when the project list is empty or the origin
    /// row is gone. Uncovered live pane cwds persist a covering grant and
    /// start or queue it — call again after tabs materialize.
    func beginSecurityScopedAccessForPinnedOrigins(projects: [Project]) {
        var seen: Set<UUID> = []
        for record in pinnedRecords {
            if let originID = record.originProjectID {
                if seen.insert(originID).inserted {
                    if let project = projects.first(where: { $0.id == originID }) {
                        beginSecurityScopedAccess(for: project)
                    } else if let data = knownFolderBookmarks[originID] {
                        beginFolderGrant(id: originID, bookmark: data)
                    } else if pinnedOriginNeedsLocalFolderGrant(originID) {
                        beginFolderGrant(id: originID, bookmark: pinnedOriginBookmark(originID))
                    }
                }
            } else if pinnedRecordNeedsLocalFolderGrant(record) {
                beginFolderGrant(id: record.id, bookmark: record.originFolderBookmark)
            }
            beginCoveringGrants(for: record)
        }
        guard let ws = pinnedWorkspace else { return }
        for tab in ws.tabs {
            persistCoveringGrantsForUncoveredLocalPanes(tabID: tab.id, extraProjects: projects)
        }
    }

    func originFolderBookmark(for projectID: UUID) -> Data? {
        knownFolderBookmarks[projectID]
    }

    /// Local folder of a remembered project — the path that belongs with
    /// `originFolderBookmark(for:)`, not a pane cwd (tree order is not identity).
    func originFolderPath(for projectID: UUID) -> String? {
        knownProjectsForAdoption.first { $0.id == projectID && !$0.isRemote }?.path
    }

    /// Project whose folder covers `path`, preferring the longest match.
    /// Pin/split/merge persist that row's id + bookmark as a covering grant.
    func coveringFolderSource(
        forLocalPath path: String,
        extraProjects: [Project] = []
    ) -> PinnedCoveringGrant? {
        var pool = knownProjectsForAdoption
        for project in extraProjects where !pool.contains(where: { $0.id == project.id }) {
            pool.append(project)
        }
        let match = pool
            .filter { !$0.isRemote && SecurityScopedBookmark.folder($0.path, covers: path) }
            .max { $0.path.count < $1.path.count }
        guard let match else { return nil }
        return PinnedCoveringGrant(
            id: match.id,
            folderPath: match.path,
            folderBookmark: match.securityScopedBookmark ?? knownFolderBookmarks[match.id]
        )
    }

    func beginOriginlessPinnedFolderGrant(_ record: PinnedTabRecord) {
        guard folderGrantUsesSecurityScope, pinnedRecordNeedsLocalFolderGrant(record) else { return }
        beginFolderGrant(id: record.id, bookmark: record.originFolderBookmark)
    }

    func beginGrantForPinnedRecord(_ record: PinnedTabRecord) {
        if let originID = record.originProjectID {
            beginFolderGrant(
                id: originID,
                bookmark: record.originFolderBookmark ?? knownFolderBookmarks[originID]
            )
        } else {
            beginOriginlessPinnedFolderGrant(record)
        }
        beginCoveringGrants(for: record)
    }

    private func beginCoveringGrants(for record: PinnedTabRecord) {
        for grant in record.coveringGrants {
            if let data = grant.folderBookmark {
                knownFolderBookmarks[grant.id] = data
            }
            beginFolderGrant(
                id: grant.id,
                bookmark: grant.folderBookmark ?? knownFolderBookmarks[grant.id]
            )
        }
    }

    /// Cross-folder merge into a pinned tab that already originated from A:
    /// start B's grant without replacing A's origin (unpin stays A). An
    /// origin-less incoming leftover uses `coveringGrantID` (the source record).
    func startAdditionalPinnedOriginGrant(_ origin: PinnedOriginStamp) {
        guard let id = origin.originProjectID ?? origin.coveringGrantID else { return }
        if let data = origin.originFolderBookmark {
            knownFolderBookmarks[id] = data
        }
        beginFolderGrant(
            id: id,
            bookmark: origin.originFolderBookmark ?? knownFolderBookmarks[id]
        )
    }

    /// Start (or keep) a folder grant. `begin(nil)` would drop a live
    /// extension, so a missing bookmark while already holding stays held
    /// and still queues a re-grant to persist one.
    private func beginFolderGrant(id: UUID, bookmark: Data?) {
        if bookmark == nil, securityScopedAccess.isHolding(id) {
            if folderGrantUsesSecurityScope {
                requestSecurityScopeRegrant(for: id)
            }
            return
        }
        let wasHolding = securityScopedAccess.isHolding(id)
        let result = securityScopedAccess.begin(projectID: id, bookmark: bookmark)
        if SecurityScopedBookmark.needsUserGrant(result, usesSecurityScope: folderGrantUsesSecurityScope) {
            requestSecurityScopeRegrant(for: id)
        }
        // `SecurityScopedAccess` is unobserved; SwiftUI retries ensureNSView
        // only when this epoch changes. Bump on a newly held grant, not only
        // after the Open-panel path (`applySuccessfulFolderGrant`).
        if folderGrantUsesSecurityScope, !wasHolding, securityScopedAccess.isHolding(id) {
            folderGrantEpoch += 1
        }
    }

    func isHoldingSecurityScopedAccess(for projectID: UUID) -> Bool {
        securityScopedAccess.isHolding(projectID)
    }

    func requestSecurityScopeRegrant(for projectID: UUID) {
        if pendingSecurityScopeRegrantID == projectID { return }
        if pendingSecurityScopeRegrantID == nil {
            pendingSecurityScopeRegrantID = projectID
            return
        }
        if !securityScopeRegrantQueue.contains(projectID) {
            securityScopeRegrantQueue.append(projectID)
        }
    }

    private func dequeueSecurityScopeRegrant() {
        guard pendingSecurityScopeRegrantID == nil else { return }
        while let next = securityScopeRegrantQueue.first {
            securityScopeRegrantQueue.removeFirst()
            pendingSecurityScopeRegrantID = next
            return
        }
    }

    /// Recoverable re-grant after a stale/missing bookmark — never crash.
    func presentSecurityScopeRegrant(store: ProjectStore, projectID: UUID) {
        pendingSecurityScopeRegrantID = nil
        defer { dequeueSecurityScopeRegrant() }
        guard let target = folderGrantTarget(store: store, projectID: projectID) else { return }
        let panel = NSOpenPanel()
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.allowsMultipleSelection = false
        panel.directoryURL = URL(fileURLWithPath: target.path)
        panel.message = "FuturaTerm needs access to “\(target.name)” again."
        let picked = panel.runModal() == .OK ? panel.url : nil
        completeSecurityScopeRegrant(store: store, projectID: projectID, pickedURL: picked)
    }

    enum SecurityScopeRegrantOutcome: Equatable {
        case dismissed
        case wrongFolder
    ...
    /// Apply an Open-panel pick (or cancel). Wrong-folder re-queues; cancel
    /// does not. A successful grant rebuilds any surfaces that already spawned
    /// without the folder, then warms what was deferred.
    @discardableResult
    func completeSecurityScopeRegrant(
        store: ProjectStore,
        projectID: UUID,
        pickedURL: URL?
    ) -> SecurityScopeRegrantOutcome {
        guard let pickedURL else { return .dismissed }
        guard let target = folderGrantTarget(store: store, projectID: projectID) else {
            return .dismissed
        }
        guard SecurityScopedBookmark.folder(pickedURL.path, covers: target.path) else {
            presentToast(
                "Wrong folder",
                subtitle: "Choose “\(target.name)” or a folder that contains it."
            )
            requestSecurityScopeRegrant(for: projectID)
            return .wrongFolder
        }
        guard let data = SecurityScopedBookmark.create(from: pickedURL) else {
            presentToast(
                "Couldn’t save access",
                subtitle: "Pick the folder again to grant FuturaTerm access."
            )
            requestSecurityScopeRegrant(for: projectID)
            return .bookmarkCreateFailed
        }
        persistFolderGrantBookmark(store: store, projectID: projectID, bookmark: data, path: target.path)
        beginFolderGrant(id: projectID, bookmark: data)
        guard isHoldingSecurityScopedAccess(for: projectID) else {
            requestSecurityScopeRegrant(for: projectID)
            return .bookmarkCreateFailed
        }
        applySuccessfulFolderGrant(projectID: projectID)
        return .granted
    }

    /// After a folder grant: kill local shells that started under a denied
    /// cwd and first-spawn again (not remote `reconnectSurface`, which
    /// reattaches that session). Then warm panes that were waiting.
    func applySuccessfulFolderGrant(projectID: UUID) {
        let panes = localPanesCoveredByFolderGrant(projectID)
        let spawned = panes.filter { $0.nsView != nil }
        let names = spawned.map(\.sessionName)
        for pane in spawned {
            pane.destroySurface()
            pane.resetSurfaceForFirstSpawn()
        }
        if !names.isEmpty {
            zmx.killSessionsBlocking(names)
        }
        folderGrantEpoch += 1
        for pane in panes {
    ...
    private func localPanesCoveredByFolderGrant(_ projectID: UUID) -> [Pane] {
        var panes: [Pane] = []
        let folder = folderPathForGrant(projectID)
        if let ws = workspaces[projectID], projectID != PinnedTabs.projectID {
            for pane in ws.tabs.flatMap({ $0.splitRoot.allPanes() }) where !pane.isRemote {
                if let folder, !SecurityScopedBookmark.folder(folder, covers: pane.projectPath) {
                    continue
                }
                panes.append(pane)
            }
        }
        guard let pinned = pinnedWorkspace, let folder else { return panes }
        for pane in pinned.tabs.flatMap({ $0.splitRoot.allPanes() }) where !pane.isRemote {
            guard SecurityScopedBookmark.folder(folder, covers: pane.projectPath) else { continue }
            guard !isCoveredByOtherHeldGrant(pane, besides: projectID) else { continue }
            if !panes.contains(where: { $0.id == pane.id }) {
                panes.append(pane)
            }
        }
        return panes
    ...
    private func folderPathForGrant(_ projectID: UUID) -> String? {
        if let path = securityScopedAccess.heldURL(for: projectID)?.path { return path }
        if let project = knownProjectsForAdoption.first(where: { $0.id == projectID }), !project.isRemote {
            return project.path
        }
        if let match = pinnedCoveringGrant(id: projectID) { return match.grant.folderPath }
        if let path = pinnedOriginFolderPath(projectID) { return path }
        return pinnedRecords.first { $0.id == projectID }?.originFolderPath
    }

    /// Extra-cwd panes already spawned under a different covering grant must
    /// not be first-spawn-killed when this grant's Open panel completes.
    private func isCoveredByOtherHeldGrant(_ pane: Pane, besides id: UUID) -> Bool {
        for other in pinnedFolderGrantIDs where other != id {
            guard let url = securityScopedAccess.heldURL(for: other) else { continue }
            if SecurityScopedBookmark.folder(url.path, covers: pane.projectPath) { return true }
        }
        return false
    }

    func shouldSpawnPane(_ pane: Pane, workspaceID: UUID) -> Bool {
        SecurityScopedBookmark.canSpawnLocalPane(
            isRemote: pane.isRemote,
            isHoldingGrant: isHoldingFolderGrant(for: pane, workspaceID: workspaceID),
            usesSecurityScope: folderGrantUsesSecurityScope
        )
    }

    private func isHoldingFolderGrant(for pane: Pane, workspaceID: UUID) -> Bool {
        if workspaceID == PinnedTabs.projectID {
            return isHoldingPinnedGrantCovering(pane.projectPath)
        }
        guard let id = folderGrantProjectID(for: pane, workspaceID: workspaceID) else {
            return false
        }
        guard securityScopedAccess.isHolding(id) else { return false }
        guard let url = securityScopedAccess.heldURL(for: id) else { return false }
        // Dest-project grant is not enough: a cross-project move/merge keeps
        // the source cwd while rebinding projectID.
        return SecurityScopedBookmark.folder(url.path, covers: pane.projectPath)
    }

    /// Pinned spawn keys on more than the dest origin id: a Cmd+T leftover
    /// home grant, or origin A after merging B, may be holding without
    /// covering this pane's cwd.
    private func isHoldingPinnedGrantCovering(_ path: String) -> Bool {
        for id in pinnedFolderGrantIDs {
            guard let url = securityScopedAccess.heldURL(for: id) else { continue }
            if SecurityScopedBookmark.folder(url.path, covers: path) {
                return true
            }
        }
        return false
    ...
    private var pinnedFolderGrantIDs: Set<UUID> {
        var ids = Set(knownFolderBookmarks.keys)
        for record in pinnedRecords {
            ids.insert(record.id)
            if let origin = record.originProjectID {
                ids.insert(origin)
            }
            for grant in record.coveringGrants {
                ids.insert(grant.id)
    ...
    private func folderGrantProjectID(for pane: Pane, workspaceID: UUID) -> UUID? {
        if workspaceID == PinnedTabs.projectID {
            guard let ws = pinnedWorkspace,
                  let tab = ws.tabs.first(where: { $0.splitRoot.findPane(id: pane.id) != nil })
            else { return nil }
            let record = pinnedRecord(tab.id)
            return record?.originProjectID ?? record?.id
        }
        return workspaceID
    }

    private struct FolderGrantTarget {
        let path: String
        let name: String
    }

    /// Project row if it still exists; otherwise the pinned origin's stored
    /// folder or a live/declared pane cwd so a missing ProjectStore row can
    /// still present Grant Access.
    private func folderGrantTarget(store: ProjectStore, projectID: UUID) -> FolderGrantTarget? {
        if let project = store.projects.first(where: { $0.id == projectID }), !project.isRemote {
            return FolderGrantTarget(path: project.path, name: project.name)
        }
        if let match = pinnedCoveringGrant(id: projectID) {
            return FolderGrantTarget(path: match.grant.folderPath, name: match.record.displayTitle)
        }
        if let record = pinnedRecords.first(where: { $0.id == projectID }),
           let path = pinnedRecordFolderPath(record)
        {
            return FolderGrantTarget(path: path, name: record.displayTitle)
        }
        guard pinnedRecords.contains(where: { $0.originProjectID == projectID }),
              pinnedOriginNeedsLocalFolderGrant(projectID),
              let path = pinnedOriginFolderPath(projectID)
        else { return nil }
        let name = pinnedRecords.first { $0.originProjectID == projectID }?.displayTitle ?? "Pinned Tab"
        return FolderGrantTarget(path: path, name: name)
    }

    private func pinnedCoveringGrant(id: UUID) -> (record: PinnedTabRecord, grant: PinnedCoveringGrant)? {
        for record in pinnedRecords {
            if let grant = record.coveringGrants.first(where: { $0.id == id }) {
                return (record, grant)
            }
        }
        return nil
    ...
    private func persistFolderGrantBookmark(
        store: ProjectStore,
        projectID: UUID,
        bookmark: Data,
        path: String
    ) {
        knownFolderBookmarks[projectID] = bookmark
        if store.projects.contains(where: { $0.id == projectID }) {
            store.setBookmark(id: projectID, bookmark: bookmark)
        }
        var changed = false
        for i in pinnedRecords.indices {
            if pinnedRecords[i].originProjectID == projectID
                || (pinnedRecords[i].id == projectID && pinnedRecords[i].originProjectID == nil)
            {
                pinnedRecords[i].originFolderBookmark = bookmark
                pinnedRecords[i].originFolderPath = path
                changed = true
            }
            for j in pinnedRecords[i].coveringGrants.indices
                where pinnedRecords[i].coveringGrants[j].id == projectID
            {
                pinnedRecords[i].coveringGrants[j].folderBookmark = bookmark
                pinnedRecords[i].coveringGrants[j].folderPath = path
                changed = true
            }
        }
    ...
    /// Copy the origin project's grant onto pinned records before the row
    /// leaves ProjectStore, so relaunch can re-start access / re-prompt.
    /// Replace-path that shrinks the origin (`/repo` → `/repo/src`) must
    /// persist leftover dest cwd covering grants (old path/bookmark, under
    /// `record.id` when that slot is free) and start them before the parent
    /// scope is dropped.
    func preservePinnedOriginGrant(from project: Project) {
        guard !project.isRemote else { return }
        let origin = PinnedOriginStamp(
            originProjectID: project.id,
            originFolderPath: project.path,
            originFolderBookmark: project.securityScopedBookmark
        )
        var changed = false
        for i in pinnedRecords.indices {
            let originMatch = pinnedRecords[i].originProjectID == project.id
            let coveringMatch = pinnedRecords[i].coveringGrants.contains { $0.id == project.id }
            guard originMatch || coveringMatch else { continue }
            if originMatch {
                persistUncoveredDestCoveringGrants(at: i, replacingWith: origin)
            }
            persistUncoveredCoveringGrants(at: i, replacingProject: project)
            // Start leftover dest grants while `project.id` still holds the
            // parent bookmark. `beginGrantForPinnedRecord` after the restamp
            // would drop that scope first.
            beginGrantForPinnedRecord(pinnedRecords[i])
            if originMatch {
                pinnedRecords[i].originFolderPath = project.path
                pinnedRecords[i].originFolderBookmark = project.securityScopedBookmark
            }
            if uncoveredCoveringLeftoversCopied(at: i, replacingProject: project) {
                for j in pinnedRecords[i].coveringGrants.indices
                    where pinnedRecords[i].coveringGrants[j].id == project.id
                {
                    pinnedRecords[i].coveringGrants[j].folderPath = project.path
                    pinnedRecords[i].coveringGrants[j].folderBookmark = project.securityScopedBookmark
                }
            }
            persistCoveringGrantsForUncoveredLocalPanes(tabID: pinnedRecords[i].id)
            changed = true
        }
        if changed { saveWorkspaces() }
    }

    private func pinnedRecordNeedsLocalFolderGrant(_ record: PinnedTabRecord) -> Bool {
        pinnedRecordFolderPath(record) != nil
    }

    private func pinnedRecordFolderPath(_ record: PinnedTabRecord) -> String? {
        if let path = record.originFolderPath, !ProjectPath.isRemote(path) {
            return path
        }
        if let tab = pinnedWorkspace?.tabs.first(where: { $0.id == record.id }),
           let path = tab.splitRoot.allPanes().first(where: { !$0.isRemote })?.projectPath
        {
            return path
        }
        return record.declaredLocalCwd
    }

    private func pinnedOriginNeedsLocalFolderGrant(_ originID: UUID) -> Bool {
        for record in pinnedRecords where record.originProjectID == originID {
            if let path = record.originFolderPath, !ProjectPath.isRemote(path) {
                return true
            }
            if let tab = pinnedWorkspace?.tabs.first(where: { $0.id == record.id }),
               tab.splitRoot.allPanes().contains(where: { !$0.isRemote })
            {
                return true
            }
            if record.declaredLocalCwd != nil { return true }
        }
        return false
    }

    private func pinnedOriginBookmark(_ originID: UUID) -> Data? {
        pinnedRecords.first { $0.originProjectID == originID }?.originFolderBookmark
    }

    private func pinnedOriginFolderPath(_ originID: UUID) -> String? {
        for record in pinnedRecords where record.originProjectID == originID {
            if let path = record.originFolderPath, !ProjectPath.isRemote(path) {
                return path
            }
            ...
            if let cwd = record.declaredLocalCwd { return cwd }
        }
        return nil
    }

    private func endFolderGrantUnlessPinned(_ projectID: UUID) {
        guard !pinnedRecords.contains(where: {
            $0.originProjectID == projectID
                || $0.id == projectID
                || $0.coveringGrants.contains { $0.id == projectID }
        })
        else { return }
        if let folder = securityScopedAccess.heldURL(for: projectID)?.path,
           let ws = pinnedWorkspace,
           ws.tabs.contains(where: { tab in
               tab.splitRoot.allPanes().contains { pane in
                   !pane.isRemote && SecurityScopedBookmark.folder(folder, covers: pane.projectPath)
               }
           })
        {
            return
        }
        securityScopedAccess.end(projectID: projectID)
    }
    ...
    /// `futuraterm.owner=<installationID>` on every session our panes claim and
    ...
    // MARK: - Grok session adoption (POR-372)

    private func rememberProjectsForAdoption(_ projects: [Project]) {
        knownProjectsForAdoption = projects
    }

    private func rememberProjectForAdoption(_ project: Project) {
        if let index = knownProjectsForAdoption.firstIndex(where: { $0.id == project.id }) {
            knownProjectsForAdoption[index] = project
        } else {
            knownProjectsForAdoption.append(project)
        }
    }

    /// Launch sequence: pinned materialize first (those names are claims),
    /// then adopt unclaimed grok sessions, then local reap against the
    /// post-adopt claim set. One Task so adopt and reap cannot race.
    func materializePinnedAdoptGrokAndReap(projects: [Project]) async {
        await materializeRestoredPinnedTabs(projects: projects)
        guard !workspaceStore.loadFailed else { return }
        await adoptUnclaimedGrokSessions(projects: projects)
        await zmx.reapOrphans(knownSessionNames: claimedSessionNames())
    }

    /// `selectProject` path: adopt first so the subsequent sweep's claim set
    /// includes the grok sessions, then warm any newly appended tabs.
    func adoptUnclaimedGrokSessionsThenSweep(
        project: Project,
        candidates: [GrokSessionAdoption.Candidate]? = nil
    ) async {
        await adoptUnclaimedGrokSessions(projects: knownProjectsForAdoption, candidates: candidates)
        sweepOrphanSessions(project: project)
        warmFocusedProject()
    }

    /// Attach unclaimed grok-fronted zmx sessions as tabs in the unique
    /// matching project. `candidates` is a test seam; production lists zmx.
    /// Live gather is skipped under `Preferences.isTestRun` so hosted tests
    /// cannot scan the developer's daemon.
    func adoptUnclaimedGrokSessions(
        projects: [Project],
        candidates: [GrokSessionAdoption.Candidate]? = nil
    ) async {
        guard !workspaceStore.loadFailed else { return }
        let resolved: [GrokSessionAdoption.Candidate]
        if let candidates {
            resolved = candidates
        } else if Preferences.isTestRun {
            return
        } else {
            guard let snapshot = await zmx.sessionListSnapshot() else { return }
            ZmxForegroundResolver.updateCache(snapshot.leaders)
            resolved = GrokSessionAdoption.candidates(
                from: snapshot.entries,
                claimed: claimedSessionNames(),
                inspect: Self.inspectGrokSession
            )
        }
        applyGrokSessionAssignments(
            GrokSessionAdoption.assign(resolved, projects: projects),
            projects: projects
        )
    }

    /// Rebuild environment-manager rows from `lsof` TCP LISTEN.
    /// Live gather is skipped under `Preferences.isTestRun` (hosted tests
    /// must not spawn `lsof`).
    func refreshLocalListeners(projects: [Project]) async {
        if Preferences.isTestRun {
            localListenerRows = []
            localListenerUnavailable = false
            isLocalListenerLoading = false
            return
        }
        isLocalListenerLoading = true
        defer { isLocalListenerLoading = false }
        let snapshot = await LocalListenerInventory.probe(
            uid: getuid(),
            selfPid: getpid(),
            cwd: { ProcessInspector.workingDirectory(pid: $0) },
            argv: { ProcessInspector.argv(pid: $0) },
            projects: projects
        )
        applyLocalListenerSnapshot(snapshot)
    }

    func applyLocalListenerSnapshot(_ snapshot: LocalListenerInventory.Snapshot) {
        switch snapshot {
        case .unavailable:
            localListenerRows = []
            localListenerUnavailable = true
        case let .ready(rows):
            localListenerUnavailable = false
            localListenerRows = rows
        }
    }

    /// SIGTERM each distinct live pid for the selected environment rows.
    /// Stale ids no-op. Never signals pid ≤ 1 or this process. Does not close
    /// panes or call zmx. Signaled rows drop immediately; refresh waits for
    /// those sockets to close (bounded) so a graceful shutdown isn't republished.
    func killLocalListeners(
        ids: [String],
        projects: [Project],
        signal: (pid_t) -> Int32 = { kill($0, SIGTERM) }
    ) async {
        let selfPid = ProcessInfo.processInfo.processIdentifier
        var signaled = Set<pid_t>()
        for id in ids {
            guard let row = localListenerRows.first(where: { $0.id == id }) else { continue }
            let pid = row.pid
            guard pid > 1, pid != selfPid else { continue }
            guard let start = row.startDate,
                  ProcessInspector.startDate(pid: pid) == start
            else { continue }
            guard ProcessInspector.comm(pid: pid) != nil else { continue }
            guard signaled.insert(pid).inserted else { continue }
            _ = signal(pid)
        }
        localListenerRows.removeAll { signaled.contains($0.pid) }
        await refreshLocalListenersAfterKill(signaled: signaled, projects: projects)
    }

    /// Hosted tests skip the wait (`refreshLocalListeners` already no-ops live
    /// gather). Production polls until the signaled pids disappear from `lsof`
    /// or 2s elapses.
    private func refreshLocalListenersAfterKill(
        signaled: Set<pid_t>,
        projects: [Project]
    ) async {
        if Preferences.isTestRun || signaled.isEmpty {
            await refreshLocalListeners(projects: projects)
            return
        }
        ...
        repeat {
            ...
            await refreshLocalListeners(projects: projects)
            if !localListenerRows.contains(where: { signaled.contains($0.pid) }) {
                return
            }
        } while ContinuousClock.now < deadline
    }

    enum OpenListenerResult: Equatable {
        case opened
        case failed
        case notFound
    }

    enum ShowListenerResult: Equatable {
        case focusedPane
        case selectedProject
        case none
    }

    /// Open the row's `displayURL` (`http://127.0.0.1:PORT` only). Injectable
    /// `openURL` so hosted tests never call `NSWorkspace`.
    @discardableResult
    func openLocalListenerInBrowser(
        id: String,
        openURL: (URL) -> Bool = { NSWorkspace.shared.open($0) }
    ) -> OpenListenerResult {
        guard let row = localListenerRows.first(where: { $0.id == id }) else {
            return .notFound
        }
        guard let url = URL(string: row.displayURL),
              url.scheme == "http",
              url.host == "127.0.0.1"
        else {
            return .failed
        }
        return openURL(url) ? .opened : .failed
    }

    enum ViewDesktopResult: Equatable {
        case opened
        case missing(String)
        case notRemote
    }

    /// Test seams for Sunshine pairing. Production leaves this nil and pairs
    /// only when `perform` is also nil (never spawn ssh/Moonlight in unit tests).
    struct ViewDesktopPairing {
        var backgroundSSH: Bool?
        var hostReady: ((Project) async -> Bool)?
        var isPaired: ((String) async -> Bool)?
        var startPair: ((String, String) async -> Bool)?
        var submitPin: ((Project, String, String) async -> Bool)?
    }

    /// Hand off the remote project's screen to Moonlight or Screen Sharing.
    /// Never creates a pane. Production resolves OS via `TailscaleStatus.probe`
    /// (async, non-blocking from the menu). Injectable `probe` / `perform` so
    /// tests never call `NSWorkspace` or the live Tailscale CLI.
    @discardableResult
    func viewDesktop(
        project: Project,
        osHint: String? = nil,
        devices: [TailscaleDevice] = [],
        moonlightAvailable: Bool? = nil,
        vncAvailable: Bool = true,
        probe: (@Sendable () async -> TailscaleSnapshot)? = nil,
        perform: ((DesktopLaunchAction) -> Bool)? = nil,
        ensureMoonlight: (() async -> Bool)? = nil,
        pairing: ViewDesktopPairing? = nil
    ) async -> ViewDesktopResult {
        guard project.isRemote, let host = ProjectPath.remoteHost(from: project.path) else {
            pendingViewDesktopError = DesktopView.notRemoteReason
            return .notRemote
        }
        let hint: String?
        if let osHint {
            ...
            hint = DesktopView.osHint(host: host, devices: devices)
        } else {
            let snapshot: TailscaleSnapshot = if let probe {
                await probe()
            } else {
                await TailscaleStatus.probe()
            }
            hint = DesktopView.osHint(host: host, devices: snapshot.devices)
        }
        var moonlight = moonlightAvailable ?? Self.macMoonlightAvailable()
        let needsMoonlight = DesktopPeerOS.fromHint(hint) != .macOS
        if !moonlight, needsMoonlight {
            if moonlightAvailable == nil {
                moonlight = await Self.installMoonlightIfNeeded()
            } else if let ensureMoonlight {
                moonlight = await ensureMoonlight()
            }
        }
        let plan = DesktopView.plan(
            host: host,
            osHint: hint,
            moonlightAvailable: moonlight,
            vncAvailable: vncAvailable
        )
        switch plan {
        case let .missing(reason):
            pendingViewDesktopError = reason
            return .missing(reason)
        case .moonlight,
             .vnc:
            if case let .moonlight(moonHost) = plan {
                let paired = await pairMoonlightIfNeeded(
                    host: moonHost,
                    project: project,
                    perform: perform,
                    pairing: pairing
                ...
                if !paired {
                    pendingViewDesktopError = DesktopView.moonlightLaunchFailedReason
                    return .missing(DesktopView.moonlightLaunchFailedReason)
                }
            }
            guard let action = DesktopView.action(for: plan) else {
                let reason = DesktopPeerOS.fromHint(hint) == .macOS
                    ? DesktopView.vncMissingReason
                    : DesktopView.moonlightMissingReason
                pendingViewDesktopError = reason
                return .missing(reason)
            }
            let ok: Bool = if let perform {
                perform(action)
            } else {
                await Self.performDesktopLaunch(action)
            }
            guard ok else {
                let reason = DesktopPeerOS.fromHint(hint) == .macOS
                    ? DesktopView.vncHostNotSharingReason
                    : DesktopView.moonlightLaunchFailedReason
                pendingViewDesktopError = reason
                return .missing(reason)
            }
            pendingViewDesktopError = nil
            return .opened
        }
    }

    /// Moonlight.app via Launch Services bundle id, then /Applications, then ~/Applications.
    static func moonlightAppURL(
        bundleURL: (String) -> URL? = {
            NSWorkspace.shared.urlForApplication(withBundleIdentifier: $0)
        },
        appExists: (String) -> Bool = { FileManager.default.fileExists(atPath: $0) }
    ) -> URL? {
        for id in DesktopView.moonlightBundleIDs {
            if let url = bundleURL(id) { return url }
        }
        let candidates = [
            MoonlightMacInstall.systemMoonlightURL(),
            MoonlightMacInstall.applicationsMoonlightURL(),
        ]
        return candidates.first { appExists($0.path) }
    }

    static func installMoonlightIfNeeded() async -> Bool {
        if macMoonlightAvailable() { return true }
        _ = await MoonlightMacInstall.installFromGitHub()
        return macMoonlightAvailable()
    }

    static func macMoonlightAvailable(
        bundleURL: (String) -> URL? = {
            NSWorkspace.shared.urlForApplication(withBundleIdentifier: $0)
        },
        appExists: (String) -> Bool = { FileManager.default.fileExists(atPath: $0) },
        ...
        if moonlightAppURL(bundleURL: bundleURL, appExists: appExists) != nil { return true }
        if binaryURL != nil { return true }
        if probePATH { return MoonlightMacInstall.binaryURL() != nil }
        return false
    }

    /// Moonlight: open the .app with `OpenConfiguration.arguments` =
    /// `stream <host> Desktop`. VNC: `vnc://<host>` via Screen Sharing.
    static func performDesktopLaunch(_ action: DesktopLaunchAction) async -> Bool {
        switch action {
        case let .moonlight(arguments):
            if let app = moonlightAppURL() {
            ...
            if let binary = MoonlightMacInstall.binaryURL() {
                let proc = Process()
    ...
    /// Pair Moonlight with Sunshine before `stream`. Injected tests with only
    /// `perform` skip this so they never spawn ssh or Moonlight.
    /// `false` means do not open Moonlight — the host is not accepting a pair.
    @discardableResult
    private func pairMoonlightIfNeeded(
        host: String,
        project: Project,
        perform: ((DesktopLaunchAction) -> Bool)?,
        pairing: ViewDesktopPairing?
    ) async -> Bool {
        let pairingEnabled = pairing?.backgroundSSH ?? Preferences.shared.backgroundSSHConnections
        guard pairingEnabled else { return true }
        let injected = pairing?.isPaired != nil || pairing?.startPair != nil || pairing?.submitPin != nil
            || pairing?.hostReady != nil
        guard injected || perform == nil else { return true }

        let alreadyPaired: Bool = if let isPaired = pairing?.isPaired {
            await isPaired(host)
        } else {
            await Self.moonlightHostIsPaired(host)
        }
        if alreadyPaired { return true }

        let ready: Bool = if let hostReady = pairing?.hostReady {
            await hostReady(project)
        } else if injected {
            true
        } else {
            await Self.sunshineHostReady(project)
        }
        guard ready else { return false }

        let pin = DesktopView.randomPairingPin()
        let name = DesktopView.pairingClientName(ProcessInfo.processInfo.hostName)
        let started: Bool = if let startPair = pairing?.startPair {
            await startPair(host, pin)
        } else {
            await Self.startMoonlightPair(host: host, pin: pin)
        }
        guard started else { return false }
        let submitted: Bool = if let submitPin = pairing?.submitPin {
            await submitPin(project, pin, name)
        } else {
            await Self.submitSunshinePin(project: project, pin: pin, clientName: name)
        }
        if pairing?.startPair == nil {
            try? await Task.sleep(for: .seconds(2))
        }
        return submitted
    }

    private static func moonlightCLIExecutable() -> URL? {
        if let binary = MoonlightMacInstall.binaryURL() { return binary }
        if let app = moonlightAppURL() {
            let macOS = app.appendingPathComponent("Contents/MacOS/Moonlight", isDirectory: false)
            if FileManager.default.isExecutableFile(atPath: macOS.path) { return macOS }
        }
        return nil
    }

    private static func moonlightHostIsPaired(_ host: String) async -> Bool {
        guard let exec = moonlightCLIExecutable() else { return false }
        return await runCLI(exec, DesktopView.moonlightListArguments(host: host), timeout: 5) == 0
    }

    private static func startMoonlightPair(host: String, pin: String) async -> Bool {
        guard let args = DesktopView.moonlightPairArguments(host: host, pin: pin) else { return false }
        if let exec = moonlightCLIExecutable() {
            return await runCLI(exec, args, timeout: 0, wait: false) != nil
        }
        return await performDesktopLaunch(.moonlight(arguments: args))
    }

    private static func submitSunshinePin(project: Project, pin: String, clientName: String) async -> Bool {
        guard let path = ProjectPath.parse(project.path),
              let argv = RemoteSpawn.sunshinePairArgv(remote: path, pin: pin, clientName: clientName)
        else { return false }
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/usr/bin/env")
        proc.arguments = ["ssh"] + argv
        proc.standardOutput = FileHandle.nullDevice
        proc.standardError = FileHandle.nullDevice
        return await runProcess(proc, timeout: 20) == 0
    }

    private static func sunshineHostReady(_ project: Project) async -> Bool {
        guard let path = ProjectPath.parse(project.path),
              let argv = RemoteSpawn.sunshineProbeArgv(remote: path)
        else { return false }
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: "/usr/bin/env")
        proc.arguments = ["ssh"] + argv
        ...
        let status = await runProcess(proc, timeout: 8)
    ...
    private static func runCLI(
        ...
        return await runProcess(proc, timeout: timeout)
    }

    private static func runProcess(_ proc: Process, timeout: TimeInterval) async -> Int32? {
        await withCheckedContinuation { continuation in
            let box = ProcessWaitBox()
    ...
    /// Focus a live pane whose foreground pid or process group matches the
    /// listener, else select the row's local project. Never creates a project
    /// or attaches zmx.
    @discardableResult
    func showLocalListenerInApp(
        id: String,
        projects: [Project],
        resolvedForegroundPID: (Pane) -> pid_t? = { ProcessInspector.resolvedForegroundPID(forPane: $0) },
        processGroupID: (pid_t) -> pid_t? = { ProcessInspector.processGroupID(pid: $0) }
    ) -> ShowListenerResult {
        let outcome = localListenerShowOutcome(
            id: id,
            projects: projects,
            resolvedForegroundPID: resolvedForegroundPID,
            processGroupID: processGroupID
        )
        switch outcome {
        case let .focusedPane(projectID, tabID, paneID):
            ...
            workspaces[projectID]?.tabs.first { $0.id == tabID }?.focusPane(paneID)
            return .focusedPane
    ...
    func canShowLocalListenerInApp(
        id: String,
        projects: [Project],
        resolvedForegroundPID: (Pane) -> pid_t? = { ProcessInspector.resolvedForegroundPID(forPane: $0) },
        processGroupID: (pid_t) -> pid_t? = { ProcessInspector.processGroupID(pid: $0) }
    ) -> Bool {
        localListenerShowOutcome(
            id: id,
            projects: projects,
            resolvedForegroundPID: resolvedForegroundPID,
            processGroupID: processGroupID
    ...
    private enum LocalListenerShowPlan: Equatable {
        case focusedPane(projectID: UUID, tabID: UUID, paneID: UUID)
        case selectedProject(Project)
        case none
    }

    private func localListenerShowOutcome(
        id: String,
        projects: [Project],
        resolvedForegroundPID: (Pane) -> pid_t?,
        processGroupID: (pid_t) -> pid_t?
    ) -> LocalListenerShowPlan {
        guard let row = localListenerRows.first(where: { $0.id == id }) else {
            return .none
        }
        for (projectID, workspace) in workspaces {
            for tab in workspace.tabs {
                for pane in tab.splitRoot.allPanes()
                    where paneOwnsListener(
                        panePID: resolvedForegroundPID(pane),
                        listenerPID: row.pid,
                        processGroupID: processGroupID
                    )
                {
                    return .focusedPane(projectID: projectID, tabID: tab.id, paneID: pane.id)
                }
            }
        }
    ...
    /// `npm run dev` (and similar wrappers) put the socket on a child whose
    /// pgid is the pane's foreground tpgid, not the child's own pid.
    private func paneOwnsListener(
        panePID: pid_t?,
        listenerPID: pid_t,
        processGroupID: (pid_t) -> pid_t?
    ) -> Bool {
        guard let panePID else { return false }
        if panePID == listenerPID { return true }
        return processGroupID(listenerPID) == panePID
    }

    /// Rebuild session-manager rows from a zmx listing joined to live panes.
    /// Live gather is skipped under `Preferences.isTestRun` unless `snapshot`
    /// is injected (hosted tests must not scan the developer's daemon).
    func refreshSessionInventory(
        projects: [Project],
        inspect: ((String) -> (comm: String?, cwd: String?))? = nil
    ) async {
        if Preferences.isTestRun {
            sessionInventoryRows = []
            sessionInventoryUnavailable = false
            isSessionInventoryLoading = false
            return
        }
        isSessionInventoryLoading = true
        defer { isSessionInventoryLoading = false }
        await applySessionInventorySnapshot(
            zmx.sessionListSnapshot(),
            projects: projects,
            inspect: inspect
        )
    }

    /// Test seam: apply a stub snapshot (including `nil` = listing unavailable).
    func applySessionInventorySnapshot(
        _ snapshot: (entries: [ZmxSessionListParser.Entry], leaders: [String: pid_t])?,
        projects: [Project],
        inspect: ((String) -> (comm: String?, cwd: String?))? = nil
    ) {
        guard let snapshot else {
            sessionInventoryRows = []
            sessionInventoryUnavailable = true
            return
        }
        sessionInventoryUnavailable = false
        ZmxForegroundResolver.updateCache(snapshot.leaders)
        sessionInventoryRows = SessionInventory.rows(
            entries: snapshot.entries,
            leaders: snapshot.leaders,
            claims: sessionInventoryClaims(projects: projects),
            projects: projects,
            inspect: inspect ?? Self.inspectGrokSession
        )
    }

    enum OpenInventoryResult: Equatable {
        case focused
        case attached
    ...
    /// Focus an already-attached inventory session, or adopt an unattached
    /// one as a new tab (`command: nil`, session name verbatim).
    @discardableResult
    func openInventorySession(_ name: String, projects: [Project]) -> OpenInventoryResult {
        // Live workspaces win over a stale inventory snapshot (second open
        // before the sheet refreshes must not spawn a duplicate pane).
        if let claim = sessionInventoryClaims(projects: projects)[name] {
            focusClaimedInventorySession(claim, projects: projects)
            return .focused
        }
        guard let row = sessionInventoryRows.first(where: { $0.name == name }) else {
            return .notFound
        }
        guard let dest = destinationForUnattachedInventorySession(row, projects: projects) else {
            return .needsLocalProject
        }
        attachInventorySession(name: name, to: dest)
        return .attached
    }

    private func focusClaimedInventorySession(_ claim: SessionInventory.Claim, projects: [Project]) {
        if claim.projectID == PinnedTabs.projectID {
            selectPinnedTab(claim.tabID)
        } else if let project = projects.first(where: { $0.id == claim.projectID }) {
            selectProject(project)
        }
        selectTab(claim.tabID, projectID: claim.projectID)
        workspaces[claim.projectID]?.tabs.first { $0.id == claim.tabID }?.focusPane(claim.paneID)
    }

    private func destinationForUnattachedInventorySession(
        _ row: SessionInventory.Row,
        projects: [Project]
    ) -> Project? {
        if let suggested = row.suggestedProjectName {
            let matches = projects.filter {
                $0.name == suggested && !$0.isRemote && $0.id != PinnedTabs.projectID
            }
            if matches.count == 1 { return matches[0] }
        }
        if let active = projects.first(where: { $0.id == activeProjectID }),
           !active.isRemote,
           active.id != PinnedTabs.projectID
        {
            return active
        }
        if activeProjectID == PinnedTabs.projectID,
           ZmxSessionName.slug(fromName: row.name) == ZmxSessionName.quickTerminalSlug
        {
    ...
    private func attachInventorySession(name: String, to dest: Project) {
        let projectID = dest.id
        let projectPath = dest.path
        ensureWorkspace(projectID: projectID, path: projectPath)
        ...
        workspace.adoptTab(tab, activate: true)
        if projectID == PinnedTabs.projectID {
            selectPinnedTab(tab.id)
        } else {
            selectProject(dest)
            selectTab(tab.id, projectID: projectID)
        }
        saveWorkspaces()
    ...
    /// Kill selected inventory sessions after the sheet’s one confirmation.
    /// Attached rows force-close their pane (`closePane`); unattached/foreign
    /// rows go through `zmx.killSession`. Unknown names are ignored.
    func killInventorySessions(_ names: [String], projects: [Project]) async {
        guard !names.isEmpty else { return }
        let rowsByName = Dictionary(uniqueKeysWithValues: sessionInventoryRows.map { ($0.name, $0) })
        var killed = false
        for name in names {
            guard let row = rowsByName[name] else { continue }
            killed = true
            if row.attachment == .attached, let claim = row.claim {
                await zmx.killSession(name)
                closePane(claim.paneID, projectID: claim.projectID)
            } else {
                await zmx.killSession(name)
            }
        }
        ...
        if Preferences.isTestRun { return }
        await refreshSessionInventory(projects: projects)
    }

    func sessionInventoryClaims(projects: [Project]) -> [String: SessionInventory.Claim] {
        let namesByID = Dictionary(uniqueKeysWithValues: projects.map { ($0.id, $0.name) })
        var claims: [String: SessionInventory.Claim] = [:]
        for (projectID, workspace) in workspaces {
            let projectName: String = if projectID == PinnedTabs.projectID {
                PinnedTabs.project.name
            } else {
                namesByID[projectID] ?? ""
            }
            for tab in workspace.tabs {
                for pane in tab.splitRoot.allPanes() {
                    claims[pane.sessionName] = SessionInventory.Claim(
                        paneID: pane.id,
                        tabID: tab.id,
                        projectID: projectID,
    ...
    nonisolated private static func inspectGrokSession(sessionName: String) -> (comm: String?, cwd: String?) {
        guard let pid = ZmxForegroundResolver.foregroundPID(sessionName: sessionName) else {
            return (nil, nil)
        }
        return (ProcessInspector.comm(pid: pid), ProcessInspector.workingDirectory(pid: pid))
    }

    private func applyGrokSessionAssignments(
        _ assignments: [GrokSessionAdoption.Assignment],
        projects: [Project]
    ) {
        guard !assignments.isEmpty else { return }
        var adopted = 0
        var adoptedActive = false
        for assignment in assignments {
            if claimedSessionNames().contains(assignment.sessionName) { continue }
            guard let project = projects.first(where: { $0.id == assignment.projectID }),
                  !project.isRemote,
                  project.id != PinnedTabs.projectID
            else { continue }
            ensureWorkspace(projectID: project.id, path: project.path)
            guard let workspace = workspaces[project.id] else { continue }
            let pane = Pane(
            ...
            let tab = TerminalTab(id: UUID(), splitRoot: .pane(pane), focusedPaneID: pane.id)
            workspace.adoptTab(tab, activate: false)
            adopted += 1
        ...
        logger.info("adopted \(adopted, privacy: .public) grok session(s)")
        saveWorkspaces()
        if adoptedActive {
            warmFocusedProject()
        }
    ...
    func openProject(store: ProjectStore) -> Project? {
        ...
        // Find-or-select: a folder that already backs a sidebar project jumps
        // to that row rather than appending a duplicate.
        let bookmark = SecurityScopedBookmark.create(from: url)
        return addOrSelectProject(
            store: store,
    ...
    func replaceProjectPathWithCurrentDir(projectStore: ProjectStore) {
        ...
        let covers = SecurityScopedBookmark.folder(project.path, covers: pwd)
        let picked: URL? = if covers {
            nil
        } else if folderGrantUsesSecurityScope {
            pickFolderForPathReplacement?(pwd)
                ?? presentReplacementFolderPanel(startingAt: pwd, projectName: project.name)
        } else {
            URL(fileURLWithPath: pwd)
        }
        _ = commitReplacedProjectPath(
    ...
    private func presentReplacementFolderPanel(startingAt path: String, projectName: String) -> URL? {
        let panel = NSOpenPanel()
        panel.canChooseDirectories = true
        panel.canChooseFiles = false
        panel.allowsMultipleSelection = false
        panel.directoryURL = URL(fileURLWithPath: path)
        panel.message = "FuturaTerm needs access to “\(projectName)” at this folder."
        panel.prompt = "Grant Access"
        return panel.runModal() == .OK ? panel.url : nil
    }

    /// Persist a replaced project path only after a covering bookmark exists.
    /// `pickedURL` is required when the new path leaves the granted tree.
    @discardableResult
    func commitReplacedProjectPath(
        projectStore: ProjectStore,
        projectID: UUID,
        newPath: String,
        pickedURL: URL?
        ...
        let bookmark: Data?
        if SecurityScopedBookmark.folder(project.path, covers: normalized) {
            bookmark = remintCoveringBookmark(
                projectID: projectID,
                targetPath: normalized
            ) ?? project.securityScopedBookmark
        } else {
            guard let pickedURL,
                  SecurityScopedBookmark.folder(pickedURL.path, covers: normalized)
            else { return false }
            guard let data = SecurityScopedBookmark.create(from: pickedURL) else { return false }
            bookmark = data
        }
        projectStore.setPath(id: projectID, to: normalized, bookmark: bookmark)
        if let updated = projectStore.projects.first(where: { $0.id == projectID }) {
            rememberProjectForAdoption(updated)
            preservePinnedOriginGrant(from: updated)
            beginSecurityScopedAccess(for: updated)
        }
        return true
    }

    /// Remint using the live held URL as the grant root. A parent Open-panel
    /// pick starts that parent while `project.path` stays nested; treating the
    /// project path as the root mints a sibling of the held folder.
    private func remintCoveringBookmark(projectID: UUID, targetPath: String) -> Data? {
        let url: URL = if let held = securityScopedAccess.heldURL(for: projectID) {
            SecurityScopedBookmark.childURL(
                held: held,
                grantedPath: held.path,
                targetPath: targetPath
            )
        } else {
            URL(fileURLWithPath: targetPath)
        }
        return SecurityScopedBookmark.create(from: url)
    }
    ...
    func unloadProject(_ projectID: UUID) {
        ...
        endFolderGrantUnlessPinned(projectID)
        if activeProjectID == projectID {
            activeProjectID = nil
        }
        saveWorkspaces()
    ...
    /// no live workspace. Cmd+T, palette, sidebar New Tab, and CLI `tab.new`
    /// all land here — the one place a default `grok` start is applied.
    @discardableResult
    func createTab(projectID: UUID, projectPath: String, command: String? = nil) -> UUID? {
        let command = resolvedNewTabCommand(explicit: command, projectID: projectID)
    ...
    /// Resolve the spawn command for a new tab. A non-empty explicit command
    /// always wins; pinned tabs and a disabled preference stay blank.
    func resolvedNewTabCommand(explicit: String?, projectID: UUID) -> String? {
        if let explicit, !explicit.isEmpty { return explicit }
        if projectID == PinnedTabs.projectID { return nil }
        guard Preferences.shared.startGrokInNewTabs else { return nil }
        return "grok"
    }
    ...
    func moveTab(
        ...
        if ProjectPath.isRemote(destPath) {
            knownRemoteProjectIDs.insert(destProjectID)
        } else {
            knownRemoteProjectIDs.remove(destProjectID)
        }
    ...
    func mergeTab(
        ...
        // Capture before detach: merging a pinned tab removes its record.
        let sourceFolderPath = workspaces[sourceProjectID]?
            .tabs.first(where: { $0.id == tabID })?
            .splitRoot.allPanes().first { !$0.isRemote }?.projectPath
        let mergeOrigin: PinnedOriginStamp? = if destProjectID == PinnedTabs.projectID {
            pinnedOriginStamp(
        ...
        guard let sourceTab = detachTabForMerge(tabID, from: sourceProjectID, to: destProjectID)
        else { return }
        ...
        if let mergeOrigin {
            inheritPinnedOriginIfNeeded(forTab: destTab.id, origin: mergeOrigin)
        }
    ...
    func separateTabPanes(_ tabID: UUID, projectID: UUID) {
        ...
        let origin: PinnedOriginStamp? = if projectID == PinnedTabs.projectID {
            pinnedOriginStamp(
                from: projectID,
                sourceTabID: tabID,
                localFolderPath: firstPane.isRemote ? nil : firstPane.projectPath
            )
        } else {
            nil
        }
        for (offset, pane) in panes.dropFirst().enumerated() {
            ...
            if let origin, origin.hasOrigin {
                stampPinnedOrigin(forTab: newTab.id, origin: origin)
            }
        }
        ws.selectTab(tabID)
        saveWorkspaces()
    ...
    func separatePane(_ paneID: UUID, toProject destProjectID: UUID, destPath: String, at index: Int? = nil) {
        ...
        if sourceTab.zoomedPaneID == paneID {
        ...
        if Preferences.shared.autoTilingEnabled {
            sourceTab.splitRoot.rebalanced()
        }
        ...
        if destProjectID == PinnedTabs.projectID {
            let origin = pinnedOriginStamp(
                ...
                localFolderPath: pane.isRemote ? nil : pane.projectPath
            )
            stampPinnedOrigin(forTab: newTab.id, origin: origin)
        }
        activeProjectID = destProjectID
    ...
    func splitPane(direction: SplitDirection, projectID: UUID) {
        ...
        coverUncoveredPinnedPaneCwdsIfNeeded(tab: tab, projectID: projectID)
        saveWorkspaces()
    }
    ...
    /// `pendingLayoutError` (the alert in `FuturaTermApp`). The shared entry
    ...
    /// ghostty `prompt_surface_title` keybind's FuturaTerm mapping (titles live
    ...
    /// `paneID` — the ghostty `set_tab_title` keybind's FuturaTerm mapping,
    ...
    func navigateToPane(_ paneID: UUID, projectID: UUID) {
        ...
        if let tab = workspaces[projectID]?.tabs.first(where: { $0.splitRoot.findPane(id: paneID) != nil }) {
            ...
            // FuturaTerm is inactive) returns nil and makes restoreFocus no-op. Fall
