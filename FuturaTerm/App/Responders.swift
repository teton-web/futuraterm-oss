...
// FuturaTermApp.swift. These responders own focused slices of that logic and get
...
final class QuickTerminalResponder: KeyResponder {
    ...
    func handle(_ event: NSEvent) -> KeyDisposition {
        ...
        // Quick-terminal toggle keystroke arrived while FuturaTerm itself is
...
final class MainAppResponder: KeyResponder {
    ...
    func handle(_ event: NSEvent) -> KeyDisposition {
        ...
        // hot key (see QuickTerminalService) for when FuturaTerm isn't active;
        ...
        if HotkeyRegistry.matches(event, action: .nextProject) {
            appState.selectNextProject(projects: projectStore.sidebarProjectOrder)
            return .handled
        }
        if HotkeyRegistry.matches(event, action: .previousProject) {
            appState.selectPreviousProject(projects: projectStore.sidebarProjectOrder)
            return .handled
        }
