...
///
/// FuturaTerm only stores app-shaped state here (window opacity/blur, quick
/// terminal, hotkeys, etc.). Anything that's a ghostty config setting lives
/// in the user's Ghostty config instead — see `FuturaTermConfig` for the wrapper
/// files FuturaTerm generates around it.
@MainActor @Observable
final class Preferences {
    static let shared = Preferences(defaults: defaults)

    /// The UserDefaults domain all FuturaTerm state persists to — `.standard` in
    ...
    /// Type `grok` into each new project tab's shell (Cmd+T, palette, sidebar,
    /// CLI `tab.new`). Off leaves a blank prompt. Does not apply to the
    /// workspace's first tab, restored snapshots, layouts, splits, or pinned
    /// tabs. On by default.
    var startGrokInNewTabs: Bool {
        didSet { defaults.set(startGrokInNewTabs, forKey: Keys.startGrokInNewTabs) }
    }
    ...
    /// Remaining quota for signed-in coding agents in the sidebar footer.
    /// On by default. Off hides the usage row without changing New Project.
    var showAgentUsage: Bool {
        didSet { defaults.set(showAgentUsage, forKey: Keys.showAgentUsage) }
    }
    ...
    /// can't suppress, so off narrows FuturaTerm's ssh traffic to the panes' own
    ...
    /// onto remote zmx sessions as a `futuraterm.owner` label so the orphan sweep
    ...
    /// Expanded sidebar project folders (POR-327). Independent of in-memory
    /// `expandedProjects` so folder collapse survives relaunch.
    var expandedSidebarFolderIDs: Set<UUID> {
        didSet {
            defaults.set(expandedSidebarFolderIDs.map(\.uuidString), forKey: Keys.expandedSidebarFolderIDs)
        }
    }
    ...
    /// bindings live in raw UserDefaults keys (`futuraterm.hotkey.<action_id>`),
    ...
    /// FuturaTerm-painted window background opacity (0–1). FuturaTerm composites
    /// this translucency at the window level while `futuraterm-overrides.conf`
    ...
    /// `.futuraTermConfigDidChange` post it ends with — heavyweight, and fired
    ...
    /// `com.davidsolheim.futuraterm.debug` domain — a test mutating a preference (even
    ...
    private init(defaults: UserDefaults) {
        ...
        Self.migrateFromLegacyKeys(defaults: defaults)
        ...
        startGrokInNewTabs = defaults.object(forKey: Keys.startGrokInNewTabs) as? Bool ?? true
        showNewProjectButton = defaults.object(forKey: Keys.showNewProjectButton) as? Bool ?? true
        showAgentUsage = defaults.object(forKey: Keys.showAgentUsage) as? Bool ?? true
        ...
        expandedSidebarFolderIDs = Set(
            (defaults.array(forKey: Keys.expandedSidebarFolderIDs) as? [String] ?? [])
    ...
    /// Copy each live `futuraterm.*` key from its `macterm.` legacy key when
    /// the new key is absent, so an upgrade keeps the user's stored prefs.
    private static func migrateFromLegacyKeys(defaults: UserDefaults) {
        let prefix = "futuraterm."
        let legacyPrefix = "macterm."
        for key in Keys.all {
            guard key.hasPrefix(prefix), defaults.object(forKey: key) == nil else { continue }
            let legacy = legacyPrefix + key.dropFirst(prefix.count)
    ...
    /// are dead. Drop them so `defaults read com.davidsolheim.futuraterm` is clean
    ...
    private static func runOneTimeMigrations(defaults: UserDefaults) {
        ...
        // app-side slider key is dead. Drop the stored legacy key.
    ...
    enum Keys {
        static let autoTiling = "futuraterm.autoTiling.enabled"
        static let terminalScrollSpeed = "futuraterm.terminal.scrollSpeed"
        static let windowOpacity = "futuraterm.window.opacity"
        static let windowBlurRadius = "futuraterm.window.blurRadius"
        static let windowGlassEnabled = "futuraterm.window.glassEnabled"
        static let windowGlassStyle = "futuraterm.window.glassStyle"
        static let adaptiveTerminalChromeEnabled = "futuraterm.window.adaptiveTerminalChromeEnabled"
        static let hideTitleBar = "futuraterm.window.hideTitleBar"
        static let loadsDefaultGhosttyConfigFiles = "futuraterm.ghostty.loadsDefaultConfigFiles"
        static let customGhosttyConfigPaths = "futuraterm.ghostty.customConfigPaths"
        /// Legacy single-path key. Read only for migration.
        static let userGhosttyConfigPath = "futuraterm.ghostty.userConfigPath"
        static let passthroughPrograms = "futuraterm.hotkey.passthroughPrograms"
        static let quickTerminalWidth = "futuraterm.quickTerminal.width"
        static let quickTerminalHeight = "futuraterm.quickTerminal.height"
        static let quickTerminalPositionMode = "futuraterm.quickTerminal.positionMode"
        static let quickTerminalFixedX = "futuraterm.quickTerminal.fixedX"
        static let quickTerminalFixedY = "futuraterm.quickTerminal.fixedY"
        static let quickTerminalSizeMode = "futuraterm.quickTerminal.sizeMode"
        static let quickTerminalDynamicWidth = "futuraterm.quickTerminal.dynamicWidth"
        static let quickTerminalDynamicHeight = "futuraterm.quickTerminal.dynamicHeight"
        static let quickTerminalPositionX = "futuraterm.quickTerminal.positionX"
        static let quickTerminalPositionY = "futuraterm.quickTerminal.positionY"
        static let activeProjectID = "futuraterm.activeProjectID"
        static let projectIconSymbol = "futuraterm.sidebar.projectIcon"
        static let tabIconSymbol = "futuraterm.sidebar.tabIcon"
        static let sidebarIconSize = "futuraterm.sidebar.iconSize"
        static let showAgentIcons = "futuraterm.sidebar.showAgentIcons"
        static let showTabStatusIndicator = "futuraterm.sidebar.showTabStatusIndicator"
        static let showSpinnerOverAgentIcons = "futuraterm.sidebar.showSpinnerOverAgentIcons"
        static let autoNameTabs = "futuraterm.tabs.autoName"
        static let startGrokInNewTabs = "futuraterm.tabs.startGrokInNewTabs"
        static let showNewProjectButton = "futuraterm.sidebar.showNewProjectButton"
        static let showAgentUsage = "futuraterm.sidebar.showAgentUsage"
        static let backgroundSSHConnections = "futuraterm.remote.backgroundSSHConnections"
        static let reconnectRemotePanes = "futuraterm.remote.reconnectDroppedPanes"
        static let installationID = "futuraterm.installationID"
        static let peekSidebarWhenHidden = "futuraterm.sidebar.peekWhenHidden"
        static let sidebarWidth = "futuraterm.sidebar.width"
        static let expandedSidebarFolderIDs = "futuraterm.sidebar.expandedFolderIDs"
        static let updateChannel = "futuraterm.updates.channel"
        static let tabSwitcherVisibility = "futuraterm.toolbar.tabSwitcherVisibility"
        static let tabSwitcherPosition = "futuraterm.toolbar.tabSwitcherPosition"
        static let migrationV2GhosttyConfigOwned = "futuraterm.migration.v2_ghostty_config_owned"
        static let migrationRetiredToggleKeys = "futuraterm.migration.retired_toggle_keys"
        static let migrationRetiredPaneDimKey = "futuraterm.migration.retired_pane_dim_key"

        /// Live keys copied from a matching `macterm.` legacy key on first read.
        static let all: [String] = [
            autoTiling, terminalScrollSpeed, windowOpacity, windowBlurRadius,
            windowGlassEnabled, windowGlassStyle, adaptiveTerminalChromeEnabled,
            hideTitleBar, loadsDefaultGhosttyConfigFiles, customGhosttyConfigPaths,
            userGhosttyConfigPath, passthroughPrograms, quickTerminalWidth,
            quickTerminalHeight, quickTerminalPositionMode, quickTerminalFixedX,
            quickTerminalFixedY, quickTerminalSizeMode, quickTerminalDynamicWidth,
            quickTerminalDynamicHeight, quickTerminalPositionX, quickTerminalPositionY,
            activeProjectID, projectIconSymbol, tabIconSymbol, sidebarIconSize,
            showAgentIcons, showTabStatusIndicator, showSpinnerOverAgentIcons,
            autoNameTabs, startGrokInNewTabs, showNewProjectButton, showAgentUsage,
            backgroundSSHConnections, reconnectRemotePanes, installationID,
            peekSidebarWhenHidden, sidebarWidth, expandedSidebarFolderIDs,
            updateChannel, tabSwitcherVisibility, tabSwitcherPosition,
            migrationV2GhosttyConfigOwned, migrationRetiredToggleKeys,
            migrationRetiredPaneDimKey,
        ]
    }
}
