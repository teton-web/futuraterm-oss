...
struct AppCommandMenuItem: View {
    ...
    var body: some View {
        ...
        // call site can't silently orphan the sync. Help is registered for
        // every item, including Unload/Remove which have no `HotkeyAction`,
        // because SwiftUI `.help()` on menu-bar items is not always copied
        // onto the live `NSMenuItem`.
        .task {
            if let hotkeyAction = command.hotkeyAction {
                HotkeyMenuSync.register(title: title, action: hotkeyAction)
            }
            HotkeyMenuSync.registerHelp(title: title, tooltip: command.help)
        }
    }
}
