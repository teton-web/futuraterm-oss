...
struct AppCommandMenuItem: View {
    ...
    var body: some View {
        ...
        .modifier(ProjectActionHelpModifier(command: command))
        ...
        // call site can't silently orphan the sync. Unload/Remove also pin
        // `toolTip` here because SwiftUI `.help()` on menu-bar items is not
        // always copied onto the live `NSMenuItem`.
        .task {
            if let hotkeyAction = command.hotkeyAction {
                HotkeyMenuSync.register(title: title, action: hotkeyAction)
            }
            if let help = ProjectActionHelp.help(for: command) {
                HotkeyMenuSync.registerHelp(title: title, tooltip: help)
            }
        }
    }
}
...
/// Maps FuturaTerm's stringly-typed shortcut (e.g. `cmd+shift+l`) to SwiftUI's
...
/// key (e.g. function keys aren't in FuturaTerm's hotkey grammar yet).
@MainActor
