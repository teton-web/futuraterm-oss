...
enum UpdaterAvailability {
    ...
    /// Public Sparkle feed. Must match `SUFeedURL` in Info.plist and the
    /// `https://futuraterm.com/appcast.xml` URL written by `scripts/publish-appcast.sh`.
    static let appcastURL = "https://futuraterm.com/appcast.xml"

    /// Settings copy when Sparkle is not started (Debug, local Release with
    /// the placeholder key, benchmark). One sentence; `.settingsCaption()`.
    static let localBuildUpdatesCaption =
        "Checking for updates is available in released builds of FuturaTerm, not this local build."

    static let appStoreUpdatesCaption =
        "Mac App Store builds receive updates from the App Store, not Sparkle."
...
#if APP_STORE
/// MAS must not ship Sparkle. Same surface as the Direct wrapper so Settings
/// and commands compile; every check is a no-op.
...
final class Updater {
    ...
    var updateChannel: UpdateChannel {
        ...
        set { Preferences.shared.updateChannel = newValue }
    }
