...
enum EnvironmentSetup {
    ...
    static func runOnce() {
        ...
        // MAS sandbox cannot use the default `/tmp/zmx-<uid>` (or a user
        // TMPDIR outside the container). Pin ZMX_DIR so zmx still wraps;
        // never skip the daemon on this channel.
        #if APP_STORE
        pinAppStoreZmxDir()
        #endif
    }
    ...
    #if APP_STORE
    private static func pinAppStoreZmxDir() {
        let caches = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first
            ?? FileStorage.appSupportDirectory()
        var extras: [URL] = []
        if let darwin = AppStoreZmxDir.darwinUserTemporaryDirectory() {
            extras.append(darwin)
        }
        extras.append(AppStoreZmxDir.entitledConfigDirectory())
        let dir = AppStoreZmxDir.url(
            appSupport: FileStorage.appSupportDirectory(),
            caches: caches,
