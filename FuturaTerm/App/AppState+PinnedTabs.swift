...
extension AppState {
    ...
    func pinTab(_ tabID: UUID, fromProject sourceProjectID: UUID, toRecordIndex: Int? = nil) {
        ...
        // Origin folder is the source project, not first-pane cwd: merge at
        // `.rootEdge(.top)` / `.left` puts B first, and stamping B here would
        // make persist treat B as already covered and skip A's covering grant.
        let record = PinnedTabRecord(
            id: tabID,
            declaration: declaration,
            originProjectID: sourceProjectID,
            originFolderPath: originFolderPath(for: sourceProjectID),
            originFolderBookmark: originFolderBookmark(for: sourceProjectID)
        )
        pinnedRecords.insert(record, at: recordIndex)
        // Mixed-cwd tabs (merge B into A, then Pin) need covering grants for
        // every local pane the origin folder does not cover — otherwise
        // removing B drops the only bookmark and restore never prompts.
        persistCoveringGrantsForUncoveredLocalPanes(tabID: tabID)
    ...
    func restorePinnedState(_ snapshots: [PinnedTabSnapshot], activeTabID: UUID? = nil) {
        pinnedRecords = snapshots.map {
            PinnedTabRecord(
                ...
                originProjectID: $0.originProjectID,
                originFolderPath: $0.originFolderPath,
                originFolderBookmark: $0.originFolderBookmark,
                coveringGrants: $0.coveringGrants ?? []
            )
        }
    ...
    func materializeRestoredPinnedTabs(projects: [Project] = []) async {
        ...
        // Covering grants match live pane cwds against the project list —
        // restore used to start this before tabs existed, which no-op'd.
    ...
    func warmPinnedTabs() {
        ...
        let panes = ws.tabs.flatMap { $0.splitRoot.allPanes() }
            .filter { shouldSpawnPane($0, workspaceID: PinnedTabs.projectID) }
        warmStaggered(panes) { [weak self] pane in
    ...
    /// the workspace — CLI `tab new`, a separated pane whose origin was
    /// already stamped, a merge — is pinned from birth), then rewrite
    /// `pinned.yaml` iff membership changed. Called from every
    /// `saveWorkspaces()`, so it must stay cheap on the steady path: a set
    /// compare when nothing moved. Origin-less minting is only for tabs
    /// born pinned (Cmd+T); a live pane separated or merged from a project
    /// stamps origin first so MAS spawn gating keeps the source grant.
    func syncPinnedRecordsWithWorkspace() {
        if let ws = pinnedWorkspace {
            ...
            for tab in ws.tabs where !known.contains(tab.id) {
                let declaration = LayoutSerializer.pinnedDeclaration(for: tab)
                let originFolderPath = tab.splitRoot.allPanes().first { !$0.isRemote }?.projectPath
                let record = PinnedTabRecord(
                    id: tab.id,
                    declaration: declaration,
                    originProjectID: nil,
                ...
                beginGrantForPinnedRecord(record)
            }
        }
    ...
    // MARK: - Origin stamps (MAS folder grant)

    /// Source-project (or source-record) origin to copy onto a live pinned tab.
    /// `pinTab` stamps this at pin time; separate-into-pinned and merge into
    /// an origin-less pinned tab must stamp the same way so spawn gating
    /// keeps using the grant the surface was created under.
    struct PinnedOriginStamp {
        var originProjectID: UUID?
        var originFolderPath: String?
        var originFolderBookmark: Data?
        /// Origin-less source (Cmd+T) uses the source record id so a merge
        /// into an originated dest can persist that leftover as a covering grant.
        var coveringGrantID: UUID?

        var hasOrigin: Bool { originProjectID != nil || originFolderBookmark != nil }
    }

    func pinnedOriginStamp(
        from sourceProjectID: UUID,
        sourceTabID: UUID,
        localFolderPath: String?
    ) -> PinnedOriginStamp {
        if sourceProjectID == PinnedTabs.projectID {
            if let record = pinnedRecord(sourceTabID) {
                return PinnedOriginStamp(
                    originProjectID: record.originProjectID,
                    originFolderPath: record.originFolderPath ?? localFolderPath,
                    originFolderBookmark: record.originFolderBookmark,
                    coveringGrantID: record.originProjectID == nil ? record.id : nil
                )
            }
            return PinnedOriginStamp(
                originProjectID: nil,
                originFolderPath: localFolderPath,
                originFolderBookmark: nil,
                coveringGrantID: sourceTabID
            )
        }
        return PinnedOriginStamp(
            originProjectID: sourceProjectID,
            originFolderPath: originFolderPath(for: sourceProjectID) ?? localFolderPath,
            originFolderBookmark: originFolderBookmark(for: sourceProjectID)
        )
    }

    /// Insert or update a live pinned tab's record with the source origin
    /// *before* `saveWorkspaces` mints an origin-less row around a running
    /// surface. Path and bookmark always come from `origin` — Cmd+T dests
    /// already have home leftovers, and keeping those would start the source
    /// id with a grant that does not cover the incoming pane cwd. Leftover
    /// dest cwd grants the new origin does not cover stay as covering grants
    /// so restore can still spawn (or prompt for) those panes.
    func stampPinnedOrigin(forTab tabID: UUID, origin: PinnedOriginStamp) {
        if let i = pinnedRecords.firstIndex(where: { $0.id == tabID }) {
            persistUncoveredDestCoveringGrants(at: i, replacingWith: origin)
            pinnedRecords[i].originProjectID = origin.originProjectID
            pinnedRecords[i].originFolderPath = origin.originFolderPath
            pinnedRecords[i].originFolderBookmark = origin.originFolderBookmark
            beginGrantForPinnedRecord(pinnedRecords[i])
            persistCoveringGrantsForUncoveredLocalPanes(tabID: tabID)
            return
        }
        guard let ws = pinnedWorkspace, let tab = ws.tabs.first(where: { $0.id == tabID }) else { return }
        let record = PinnedTabRecord(
            id: tabID,
            declaration: LayoutSerializer.pinnedDeclaration(for: tab),
            originProjectID: origin.originProjectID,
            originFolderPath: origin.originFolderPath,
            originFolderBookmark: origin.originFolderBookmark
        )
        pinnedRecords.insert(record, at: recordIndex(forLiveTab: tabID, in: ws))
        beginGrantForPinnedRecord(record)
        persistCoveringGrantsForUncoveredLocalPanes(tabID: tabID)
    }

    /// Merging a project (or originated pinned) tab into a pinned dest must
    /// not drop the source grant. Origin-less dests (Cmd+T) replace home
    /// leftovers from the source stamp. A dest originated from A keeps A
    /// for unpin, and still starts a grant covering the incoming pane cwd —
    /// including an origin-less (Cmd+T) leftover that has no originProjectID.
    func inheritPinnedOriginIfNeeded(forTab tabID: UUID, origin: PinnedOriginStamp) {
        guard let i = pinnedRecords.firstIndex(where: { $0.id == tabID }) else { return }
        if pinnedRecords[i].originProjectID == nil {
            if origin.hasOrigin {
                stampPinnedOrigin(forTab: tabID, origin: origin)
            }
            persistCoveringGrantsForUncoveredLocalPanes(tabID: tabID)
            return
        }
        appendCoveringGrant(at: i, origin: origin)
        startAdditionalPinnedOriginGrant(origin)
        persistCoveringGrantsForUncoveredLocalPanes(tabID: tabID)
    }

    /// Keep dest cwd grants the incoming origin does not cover (Cmd+T home).
    func persistUncoveredDestCoveringGrants(
        at index: Int,
        replacingWith origin: PinnedOriginStamp
    ) {
        if let newPath = origin.originFolderPath, !ProjectPath.isRemote(newPath) {
            pinnedRecords[index].coveringGrants.removeAll {
                SecurityScopedBookmark.folder(newPath, covers: $0.folderPath)
            }
        }
        let record = pinnedRecords[index]
        guard let leftoverPath = record.originFolderPath, !ProjectPath.isRemote(leftoverPath)
        else { return }
        if let newPath = origin.originFolderPath, !ProjectPath.isRemote(newPath),
           SecurityScopedBookmark.folder(newPath, covers: leftoverPath)
        {
            return
        }
        appendCoveringGrant(
            at: index,
            id: record.id,
            folderPath: leftoverPath,
            bookmark: record.originFolderBookmark
        )
    }

    /// Keep covering grants keyed by a replaced project when the new folder
    /// no longer covers them (origin shrink). Copy leftovers onto a leftover
    /// grant id so `project.id` can hold the child without dropping the parent.
    /// Dest-home leftovers already occupy `record.id`; a leftover that is
    /// disjoint *or* strictly narrower must mint its own id rather than
    /// first-id-wins drop or fill dest-home with a child bookmark.
    func persistUncoveredCoveringGrants(at index: Int, replacingProject project: Project) {
        guard !ProjectPath.isRemote(project.path) else { return }
        let leftovers = pinnedRecords[index].coveringGrants.filter {
            $0.id == project.id && !SecurityScopedBookmark.folder(project.path, covers: $0.folderPath)
        }
        for leftover in leftovers {
            appendCoveringGrant(
                at: index,
                id: pinnedRecords[index].id,
                folderPath: leftover.folderPath,
                bookmark: leftover.folderBookmark
            )
        }
    }

    /// True when every shrink leftover keyed by `project.id` now also lives
    /// under a different covering-grant id. Restamping `project.id` onto the
    /// child folder must not run until those copies exist.
    func uncoveredCoveringLeftoversCopied(at index: Int, replacingProject project: Project) -> Bool {
        guard !ProjectPath.isRemote(project.path) else { return true }
        let leftovers = pinnedRecords[index].coveringGrants.filter {
            $0.id == project.id && !SecurityScopedBookmark.folder(project.path, covers: $0.folderPath)
        }
        return leftovers.allSatisfy { leftover in
            pinnedRecords[index].coveringGrants.contains {
                $0.id != project.id
                    && SecurityScopedBookmark.folder($0.folderPath, covers: leftover.folderPath)
            }
        }
    }

    private func appendCoveringGrant(at index: Int, origin: PinnedOriginStamp) {
        guard let path = origin.originFolderPath, !ProjectPath.isRemote(path),
              let id = origin.originProjectID ?? origin.coveringGrantID
        else { return }
        if let destPath = pinnedRecords[index].originFolderPath,
           SecurityScopedBookmark.folder(destPath, covers: path)
        {
            return
        }
        appendCoveringGrant(
            ...
            id: id,
            folderPath: path,
            bookmark: origin.originFolderBookmark
        )
    }

    /// Split/grid inherit live cwd. A pinned pane that cd'd outside the origin
    /// must persist (and start or queue) a covering grant this session.
    func coverUncoveredPinnedPaneCwdsIfNeeded(tab: TerminalTab, projectID: UUID) {
        guard projectID == PinnedTabs.projectID else { return }
        persistCoveringGrantsForUncoveredLocalPanes(tabID: tab.id)
    }

    func persistCoveringGrantsForUncoveredLocalPanes(
        tabID: UUID,
        extraProjects: [Project] = []
    ) {
        guard let i = pinnedRecords.firstIndex(where: { $0.id == tabID }),
              let tab = pinnedWorkspace?.tabs.first(where: { $0.id == tabID })
        else { return }
        let originPath = pinnedRecords[i].originFolderPath
            ?? pinnedRecords[i].originProjectID.flatMap { originFolderPath(for: $0) }
        for pane in tab.splitRoot.allPanes() where !pane.isRemote {
            persistCoveringGrantIfUncovered(
                at: i,
                folderPath: pane.projectPath,
    ...
    private func persistCoveringGrantIfUncovered(
        at index: Int,
        folderPath: String,
        originPath: String?,
        extraProjects: [Project]
    ) {
        guard !ProjectPath.isRemote(folderPath) else { return }
        if let originPath, !ProjectPath.isRemote(originPath),
           SecurityScopedBookmark.folder(originPath, covers: folderPath)
        {
            return
        }
        if pinnedRecords[index].coveringGrants.contains(where: {
            SecurityScopedBookmark.folder($0.folderPath, covers: folderPath)
        }) {
            return
        }
        if let source = coveringFolderSource(forLocalPath: folderPath, extraProjects: extraProjects) {
            appendCoveringGrant(
                at: index,
                id: source.id,
                folderPath: source.folderPath,
                bookmark: source.folderBookmark
            )
            return
        }
        appendCoveringGrant(
    ...
    /// First-id-wins only when that id already covers `folderPath` *and* the
    /// leftover still covers the existing folder (same tree). Dest-home
    /// leftovers and origin-shrink leftovers both prefer `record.id`; mint a
    /// unique leftover id when that slot holds a disjoint folder, or when
    /// filling a nil dest-home bookmark with a narrower leftover would drop
    /// spawn for the wider dest cwd.
    private func appendCoveringGrant(
        at index: Int,
        id: UUID,
        folderPath: String,
        bookmark: Data?
    ) {
        if let existing = pinnedRecords[index].coveringGrants.firstIndex(where: { $0.id == id }) {
            let existingPath = pinnedRecords[index].coveringGrants[existing].folderPath
            if SecurityScopedBookmark.folder(existingPath, covers: folderPath),
               SecurityScopedBookmark.folder(folderPath, covers: existingPath)
            {
                if pinnedRecords[index].coveringGrants[existing].folderBookmark == nil {
                    pinnedRecords[index].coveringGrants[existing].folderBookmark = bookmark
                }
                return
            }
            pinnedRecords[index].coveringGrants.append(
                PinnedCoveringGrant(id: UUID(), folderPath: folderPath, folderBookmark: bookmark)
            )
            return
        }
        pinnedRecords[index].coveringGrants.append(
            PinnedCoveringGrant(id: id, folderPath: folderPath, folderBookmark: bookmark)
        )
    }
