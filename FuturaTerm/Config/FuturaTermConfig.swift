...
private let logger = Logger(subsystem: appBundleID, category: "FuturaTermConfig")

/// Generates the two ghostty config files FuturaTerm wraps around the user's
/// own Ghostty config. The user is normally the source of truth for every
/// Ghostty setting; FuturaTerm provides first-launch defaults that the user
/// overrides, and a minimal must-win overrides file for keys FuturaTerm needs to
/// control. That includes background opacity/blur, compatibility fixes, and
/// the terminal background only when the user explicitly selects FuturaTerm's
...
///
/// See the README for the full list of Ghostty config settings FuturaTerm honors
...
@MainActor
final class FuturaTermConfig {
    static let shared = FuturaTermConfig()
    ...
    private init() {
        ...
        defaultsURL = dir.appendingPathComponent("futuraterm-defaults.conf")
        overridesURL = dir.appendingPathComponent("futuraterm-overrides.conf")
        regenerate()
    }
    ...
    /// on launch and whenever FuturaTerm-side state changes that's reflected in
    ...
    /// The full text of `futuraterm-overrides.conf`. Pure — live inputs are
    ...
    static func overridesBody(
        ...
        var overrides = [
            // FuturaTerm composites window translucency at the AppKit level —
        ...
        // FuturaTerm serves natively via the bundled `ghostty` shim, which
        // relays to `futuraterm ssh` (see SSHWrapper). Point GHOSTTY_BIN_DIR at
    ...
    /// the "Bundle futuraterm CLI" post-build step), or nil when the shim isn't
