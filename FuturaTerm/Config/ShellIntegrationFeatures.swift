...
/// Composes the `shell-integration-features` line FuturaTerm writes into
/// `futuraterm-overrides.conf` so it *merges with* the user's own value instead
...
/// the selected user Ghostty config. So FuturaTerm re-emits the user's effective
...
enum ShellIntegrationFeatures {
    ...
    /// `futuraterm-overrides.conf`: the overrides re-emit this key to force
    /// plumbing flags (`no-path` — see `FuturaTermConfig.regenerate`), and a
    /// consumer of an intent flag like `ssh-terminfo` must never be coupled to
    /// whatever plumbing FuturaTerm happens to force alongside it.
    ...
    /// futuraterm-overrides.conf, or nil when no override line is needed.
    /// `disabled` are the `no-*` flags FuturaTerm must force (see
    /// `FuturaTermConfig.regenerate`).
