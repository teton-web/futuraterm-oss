...
/// owns which projects exist, and `~/.config/futuraterm/projects/*.yaml` owns their
...
struct ProjectsSettings: View {
        ...
        Form {
            ...
            Section("Layouts") {
                ...
                LabeledContent("Folder") {
                    ...
                    .help(ChromeHelp.revealLayoutsFolder)
                }
    ...
    /// Folder picker → find-or-select, the same as the sidebar's "Local Folder…".
    /// A directory that already backs a sidebar project is selected, not duplicated.
    ...
    /// path. Drives the muted "Create Project" affordance: invoking it on a
    /// matching path selects the existing project (CLI / palette Add another
    /// remain the duplicate-create paths).
    ...
    /// Add or select a project for this file's declaration. Selecting is
    ...
    private func createProject(from layout: ProjectFileStore.Listing) {
        ...
        // which is the slug of a name FuturaTerm itself wrote.
        ...
        appState.addOrSelectProject(store: projectStore, name: name, path: path)
    }
...
private struct ProjectRow: View {
    ...
    private var pathCaption: String {
        guard let groupID = projectStore.resolvedGroupID(project.groupID) else {
            return project.path
        }
        let names = projectStore.folderPathNames(for: groupID)
        guard !names.isEmpty else { return project.path }
        return (names + [project.path]).joined(separator: " · ")
    }
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            .menuIndicator(.hidden)
            .fixedSize()
            .help(ChromeHelp.projectRowMenu)
        }
        .padding(.vertical, 2)
    }
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: layout.isInvalid ? "exclamationmark.triangle" : "doc.text")
                .foregroundStyle(layout.isInvalid ? AnyShapeStyle(FuturaTermTheme.warning) : AnyShapeStyle(.secondary))
                .frame(width: 16)
