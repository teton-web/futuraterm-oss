...
private enum SettingsPane: String, CaseIterable, Identifiable {
    ...
    /// MAS has no Sparkle; keep `.updates` in `allCases` so CaseIterable
    /// tests stay intact, but omit it from the sidebar.
    static var sidebarPanes: [SettingsPane] {
...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationSplitView(columnVisibility: .constant(.all)) {
            List(SettingsPane.sidebarPanes, selection: $selection) { pane in
...
private struct GeneralSettings: View {
    ...
    @State private var startGrokInNewTabs: Bool = Preferences.shared.startGrokInNewTabs
    ...
    /// is dropping what FuturaTerm posts. Both halves change only outside this
    ...
    var body: some View {
        Form {
            if hasFullDiskAccess == false, !AppDistribution.isAppStore {
            ...
            Section("New Tabs") {
                Toggle("Start Grok in new tabs", isOn: $startGrokInNewTabs)
                    .onChange(of: startGrokInNewTabs) { _, v in
                        Preferences.shared.startGrokInNewTabs = v
                    }
                Text("Types grok into each new tab's shell.")
                    .settingsCaption()
            }
            ...
            Section("Remote Projects") {
                ...
                Text(
                    "Probes remote hosts for live tab names and close warnings, "
                        + "and auto-pairs View Desktop over that same SSH. "
            ...
            if ZmxClient.live.executableURL() == nil {
                Section("Session Persistence") {
                    ...
                    .font(.system(size: 11))
                    .foregroundStyle(FuturaTermTheme.warning)
                }
            }
        }
        .formStyle(.grouped)
    ...
    private func refreshNotificationBanner() async {
        guard NotificationConfigIntent.wantsNotifications(
            inConfigText: FuturaTermConfig.userGhosttyConfigText()
        )
        else {
        ...
        ForEach(customGhosttyConfigPaths.indices, id: \.self) { index in
            if editingCustomGhosttyConfigIndex == index {
                ...
                GhosttyConfigFileRow(
                    ...
                    onGrantAccess: GhosttyConfigFileBookmarks.needsRepick(customGhosttyConfigPaths[index])
                        ? { grantAccessToCustomGhosttyConfig(at: index) }
                        : nil,
    ...
    private func customGhosttyConfigProblem(_ path: String) -> String? {
        if GhosttyConfigFileBookmarks.needsRepick(path) {
            return "FuturaTerm needs access to this file again."
        }
    ...
    private func grantAccessToCustomGhosttyConfig(at index: Int) {
        guard finishPendingCustomGhosttyConfigEdit() else { return }
        guard customGhosttyConfigPaths.indices.contains(index) else { return }
        if GhosttyConfigFileBookmarks.presentRepick(for: customGhosttyConfigPaths[index]) {
            commitGhosttyConfig()
        }
    }
    ...
    private func commitCustomGhosttyConfigEdit() {
        ...
        if customGhosttyConfigPaths != before {
            for path in before where !customGhosttyConfigPaths.contains(path) {
                GhosttyConfigFileBookmarks.stopAccessing(path: path)
            }
            commitGhosttyConfig()
        }
    }
    ...
    private func removeCustomGhosttyConfigPath(at index: Int) {
        guard finishPendingCustomGhosttyConfigEdit() else { return }
        guard customGhosttyConfigPaths.indices.contains(index) else { return }
        let removed = customGhosttyConfigPaths.remove(at: index)
        GhosttyConfigFileBookmarks.stopAccessing(path: removed)
        commitGhosttyConfig()
    }
    ...
    private func browseForCustomGhosttyConfigFiles() {
        ...
        for url in panel.urls {
            if let data = SecurityScopedBookmark.create(from: url) {
                GhosttyConfigFileBookmarks.remember(path: url.path(percentEncoded: false), data: data)
                granted.append(url)
            } else if SecurityScopedBookmark.usesSecurityScope {
                let alert = NSAlert()
                alert.messageText = "Couldn’t save access"
                alert.informativeText = "FuturaTerm couldn’t store access to “\(url.lastPathComponent)”. Pick the file again."
        ...
        addCustomGhosttyConfigPaths(granted.map { $0.path(percentEncoded: false) })
    }
    ...
    var body: some View {
        HStack(spacing: 8) {
            if let problem {
                ...
                // for a broken path is fixing it (or re-granting access).
                Button(action: onGrantAccess ?? onEdit) {
                    Image(systemName: "exclamationmark.triangle")
                        .foregroundStyle(FuturaTermTheme.warning)
                        .frame(width: 16)
                }
            ...
            Menu {
                ...
                if let onGrantAccess {
                    Button("Grant Access…") { onGrantAccess() }
                }
            ...
            .menuIndicator(.hidden)
            .fixedSize()
            .help(ChromeHelp.ghosttyConfigRowMenu)
        }
        .padding(.vertical, 2)
    }
}
            ...
            Label {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    if let url = FullDiskAccess.settingsURL {
                        ...
                        .help(ChromeHelp.openFullDiskAccess)
                    }
                }
            } icon: {
...
/// cannot re-raise it, and a denied FuturaTerm is silent with nothing on screen to
...
private struct NotificationPermissionBanner: View {
    var body: some View {
        Section {
            Label {
                VStack(alignment: .leading, spacing: 4) {
                    Text("FuturaTerm doesn't have notification permission")
                        .font(.system(size: 13, weight: .semibold))
                    ...
                    if let url = NotificationHandler.settingsURL {
                        ...
                        .help(ChromeHelp.openNotificationSettings)
                    }
                }
            } icon: {
                Image(systemName: "exclamationmark.triangle.fill")
...
private struct AppearanceSettings: View {
    ...
    @State private var showAgentUsage: Bool = Preferences.shared.showAgentUsage
    ...
    var body: some View {
        Form {
            ...
            Section("Sidebar") {
                ...
                Toggle("Show agent usage", isOn: $showAgentUsage)
                    .onChange(of: showAgentUsage) { _, v in
                        Preferences.shared.showAgentUsage = v
                        Task { await agentUsage.noteUsagePreferenceChanged() }
                    }
                Text("Shows remaining quota for signed-in coding agents in the account panel.")
                    .settingsCaption()
    ...
    var body: some View {
        Form {
            Section("Quick Terminal") {
                ...
                Text("Works globally, even when FuturaTerm isn't active. Rebind it in Keymaps, or clear it to disable the quick terminal.")
                    .settingsCaption()
            }
...
private struct UpdatesSettings: View {
    ...
    /// Sparkle is not started in Debug or in a local Release that still has
    /// the placeholder EdDSA key, so `canCheckForUpdates` stays false. Distinct
    /// from an in-flight check on a real release, which also disables the
    /// button but should not show this caption.
    private var isLocalUpdaterBuild: Bool {
        let isDebug: Bool = {
            #if DEBUG
            true
            #else
        ...
        return !UpdaterAvailability.shouldStart(
            isDebug: isDebug,
            isBenchmark: false,
            publicKey: publicKey
        )
        ...
        Form {
            Section("Automatic Updates") {
                ...
                HStack {
                    ...
                    .help(AppCommand.checkForUpdate.help)
                }

                #if APP_STORE
                Text(UpdaterAvailability.appStoreUpdatesCaption)
                    .settingsCaption()
                #else
                if isLocalUpdaterBuild {
                    Text(UpdaterAvailability.localBuildUpdatesCaption)
                        .settingsCaption()
                }
                #endif
            ...
            Section("Version") {
                LabeledContent("Current version", value: AppVersion.displayString())
            }
        }
        .formStyle(.grouped)
    }
