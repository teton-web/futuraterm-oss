...
struct Project: Identifiable, Codable, Hashable {
    ...
    var zmxPath: String?
    /// Sidebar folder this project belongs to (`ProjectGroup.id`). nil =
    /// ungrouped. Decodes as nil from older projects.json (absent key).
    var groupID: UUID?
    /// Optional security-scoped bookmark for a local folder chosen via
    /// NSOpenPanel (MAS sandbox). `path` stays identity; this is extra
    /// capability. Nil for remotes, CLI `project create`, and older JSON.
    ...
    init(
        ...
        createdAt = Date()
        self.securityScopedBookmark = ProjectPath.isRemote(path) ? nil : securityScopedBookmark
    }
