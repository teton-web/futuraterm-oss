...
final class Pane: Identifiable {
    ...
    let sessionID: UUID
    /// The pane's zmx session name (`futuraterm-<slug>-<hex>`), fixed at creation
    ...
    /// Count of descendant AI-agent processes under this pane's foreground
    /// (excluding the foreground pid itself). 0 when the foreground is not an
    /// agent, the pane is remote, or the walk failed. Updated on the
    /// foreground poll; not persisted.
    private(set) var nestedAgentCount = 0
    ...
    func refreshForegroundProcess(trackExecution: Bool? = nil) {
        ...
        applyForegroundRefresh(
            ...
            nestedChildrenOf: ProcessInspector.childPIDs,
            nestedCommOf: ProcessInspector.comm
        )
    }
    ...
    ///
    /// This gates the name and icon. A leftover nested-agent count is fail-closed
    /// to 0 (not walked) so a prompt-hook sample cannot resurrect execution after
    /// OSC 133;D. `programTitle` expiry stays keyed on the foreground pid, so a
    /// title can never be misattributed. A long-lived program (claude, hx)
    /// reports no completion while it runs, so the gate is never armed underneath
    /// it, and a shell with no OSC 133 integration never arms it at all — those
    /// panes keep exactly today's naming behavior.
    ...
    func applyForegroundRefresh(
        ...
        argv0: () -> String? = { nil },
        nestedChildrenOf: ((pid_t) -> [pid_t])? = nil,
        nestedCommOf: ((pid_t) -> String?)? = nil
    ) {
        ...
        if !ignoresForegroundIdentity(name) {
            ...
            // Nested-agent walk is gated on the cached icon (already-recognized
            // agent foreground) so shells never pay for a descendant scan.
            publishNestedAgentCount(
                pid: foregroundPID,
                childrenOf: nestedChildrenOf,
                commOf: nestedCommOf
            )
        } else if nestedAgentCount != 0 {
            // Ignored prompt-hook samples must not feed a leftover nested
            // count into execution. Do not walk the hook pid.
            nestedAgentCount = 0
        }
    ...
    /// Walk descendants only while this pane's foreground is a recognized
    /// agent. Remote panes and missing pids fail closed to 0. Republish only
    /// when the count changes, matching the other poll fields.
    private func publishNestedAgentCount(
        pid: pid_t?,
        childrenOf: ((pid_t) -> [pid_t])?,
        commOf: ((pid_t) -> String?)?
    ) {
        guard !isRemote, agentIcon != nil, let pid else {
            if nestedAgentCount != 0 { nestedAgentCount = 0 }
            return
        }
        // The testable core never Darwin-walks a caller-supplied pid (tests
        // use fake values like 1). Production `refreshForegroundProcess`
        // always passes `childPIDs` / `comm`.
        guard let childrenOf else { return }
        let count = ProcessInspector.nestedAgentCount(
            rootPID: pid,
            childrenOf: childrenOf,
            commOf: commOf ?? ProcessInspector.comm
        )
        if count != nestedAgentCount { nestedAgentCount = count }
    }
    ...
    private func applyForegroundExecutionState(
        ...
        executionState = executionTracker.refreshNestedAgents(
            count: nestedAgentCount,
            currentState: executionState
        )
        cancelActivityQuietPollIfNeeded()
    }
    ...
    @ObservationIgnored
    private(set) var hasBuiltSurface = false

    /// After a MAS folder grant: the next `ensureNSView` is a first spawn
    /// (`command` / cwd) instead of a zmx reattach of a denied-cwd session.
    func resetSurfaceForFirstSpawn() {
    ...
    func ensureNSView() -> GhosttyTerminalNSView {
        ...
        // `futuraterm` invoked inside it can self-target (`FUTURATERM_SESSION`).
