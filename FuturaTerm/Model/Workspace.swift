...
final class TerminalTab: Identifiable {
    ...
    /// True when any pane has nested AI-agent children. Same OR as
    /// `executionState`: one nested pane tints the whole tab's spinner.
    var hasNestedAgents: Bool {
        splitRoot.allPanes().contains { $0.nestedAgentCount > 0 }
    }
...
final class Workspace: Identifiable {
    ...
    /// Adopt an existing tab — moved in from another workspace. Unlike
    /// `createTab` the `TerminalTab` (and its live panes/surfaces) is reused
    /// as-is; the caller is responsible for having removed it from its previous
    /// workspace first. `index` positions the tab in the list (a sidebar drop
    /// lands at a specific slot); nil / out-of-range appends. `activate`
    /// selects the tab (default); pass false to append without stealing focus
    /// unless the workspace had no selection.
    func adoptTab(_ tab: TerminalTab, at index: Int? = nil, activate: Bool = true) {
