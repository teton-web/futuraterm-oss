...
/// Which ProgressView tint the sidebar running glyph uses.
/// Nested agents reuse Copilot's brand purple (the existing vendor-identity
/// exception); a lone running process stays `.secondary`. Idle and done have
/// no spinner.
enum TabStatusSpinnerTint: Equatable {
    case none
    case secondary
    case nestedAgents

    static func resolve(running: Bool, nested: Bool) -> Self {
        guard running else { return .none }
        return nested ? .nestedAgents : .secondary
    }
}
