...
enum TerminalExecutionState: Equatable {
    ...
    /// Same reduce as `TerminalTab.executionState`: running > done > idle.
    /// An empty sequence is idle.
    static func combined(_ states: some Sequence<TerminalExecutionState>) -> TerminalExecutionState {
        var sawDone = false
        for state in states {
            switch state {
            case .running:
...
private enum TerminalExecutionSource: Equatable {
    ...
    case activity(Date)
    case progress
    /// Nested AI-agent children. Holds `.running` until the count returns to 0;
    /// unlike activity, this source does not quiet-settle.
    case nestedAgents
}
...
struct TerminalExecutionTracker {
    ...
    /// Why the pane is currently considered running. Foreground, explicit
    /// progress, and nested agents run until a completion/foreground
    /// transition (nested until the child count hits 0); activity is an
    ...
    mutating func markTerminalActivity(
        ...
        // progress or nested-agent ownership, or demote a canonical foreground
        // command into a run that quiet-settles while its process is still alive.
    ...
    mutating func settleIfQuiet(
        ...
        guard currentState == .running, runningSource != .nestedAgents else { return currentState }
        guard case let .activity(lastActivityAt) = runningSource,
    ...
    /// Hold `.running` while nested AI-agent children exist. Nested ownership
    /// does not quiet-settle; dropping to 0 completes only when nested was the
    /// sole owner (progress/foreground keep their own run).
    mutating func refreshNestedAgents(
        count: Int,
        currentState: TerminalExecutionState
    ) -> TerminalExecutionState {
        if count > 0 {
            ...
            case .activity,
                 .nestedAgents,
                 nil:
                runningSource = .nestedAgents
            }
            return .running
        }
        guard runningSource == .nestedAgents else { return currentState }
        runningSource = nil
        return currentState == .running ? .done : currentState
    }
    ...
    mutating func refreshForeground(
        ...
        if terminalInputIsRaw {
            ...
            // Nested ownership is a separate refresh after this returns, so
            // an idle grok with children can still go `.running`.
