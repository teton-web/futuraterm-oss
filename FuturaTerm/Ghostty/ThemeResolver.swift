...
/// Resolves ghostty's `theme = light:X,dark:Y` split syntax for FuturaTerm's own
/// chrome (window, sidebar, palette — everything driven by `FuturaTermTheme`).
///
...
/// chrome — which derives from those getters — FuturaTerm has to pick the side
...
enum ThemeResolver {
    ...
    /// The subset of theme-file colors FuturaTerm's chrome needs.
