...
final class GhosttyCallbacks: @unchecked Sendable {
    ...
    func action(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        case GHOSTTY_ACTION_RING_BELL:
            // BEL. The `title`/`border` bell-features are per-tab UI FuturaTerm
        ...
        case GHOSTTY_ACTION_OPEN_CONFIG:
            // The `open_config` keybind. FuturaTerm's source of truth is the
        ...
        case GHOSTTY_ACTION_PROMPT_TITLE:
            // `prompt_surface_title`. FuturaTerm titles live on tabs (a surface
        ...
        case GHOSTTY_ACTION_SHOW_CHILD_EXITED:
            ...
            // FuturaTerm relies on. The report is NOT used to classify the exit:
        ...
        case GHOSTTY_ACTION_SELECTION_CHANGED:
            // Selection is mouse-driven in libghostty (no set-selection C API).
            // Consumed: there is no core overlay. AX text APIs still read
            // through the existing `read_selection` / screen-contents cache.
            guard let view = surfaceView(from: target) else { return true }
            DispatchQueue.main.async { view.surfaceDidChangeSelection() }
            return true
        default:
            // Deliberately unhandled: window/tab/split management actions
            // (NEW_TAB, NEW_SPLIT, GOTO_*, TOGGLE_FULLSCREEN, QUIT, …) —
            // those concepts are FuturaTerm-owned via AppCommand/hotkeys, not
            ...
            // the grid core-side; FuturaTerm's chrome follows the resolved
            ...
            // abnormal-exit overlay, which is the error UI FuturaTerm relies on.
            return false
        }
    }
    ...
    /// Serve a clipboard read. FuturaTerm resolves every read to TEXT (file URLs
    ...
    static func imagePasteboardPath(_ pb: NSPasteboard) -> String? {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("futuraterm-paste", isDirectory: true)
    ...
    /// (e.g. OSC 52 with `clipboard-read = ask`). FuturaTerm's long-standing
