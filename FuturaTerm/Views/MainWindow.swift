...
struct MainWindow: View {
    ...
    /// Shared with `SidebarContent` so New Folder still presents the
    /// List-level alert (a Menu-hosted alert is swallowed when the item
    /// tears down). The toolbar itself lives on `SidebarContent`.
    @State
    ...
    var body: some View {
        ...
        return NavigationSplitView(columnVisibility: $columnVisibility) {
            SidebarContent(
                ...
                isNewFolderAlertPresented: $isNewFolderAlertPresented,
                newFolderName: $newFolderName
            )
            // Unplaced item on the sidebar column — not `.navigation`,
            // which the detail tab switcher uses when set to Left (#186).
            // New Folder lives on `SidebarContent`'s own toolbar so it
            // always goes through `beginNewFolder()`. Not on the detail
            // `.toolbar` below.
        ...
        .sheet(isPresented: $appState.isSessionManagerPresented) {
            SessionManagerSheet()
        }
        .sheet(isPresented: $appState.isEnvironmentManagerPresented) {
        ...
        .onChange(of: appState.pendingSecurityScopeRegrantID) { _, id in
            if let id {
                appState.presentSecurityScopeRegrant(store: projectStore, projectID: id)
            }
        }
        .task {
            var paths: [String] = []
            while let path = GhosttyConfigFileBookmarks.nextPendingRepick() {
                paths.append(path)
            }
            if !paths.isEmpty {
                _ = GhosttyApp.shared.repickConfigFilesAndReload(paths)
            }
        }
        ...
        Button(action: onExit) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .fill(FuturaTermTheme.surface)
                    .overlay(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .stroke(FuturaTermTheme.border, lineWidth: 1)
                    )
            )
...
private struct WindowStyler: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSWindowDelegate {
        ...
        func observe(window: NSWindow) {
            ...
            // .futuraTermConfigDidChange so the window tint follows the resolved
            // theme (issue #38).
            observer = NotificationCenter.default.addObserver(
                forName: .futuraTermConfigDidChange,
                object: nil,
                queue: .main
            ) { [weak window] _ in
