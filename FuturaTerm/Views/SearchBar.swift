    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 6) {
                ...
                .padding(.vertical, 4)
                .background(FuturaTermTheme.surface)
                .clipShape(RoundedRectangle(cornerRadius: 6))
                .overlay(RoundedRectangle(cornerRadius: 6).strokeBorder(FuturaTermTheme.border, lineWidth: 1))

                Button(action: onNavigateNext) {
                    Image(systemName: "chevron.up").font(.system(size: 10, weight: .semibold))
                }
                .buttonStyle(SearchButtonStyle())
                .help(ChromeHelp.searchOlder)
                Button(action: onNavigatePrevious) {
                    Image(systemName: "chevron.down").font(.system(size: 10, weight: .semibold))
                }
                .buttonStyle(SearchButtonStyle())
                .help(ChromeHelp.searchNewer)
                Button(action: onClose) {
                    Image(systemName: "xmark").font(.system(size: 10, weight: .semibold))
                }
                .buttonStyle(SearchButtonStyle())
                .help(ChromeHelp.closeSearch)
            }
            .padding(.horizontal, 8)
            .frame(height: 32)
            .background(Color.clear)
            Rectangle().fill(FuturaTermTheme.border).frame(height: 1)
        }
...
private struct SearchButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(FuturaTermTheme.fgMuted)
            .background(configuration.isPressed ? FuturaTermTheme.surface : .clear)
