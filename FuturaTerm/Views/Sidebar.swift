...
struct SidebarContent: View {
    ...
    var body: some View {
        List(selection: $selection) {
            ...
            // order is parent header + that folder's *direct* projects, then
            ...
            ForEach(folderRows) { row in
                ...
                .id("\(row.group.id.uuidString)-\(expandedFolders.contains(row.group.id))")
            }
    ...
    private func folderHeader(_ group: ProjectGroup, depth: Int) -> some View {
        ...
        // ONE drop destination — widen `SidebarDropItem` instead of stacking.
        .dropDestination(for: SidebarDropItem.self) { items, _ in
            for item in items {
                switch item {
                case let .project(dragged):
                    receiveProjectDropOntoFolder(dragged, group: group)
                case let .folder(dragged):
                    receiveFolderDropOntoFolder(dragged, group: group)
                case .tab,
                     .pane:
                    break
                }
    ...
    private func projectHeader(index projectIndex: Int, project: Project) -> some View {
        ...
        .dropDestination(for: SidebarDropItem.self) { items, _ in
            for item in items {
                ...
                case let .folder(dragged):
                    receiveFolderDropOntoProject(dragged, project: project)
                case let .pane(pane):
    ...
    /// Reparent the dragged folder under `group`. `setParent` no-ops on
    /// cycles and depth overflow; still `return true` from the destination
    /// so SwiftUI does not hang the drag.
    private func receiveFolderDropOntoFolder(_ item: MovableFolder, group: ProjectGroup) {
        ...
        projectStore.setParent(groupID: item.groupID, parentID: group.id)
        if resolvedParent(of: item.groupID) == group.id, before != group.id {
            expandedFolders.insert(group.id)
        }
    }
