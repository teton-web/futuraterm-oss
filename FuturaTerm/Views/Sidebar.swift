...
struct SidebarContent: View {
    ...
    @State
    private var expandedFolders: Set<UUID> = Preferences.shared.expandedSidebarFolderIDs
    @State
    private var isNewFolderAlertPresented = false
    @State
    ...
    private var showUsageFooter: Bool {
        showAgentUsage && !agentUsage.snapshots.isEmpty
    }
    ...
    var body: some View {
        List(selection: $selection) {
            ...
            // Folders as `Section`s, not nested `DisclosureGroup`s: a folder
            // wrapping project disclosures is the outline-crash class the
            // pinned Section already avoids. Collapse is our own header
            // chevron + `expandedFolders` (UserDefaults).
            ForEach(projectStore.groups) { group in
                Section {
                    if expandedFolders.contains(group.id) {
                        ForEach(Array(projectStore.projects(in: group.id).enumerated()), id: \.element.id) { projectIndex, project in
        ...
        .sidebarSafeAreaBar(isPresented: showNewProjectButton || showUsageFooter) {
            VStack(alignment: .leading, spacing: 0) {
                if showUsageFooter {
                    VStack(alignment: .leading, spacing: 8) {
                        ForEach(agentUsage.snapshots) { snapshot in
                            AgentUsageFooterRow(snapshot: snapshot)
                        }
                    }
                    .padding(.horizontal, 16)
                    .padding(.top, 36)
                    .padding(.bottom, showNewProjectButton ? 0 : 20)
                }
                if showNewProjectButton {
                    ...
                    .padding(.horizontal, 16)
                    .padding(.top, showUsageFooter ? 8 : 36)
        ...
        .onAppear {
            expandedFolders = Preferences.shared.expandedSidebarFolderIDs
            expandActiveProject(appState.activeProjectID)
            syncSelection()
        }
        .onChange(of: expandedFolders) { _, ids in
            Preferences.shared.expandedSidebarFolderIDs = ids
        }
    ...
    private func folderHeader(_ group: ProjectGroup) -> some View {
        SidebarFolderRow(group: group, isExpanded: expandedFolders.contains(group.id)) {
            projectStore.renameGroup(id: group.id, to: $0)
        }
        .padding(.trailing, rowTrailingInset)
        .frame(maxWidth: .infinity, alignment: .leading)
        .contentShape(Rectangle())
    ...
    private func expandActiveProject(_ projectID: UUID?) {
        guard let projectID, projectID != PinnedTabs.projectID else { return }
        expandedProjects.insert(projectID)
        if let project = projectStore.projects.first(where: { $0.id == projectID }),
           let folderID = projectStore.resolvedGroupID(project.groupID)
        {
    ...
    private func projectMenu(_ project: Project) -> some View {
        ...
        Menu("Move to Folder") {
            ...
            if !projectStore.groups.isEmpty {
                Divider()
                ForEach(projectStore.groups) { group in
                    Button(group.name) {
        ...
        Button("Unload Project") { appState.requestUnloadProject(project.id) }
            ...
            .help(ProjectActionHelp.unload)
        Button("Remove Project", role: .destructive) {
            appState.requestRemoveProject(project.id) { removeProject(project) }
        }
        .help(ProjectActionHelp.remove)
    }

    @ViewBuilder
    private func folderMenu(_ group: ProjectGroup) -> some View {
        Button("Rename Folder") { appState.renamingFolderID = group.id }
        let index = projectStore.groups.firstIndex(where: { $0.id == group.id }) ?? 0
        Button("Move Up") {
            projectStore.reorderGroups(fromOffsets: [index], toOffset: index - 1)
        }
        .disabled(index <= 0)
        Button("Move Down") {
            projectStore.reorderGroups(fromOffsets: [index], toOffset: index + 2)
        }
        .disabled(index >= projectStore.groups.count - 1)
        Divider()
    ...
    private func bulkMenu(for items: Set<SidebarItem>) -> some View {
        ...
        if folderCount == items.count {
            Button("Delete \(folderCount) Folders") { removeSelection(items) }
    ...
    private func commitNewFolder() {
        ...
        newFolderName = ""
        if let group = projectStore.createGroup(name: name) {
            expandedFolders.insert(group.id)
        }
    }
}
