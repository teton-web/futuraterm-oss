...
struct TerminalPane: View {
    ...
    /// Optional: the quick terminal has no AppState and is not folder-gated.
    @Environment(AppState.self) private var appState: AppState?

    /// MAS local panes wait for the project-folder grant (focused tab, splits,
    /// and new tabs all create surfaces here — not only the incubator).
    private var canSpawnSurface: Bool {
        appState?.shouldSpawnPane(pane, workspaceID: pane.projectID) ?? true
    }
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            TerminalSurface(
                ...
                canSpawn: canSpawnSurface,
                folderGrantEpoch: appState?.folderGrantEpoch ?? 0,
                onFocus: onFocus,
...
private struct LinkHoverBanner: View {
    ...
    var body: some View {
        Text(url)
            ...
            .padding(.vertical, 3)
            .background(FuturaTermTheme.bg, in: RoundedRectangle(cornerRadius: 6))
            .overlay(RoundedRectangle(cornerRadius: 6).strokeBorder(FuturaTermTheme.border))
...
private struct TerminalSurface: NSViewRepresentable {
    ...
    /// False until the MAS project-folder grant is held; skips `ensureNSView`
    /// / `createSurface` so the focused tab cannot spawn before the Open panel.
    let canSpawn: Bool
    let folderGrantEpoch: Int
    let onFocus: () -> Void
    let onProcessExit: () -> Void
    let onCommandFinished: () -> Void
    let onAdaptiveBackgroundChange: (CGColor?) -> Void
    ...
    func makeNSView(context: Context) -> SurfaceHost {
        ...
        guard canSpawn else {
            // Stay unfocused until a surface exists. Recording `focused`
            // here makes the post-grant `updateNSView` see `focused &&
            // wasFocused` and skip `notifySurfaceFocused` / restoreFocus.
            return host
        }
    ...
    func updateNSView(_ host: SurfaceHost, context: Context) {
        ...
        // added to the window after first makeNSView, or a folder grant just
        // flipped `canSpawn`).
        if view.surface == nil, view.window != nil {
            view.createSurface()
        }
    ...
    private func configure(_ view: GhosttyTerminalNSView) {
        ...
        view.accessibilityTitleOverride = pane.displayTitle
