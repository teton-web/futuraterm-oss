...
extension UTType {
    ...
    static let futuraTermPaneID = UTType(exportedAs: "com.davidsolheim.futuraterm.pane-id")
}

extension NSPasteboard.PasteboardType {
    static let futuraTermPaneID = NSPasteboard.PasteboardType(UTType.futuraTermPaneID.identifier)
}

/// The grab-handle drag as seen by its drop targets: the pane UUID
/// `PaneDragSource` writes as 16 raw bytes under `.futuraTermPaneID`.
///
...
struct MovablePane {
    ...
    static func fromDragPasteboard() -> MovablePane? {
        guard let data = NSPasteboard(name: .drag).pasteboardItems?
            .compactMap({ $0.data(forType: .futuraTermPaneID) })
            .first
        else { return nil }
                ...
                ZStack {
                    ...
                    if inRevealBand || isHovering || isDragging {
                        Image(systemName: "ellipsis")
                            ...
                            .foregroundStyle(FuturaTermTheme.fg.opacity(isHovering || isDragging ? 0.8 : 0.35))
...
private struct PaneDragSource: NSViewRepresentable {
    ...
    final class DragSourceView: NSView, NSDraggingSource {
        ...
        override func mouseDragged(with event: NSEvent) {
            ...
            pasteboardItem.setData(MovablePane.payload(for: pane.id), forType: .futuraTermPaneID)
...
struct PaneDropContext {
    ...
    var acceptedTypes: [UTType] {
        onMergeTab == nil ? [.futuraTermPaneID] : [.futuraTermPaneID, .futuraTermTab]
    }
}
        ...
        GeometryReader { geo in
            if let resolution {
                RoundedRectangle(cornerRadius: 4)
                    .fill(FuturaTermTheme.accent.opacity(0.3))
                    .frame(
...
struct LeafDropDelegate: DropDelegate {
    ...
    func performDrop(info: DropInfo) -> Bool {
        ...
        guard let provider = info.itemProviders(for: [.futuraTermTab]).first else { return false }
        let merge = onMergeTab
        provider.loadDataRepresentation(forTypeIdentifier: UTType.futuraTermTab.identifier) { data, _ in
...
extension MovableTab {
    ...
    static func fromDragPasteboard() -> MovableTab? {
        guard let data = NSPasteboard(name: .drag).pasteboardItems?
            .compactMap({ $0.data(forType: NSPasteboard.PasteboardType(UTType.futuraTermTab.identifier)) })
            .first
        else { return nil }
