...
#if APP_STORE
/// MAS: no private CoreGraphics blur. Public translucency is
/// `NSVisualEffectView` / `NSGlassEffectView` (`glassSupported`).
@MainActor
...
/// libghostty exposes the same call. Eager `dlsym` is Direct-only so MAS
/// never references the symbols.
...
/// Ghostty uses. FuturaTerm exposes a full-range opacity slider, and an unscaled
...
/// the setting.
///
/// FuturaTerm inserts this below the window's content view, filling the whole
...
final class FuturaTermGlassView: NSView {
...
/// different incantations (hiding NSVisualEffectView, etc.) — FuturaTerm targets
...
enum WindowAppearance {
    ...
    /// dependable here.
    private static let sidebarAutosaveName = "FuturaTermMainSidebar"
    ...
    //
    // MAS: private selector, left as AppKit's default (no swizzle).
    #if !APP_STORE
    ...
    /// double-paint problem `futuraterm-overrides.conf` sets
    ...
    static func syncKeyStatus(window: NSWindow) {
        ...
        if #available(macOS 26.0, *) {
            ...
            glass.updateKeyStatus(window.isKeyWindow, backgroundColor: FuturaTermTheme.nsBg)
        }
    }
    ...
    private static func syncGlass(window: NSWindow, backgroundColor: NSColor, opacity: Double) {
        ...
        let glass = existingGlass(in: window) ?? {
            let view = FuturaTermGlassView(topOffset: -contentView.safeAreaInsets.top)
    ...
    @available(macOS 26.0, *)
    private static func existingGlass(in window: NSWindow) -> FuturaTermGlassView? {
        guard let themeFrame = window.contentView?.superview else { return nil }
        return themeFrame.subviews.compactMap { $0 as? FuturaTermGlassView }.first
    }
    ...
    /// is unavailable. MAS never reads `_cornerRadius`.
