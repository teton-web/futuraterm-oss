    ...
    var body: some View {
        GeometryReader { geo in
            ZStack(alignment: .top) {
                // Click-outside scrim. Transparent but hit-testable. Hidden
                // from VoiceOver; Escape still dismisses via the panel.
                ...
                CommandPalettePanel()
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: Self.cornerRadius, style: .continuous)
                            .strokeBorder(FuturaTermTheme.border, lineWidth: 1)
                    )
                    .shadow(color: .black.opacity(0.35), radius: 20, x: 0, y: 8)
                    // Modal lives on the floating panel chrome, not the overlay
                    // (the scrim is a sibling) and not a nested inner group.
                    // Applied before the top offset so the AX frame matches
                    // the visible panel, not the empty space above it.
                    .accessibilityElement(children: .contain)
                    .accessibilityLabel(ChromeDialogAccessibility.commandPalette)
        ...
        return VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                Image(systemName: "magnifyingglass")
                    .font(.system(size: 14))
                    .foregroundStyle(FuturaTermTheme.fgMuted)
                    .accessibilityHidden(true)
                TextField(
                    ChromeDialogAccessibility.filter,
                    text: $appState.commandPaletteQuery,
                    prompt: Text(placeholderText)
                )
                .textFieldStyle(.plain)
                .font(.system(size: 14))
                .foregroundStyle(FuturaTermTheme.fg)
                .focused($isFieldFocused)
                .onSubmit { execute() }
                .accessibilityLabel(ChromeDialogAccessibility.filter)
            }
            .padding(.horizontal, 14)
            .padding(.vertical, 12)

            Divider()
                .background(FuturaTermTheme.border)
                .accessibilityHidden(true)
                        ...
                        ForEach(Array(sections.enumerated()), id: \.offset) { sectionIndex, section in
                            if let header = section.header {
                                Text(header)
                                    ...
                                    .foregroundStyle(FuturaTermTheme.fgDim)
                                    .padding(.horizontal, 14)
    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                Text(item.title)
                    .font(.system(size: 13))
                    .foregroundStyle(item.isEnabled ? FuturaTermTheme.fg : FuturaTermTheme.fgDim)
                    .accessibilityHidden(true)
                if let subtitle = item.subtitle {
                    Text(subtitle)
                        .font(.system(size: 11))
                        .foregroundStyle(item.isEnabled ? FuturaTermTheme.fgMuted : FuturaTermTheme.fgDim)
                        .lineLimit(1)
                        .accessibilityHidden(true)
                }
            }
        ...
        .padding(.horizontal, 14)
        .padding(.vertical, 8)
        .background(isSelected ? FuturaTermTheme.fg.opacity(0.12) : .clear)
...
/// VoiceOver name for a palette result. Lives on the row `Button` (the
/// interactive element) so selected-state is not a nested child of the
/// button's default label. Inner title/subtitle/keybind stay hidden.
private struct PaletteRowAX: ViewModifier {
    let title: String
    let subtitle: String?
    let isSelected: Bool
    ...
    func body(content: Content) -> some View {
        ...
        if let value = ChromeDialogAccessibility.paletteRowValue(subtitle) {
            labeled.accessibilityValue(value)
        } else {
            labeled
        }
    }
...
private struct KeyCap: View {
    ...
    var body: some View {
        Text(symbol)
            ...
            .background(FuturaTermTheme.surface, in: .rect(cornerRadius: 5))
            .overlay(
