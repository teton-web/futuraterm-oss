...
enum ControlReadiness {
    ...
    /// Nested CLI layout: `FuturaTerm.app/Contents/Resources/bin/futuraterm`.
    /// Prefer this over LaunchServices so a Debug CLI never starts Release
    /// (different App Support / socket). `nil` if the executable isn't nested.
