...
/// Dispatches decoded control-socket requests into app state. Mutations
/// call the same `AppState` / `ProjectStore` / `ZmxClient` methods the UI
/// uses. `project.open --run` additionally scans workspace panes for a
/// reuse match before `createTab`. Injectable stores make it fully
/// testable with the tempdir pattern used by `AppStateTests`.
@MainActor
final class ControlHandler {
    ...
    private func dispatch(_ request: ControlRequest) async throws -> ControlData {
        ...
        case "project.open": return try projectOpen(args)
        ...
        case "pane.dump": return try await paneDump(args)
        case "pane.choices": return try paneChoices(args)
        case "pane.choose": return try paneChoose(args)
        case "pane.selection": return try paneSelection(args)
        case "pane.select": return try paneSelect(args)
        ...
        case "pane.click": return try paneClick(args)
        ...
        default:
            throw ControlError(
                ...
                action: "run `futuraterm --help` for the supported commands"
            )
        }
    }
    ...
    /// full scrollback with `scrollback: true`. Optional `--quiet-ms` waits
    /// until the text is unchanged for that long (or `--timeout-ms`).
    private func paneDump(_ args: ControlArgs) async throws -> ControlData {
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        let pane = target.pane
        let scrollback = args.scrollback == true
        if args.quietMs != nil {
            let (quietMs, timeoutMs) = try PaneDumpQuietWait.validate(args)
            let snapshot = try await PaneDumpQuietWait.wait(
                quietMs: quietMs,
                timeoutMs: timeoutMs
            ) {
                guard let view = pane.nsView else { return nil }
                return view.readText(scrollback: scrollback)
            }
            return ControlData(dump: ControlPaneDump(
                id: pane.id.uuidString,
                session: pane.sessionName,
                scrollback: scrollback,
                bytes: snapshot.text.utf8.count,
                text: snapshot.text,
                quietTimedOut: snapshot.timedOut
            ))
        }
        guard let view = pane.nsView, let text = view.readText(scrollback: scrollback) else {
            throw PaneDumpQuietWait.noSurface
        }
        return ControlData(dump: ControlPaneDump(
            id: pane.id.uuidString,
            session: pane.sessionName,
    ...
    /// Viewport numbered-choice list. Empty `[]` is success — the TUI may not
    /// be on a menu. Viewport only (never scrollback), same `no_surface` as dump.
    private func paneChoices(_ args: ControlArgs) throws -> ControlData {
        let parsed = try parsedChoices(args)
        return ControlData(choices: parsed.map(Self.wireChoice))
    }

    /// Type the menu index then Return. Unknown indexes never send keys.
    private func paneChoose(_ args: ControlArgs) throws -> ControlData {
        guard let index = args.choice else {
            throw ControlError(code: .badRequest, message: "pane.choose requires a choice index")
        }
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        let pane = target.pane
        guard let view = pane.nsView, let text = view.readText(scrollback: false) else {
            throw ControlError(
                code: .noSurface,
                message: "the pane's terminal isn't live yet",
                action: "select its tab once so the surface spawns, then retry"
        ...
        let parsed = NumberedChoiceParser.parse(text)
        guard !parsed.isEmpty else {
            throw ControlError(
                code: .notFound,
                message: "no numbered choices on screen"
            )
        }
        guard parsed.contains(where: { $0.index == index }) else {
            let available = parsed.map(\.index).map(String.init).joined(separator: ", ")
            throw ControlError(
                code: .notFound,
                message: "choice \(index) is not on screen (available: \(available))"
            )
        }
        guard let ret = HotkeyRegistry.parseShortcut("return"),
              view.sendText(String(index)),
              view.sendKey(keyCode: ret.keyCode, mods: ret.modifiers)
        else {
            throw ControlError(
                code: .noSurface,
                message: "the pane's terminal isn't live yet",
    ...
    private func parsedChoices(_ args: ControlArgs) throws -> [NumberedChoice] {
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        let pane = target.pane
        guard let view = pane.nsView, let text = view.readText(scrollback: false) else {
        ...
        return NumberedChoiceParser.parse(text)
    }

    private static func wireChoice(_ choice: NumberedChoice) -> ControlPaneChoice {
        ControlPaneChoice(
            index: choice.index,
            label: choice.label,
            line: choice.line,
    ...
    /// Read the live libghostty selection (`offset_start`/`offset_len` UTF-16).
    private func paneSelection(_ args: ControlArgs) throws -> ControlData {
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        guard let view = target.pane.nsView, let snapshot = view.selectionSnapshot() else {
            throw ControlError(
                code: .noSurface,
                message: "the pane's terminal isn't live yet",
                action: "select its tab once so the surface spawns, then retry"
        ...
        selection.session = target.pane.sessionName
        return ControlData(selection: selection)
    }

    /// Set selection: UTF-16 viewport range, 1-based viewport line, or cell box.
    private func paneSelect(_ args: ControlArgs) throws -> ControlData {
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        let mode = try paneSelectMode(args)
        guard let view = target.pane.nsView else {
            throw ControlError(
                code: .noSurface,
                message: "the pane's terminal isn't live yet",
                action: "select its tab once so the surface spawns, then retry"
        ...
        switch mode {
        case let .range(start, length):
            let range = NSRange(location: start, length: length)
            guard view.selectText(range: range) else {
                throw ControlError(
                    code: .badRequest,
                    message: "selection range \(start)+\(length) is outside the viewport",
                    action: "start/length are UTF-16 units in the visible viewport"
                )
            }
        case let .line(line):
            guard let text = view.readText(scrollback: false) else {
                throw ControlError(
                    code: .noSurface,
                    message: "the pane's terminal isn't live yet",
                    action: "select its tab once so the surface spawns, then retry"
            ...
            let range = TerminalAccessibilityText.range(forLine: line - 1, in: text)
            guard range.length > 0 else {
                throw ControlError(
                    code: .badRequest,
                    message: "line \(line) is outside the viewport",
                    action: "lines are 1-based in the visible viewport"
                )
            }
            guard view.selectText(range: range) else {
                throw ControlError(
        ...
        case let .cells(start, end):
            guard let size = view.surfaceSize else {
                throw ControlError(
                    code: .noSurface,
                    message: "the pane's terminal isn't live yet",
                    action: "select its tab once so the surface spawns, then retry"
            ...
            for cell in [start, end] {
                guard cell.col <= cols, cell.row <= rows else {
                    throw ControlError(
                        code: .badRequest,
                        message: "cell (\(cell.col), \(cell.row)) is outside the \(cols)×\(rows) viewport",
                        action: "use 1-based --col/--row (and --end-col/--end-row)"
                    )
                }
            }
            guard view.selectCells(start: start, end: end) else {
                throw ControlError(
                    code: .badRequest,
                    message: "cell box is outside the \(cols)×\(rows) viewport",
                    action: "use 1-based --col/--row (and --end-col/--end-row)"
                )
            }
        }
    ...
    private enum PaneSelectMode {
        case range(start: Int, length: Int)
        case line(Int)
        case cells(start: (col: Int, row: Int), end: (col: Int, row: Int))
    }

    private func paneSelectMode(_ args: ControlArgs) throws -> PaneSelectMode {
        let hasRange = args.start != nil || args.length != nil
        let hasLine = args.line != nil
        let hasCell = args.cellCol != nil || args.cellRow != nil
            || args.endCol != nil || args.endRow != nil
        let modeCount = [hasRange, hasLine, hasCell].filter(\.self).count
        if modeCount == 0 {
            throw ControlError(
                code: .badRequest,
                message: "pane.select requires start+length, line, or cellCol+cellRow",
                action: "pass exactly one of --start/--length, --line, or --col/--row"
            )
        }
        if modeCount > 1 {
            throw ControlError(
                code: .badRequest,
                message: "pane.select accepts exactly one of start+length, line, or cell box",
                action: "do not mix --start/--length, --line, and --col/--row"
            )
        }
        if hasRange {
            guard let start = args.start, let length = args.length else {
                throw ControlError(
                    code: .badRequest,
                    message: "pane.select range requires both start and length",
                    action: "pass --start and --length together"
                )
            }
            guard start >= 0, length >= 0 else {
                throw ControlError(
                    code: .badRequest,
                    message: "start and length must be non-negative UTF-16 offsets",
                    action: "use --start 0 --length N in the visible viewport"
                )
            }
            return .range(start: start, length: length)
        }
        if hasLine {
            let line = args.line ?? 0
            guard line >= 1 else {
                throw ControlError(
                    code: .badRequest,
                    message: "line \(line) is outside the viewport",
        ...
        guard let col = args.cellCol, let row = args.cellRow else {
            throw ControlError(
                code: .badRequest,
                message: "pane.select cell box requires cellCol and cellRow",
                action: "pass 1-based --col and --row (optional --end-col/--end-row)"
            )
        }
        let endCol = args.endCol ?? col
        let endRow = args.endRow ?? row
        guard col >= 1, row >= 1, endCol >= 1, endRow >= 1 else {
            throw ControlError(
                code: .badRequest,
                message: "cells are 1-based in the visible viewport",
                action: "use --col/--row 1 or greater"
            )
        }
        return .cells(start: (col, row), end: (endCol, endRow))
    }
    ...
    private func sessionInfo(_ args: ControlArgs) async throws -> ControlData {
        ...
        guard let match = data.sessions?.first(where: { $0.name == name }) else {
            throw ControlError(
                ...
                action: "run `futuraterm session list` to see live sessions"
            )
        }
    ...
    /// Always append a project. Re-running for the same directory adds a
    /// distinct row; `--select` only activates the just-created project.
    /// Scripts that want create-or-select should use `project.open`.
    private func projectCreate(_ args: ControlArgs) throws -> ControlData {
        let canonical = try resolvedProjectPath(from: args, command: "project.create")
        // No security-scoped bookmark: CLI has no NSOpenPanel (POR-505).
    ...
    /// Find-or-create by canonical path and always select. Re-opening an
    /// existing directory reuses the first matching project instead of
    /// appending a duplicate. With `--run`, reuse a matching live pane or
    /// create one tab for that command; omitting `--run` stays select-only.
    private func projectOpen(_ args: ControlArgs) throws -> ControlData {
        let canonical = try resolvedProjectPath(from: args, command: "project.open")
        ...
        guard let run = args.run, !run.isEmpty else {
            frontOpenedProject(project)
            return projectData(project)
        }

        let pane = try resolveOrCreateRunPane(run: run, reuse: args.reuse ?? true, project: project)
        appState.navigateToPane(pane.id, projectID: project.id)
        return try projectAndPaneData(project: project, pane: pane)
    }

    /// Reuse the first matching workspace pane, or `createTab` with `run`.
    /// `--no-reuse` (`reuse == false`) always creates. Does not change `tabNew`.
    private func resolveOrCreateRunPane(run: String, reuse: Bool, project: Project) throws -> Pane {
        if reuse, let existing = firstPane(matchingRun: run, in: project.id) {
            return existing
        }
        guard let tabID = appState.createTab(projectID: project.id, projectPath: project.path, command: run),
              let workspace = appState.workspaces[project.id],
              let tab = workspace.tabs.first(where: { $0.id == tabID }),
              let pane = tab.splitRoot.allPanes().first
        else {
    ...
    /// First pane in sidebar-tab then split-tree order whose command, live
    /// process basename, or (for `grok`) `AgentIcon` identity matches `run`.
    private func firstPane(matchingRun run: String, in projectID: UUID) -> Pane? {
        guard let workspace = appState.workspaces[projectID] else { return nil }
        let basename = (run as NSString).lastPathComponent
        for tab in workspace.tabs {
            if let pane = tab.splitRoot.allPanes().first(where: { paneMatchesRun($0, run: run, basename: basename) }) {
                return pane
            }
        }
        return nil
    ...
    private func paneMatchesRun(_ pane: Pane, run: String, basename: String) -> Bool {
        if pane.command == run { return true }
        if pane.foregroundProcessName == basename { return true }
        if basename.lowercased() == "grok", AgentIcon.match(processName: pane.foregroundProcessName) == .grok {
            return true
        }
        return false
    }

    private func projectAndPaneData(project: Project, pane: Pane) throws -> ControlData {
        var data = projectData(project)
        guard let workspace = appState.workspaces[project.id],
              let tab = workspace.tabs.first(where: { $0.splitRoot.findPane(id: pane.id) != nil })
        else {
            throw ControlError(code: .internalError, message: "opened pane is not in the workspace")
        }
        data.panes = [paneInfo(pane, in: tab, workspace: workspace)]
        return data
    }

    /// Canonicalize and existence-check a project path. A remote spec
    /// (`[user@]host:dir`, #104) is stored verbatim — there is no local
    /// directory to canonicalize or existence-check; a wrong host or dir
    /// surfaces in the pane itself (ssh's error / the cd diagnostic), same
    /// as a sheet-created remote project.
    private func resolvedProjectPath(from args: ControlArgs, command: String) throws -> String {
        guard let rawPath = args.path, !rawPath.isEmpty else {
            throw ControlError(code: .badRequest, message: "\(command) requires a path")
        }
    ...
    /// Front the terminal window after `project.open`. When a workspace pane
    /// exists, reuse `navigateToPane` (same path as `pane.focus`); otherwise
    /// activate and `reopenIfNeeded` without inventing a new window API.
    private func frontOpenedProject(_ project: Project) {
        if let pane = appState.focusedPane(for: project.id) {
            appState.navigateToPane(pane.id, projectID: project.id)
            return
        }
        NSApp.activate()
    ...
    private func tabMove(_ args: ControlArgs) throws -> ControlData {
        ...
        guard slot >= 1, slot <= workspace.tabs.count else {
            throw ControlError(
                ...
                action: "run `futuraterm tab list` for the current order"
            )
        }
    ...
    private func paneFocus(_ args: ControlArgs) throws -> ControlData {
        ...
        if let raw = args.direction {
            ...
            // whether FuturaTerm can go further; at the outermost edge the answer
    ...
    private func paneKey(_ args: ControlArgs) throws -> ControlData {
        ...
        guard let shortcut = HotkeyRegistry.parseShortcut(chord) else {
            throw ControlError(
                ...
                action: "use tokens like ctrl+c, escape, up, or ctrl+\\ (see `futuraterm pane key --help`)"
            )
        }
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
    ...
    /// Left-click a 1-based viewport cell (`GhosttyTerminalNSView.clickCell`).
    /// Resolve the pane first so a bad session is `not_found`, not a missing-cell
    /// `bad_request`. A dead surface is `no_surface` (same copy as `pane.dump`);
    /// missing or out-of-range cells are `bad_request`.
    private func paneClick(_ args: ControlArgs) throws -> ControlData {
        let (_, workspace) = try resolveWorkspace(args)
        let target = try resolvePane(args, in: workspace)
        guard let col = args.cellCol, let row = args.cellRow else {
            throw ControlError(
                code: .badRequest,
                message: "pane.click requires cellCol and cellRow",
                action: "pass 1-based viewport --col and --row"
            )
        }
        guard col >= 1, row >= 1 else {
            throw ControlError(
                code: .badRequest,
                message: "cell (\(col), \(row)) is outside the viewport",
                action: "cells are 1-based in the visible viewport"
            )
        }
        guard let view = target.pane.nsView, let size = view.surfaceSize else {
            throw ControlError(
                code: .noSurface,
                message: "the pane's terminal isn't live yet",
                action: "select its tab once so the surface spawns, then retry"
        ...
        guard col <= cols, row <= rows else {
            throw ControlError(
                code: .badRequest,
                message: "cell (\(col), \(row)) is outside the \(cols)×\(rows) viewport",
                action: "use 1-based --col 1...\(cols) and --row 1...\(rows)"
            )
        }
        guard view.clickCell(col: col, row: row) else {
            throw ControlError(
                code: .badRequest,
                message: "cell (\(col), \(row)) is outside the \(cols)×\(rows) viewport",
                action: "use 1-based --col 1...\(cols) and --row 1...\(rows)"
    ...
    private func rejectPinned(_ project: Project, verb: String) throws {
        ...
        throw ControlError(
            ...
            action: "edit ~/.config/futuraterm/projects/pinned.yaml instead"
        )
    }
    ...
    private func resolveWorkspace(_ args: ControlArgs) throws -> (Project, Workspace) {
        ...
        guard let workspace = appState.workspaces[project.id] else {
            throw ControlError(
                ...
                action: "select it first: `futuraterm project select \(project.name)`"
            )
        }
    ...
    private func resolveProject(_ selector: String?) throws -> Project {
        ...
        case 0:
            throw ControlError(
                ...
                action: "run `futuraterm project list`"
            )
        default:
            throw ControlError(
                code: .ambiguous,
                message: "\(byName.count) projects are named \"\(selector)\"",
                action: "target by UUID from `futuraterm project list --json`"
            )
        }
    }
    ...
    private func resolveTab(_ args: ControlArgs, in workspace: Workspace) throws -> (Int, TerminalTab) {
        ...
        case 0:
            throw ControlError(
                ...
                action: "run `futuraterm tab list`"
            )
        default:
            throw ControlError(
                code: .ambiguous,
                message: "\(byTitle.count) tabs are titled \"\(selector)\"",
                action: "target by index or UUID from `futuraterm tab list --json`"
            )
        }
    }
    ...
    private func resolvePane(_ args: ControlArgs, in workspace: Workspace) throws -> (tab: TerminalTab, pane: Pane) {
        ...
        if let session = args.session, !session.isEmpty {
            ...
            throw ControlError(
                ...
                action: "run `futuraterm pane list` for live panes"
            )
        }
        if let selector = args.pane, !selector.isEmpty {
            ...
            guard let index = parseIndex(selector, prefix: "pane"), panes.indices.contains(index - 1) else {
                throw ControlError(
                    ...
                    action: "run `futuraterm pane list` for indexes"
                )
            }
