...
/// App-side listener for the `futuraterm` control CLI. Accepts one request per
...
final class ControlSocketServer: @unchecked Sendable {
    ...
    private let connectionQueue = DispatchQueue(
        label: "com.davidsolheim.futuraterm.control-connection",
    ...
    /// The path exported to spawned shells via `FUTURATERM_SOCKET`.
    var path: String { socketPath }

    /// The per-flavor well-known socket location (`FuturaTerm/` vs
    /// `FuturaTerm Debug/`, or the benchmark's `FUTURATERM_BENCHMARK_DATA_DIR`).
    ...
    func start() {
        ...
        thread.name = "com.davidsolheim.futuraterm.control-socket"
