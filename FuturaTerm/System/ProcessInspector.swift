...
/// table (`KERN_PROCARGS2`). Process-tree walking is used only for nested
/// AI-agent counting, gated on an already-recognized agent foreground.
///
...
enum ProcessInspector {
    ...
    /// Absolute paths of vnode file descriptors `pid` has open, or `[]` when
    /// the process is gone or unreadable. Caps at 512 fds so a leaky process
    /// cannot balloon the Grok session lookup.
    static func vnodePaths(pid: pid_t) -> [String] {
        let needed = proc_pidinfo(pid, PROC_PIDLISTFDS, 0, nil, 0)
        guard needed > 0 else { return [] }
        let fdSize = MemoryLayout<proc_fdinfo>.size
        let count = min(Int(needed) / fdSize, 512)
        ...
        let filled = fds.withUnsafeMutableBufferPointer { buf -> Int32 in
            proc_pidinfo(
                ...
                0,
                buf.baseAddress,
                Int32(buf.count * fdSize)
            )
        }
        guard filled > 0 else { return [] }
        let n = min(Int(filled) / fdSize, count)
        var paths: [String] = []
        paths.reserveCapacity(n)
        for i in 0 ..< n {
            let info = fds[i]
            guard info.proc_fdtype == UInt32(PROX_FDTYPE_VNODE) else { continue }
            var vnode = vnode_fdinfowithpath()
            let vsize = Int32(MemoryLayout<vnode_fdinfowithpath>.size)
            let got = proc_pidfdinfo(pid, info.proc_fd, PROC_PIDFDVNODEPATHINFO, &vnode, vsize)
            guard got == vsize else { continue }
            let path = withUnsafeBytes(of: &vnode.pvip.vip_path) { raw -> String? in
                let bytes = raw.bindMemory(to: CChar.self)
                return bytes.baseAddress.map { String(cString: $0) }
            }
    ...
    /// Kernel process start time, or nil when `pid` is gone.
    ...
    /// Process group id of `pid`, or nil when the process is gone.
    ...
    /// Max descendant depth walked for nested-agent counting (root is depth 0).
    static let nestedAgentMaxDepth = 8
    /// Max descendant pids visited for nested-agent counting (root excluded).
    static let nestedAgentMaxPIDs = 64

    /// Direct children of `pid` via `proc_listchildpids`, or `[]` on any Darwin
    /// error. The buffer is sized to the walk cap so a wedged fan-out cannot
    /// stall the poll. With a buffer, the C API returns the number of pids
    /// written (not a byte count); a missing pid returns 0.
    static func childPIDs(of pid: pid_t) -> [pid_t] {
        var pids = [pid_t](repeating: 0, count: nestedAgentMaxPIDs)
        let n = pids.withUnsafeMutableBufferPointer { buf -> Int in
            guard let base = buf.baseAddress else { return 0 }
            let bytes = Int32(buf.count * MemoryLayout<pid_t>.size)
            let ret = proc_listchildpids(pid, base, bytes)
            return ret > 0 ? Int(ret) : 0
        }
        guard n > 0 else { return [] }
        return Array(pids.prefix(min(n, pids.count))).filter { $0 > 0 }
    }

    /// Count of descendant processes of `rootPID` whose kernel `comm` matches
    /// `AgentIcon.match(processName:)` (login-shell `-` stripped). The root
    /// itself is never counted. Production uses `childPIDs` / `comm`; tests
    /// inject a fake tree. Fail closed: Darwin errors and missing lookups
    /// contribute 0, never a throw.
    static func nestedAgentCount(rootPID: pid_t) -> Int {
        nestedAgentCount(rootPID: rootPID, childrenOf: childPIDs, commOf: comm)
    }

    static func nestedAgentCount(
        rootPID: pid_t,
        ...
        var stack: [(pid: pid_t, depth: Int)] = [(rootPID, 0)]
        while let node = stack.popLast() {
            if node.depth >= nestedAgentMaxDepth { continue }
            for child in childrenOf(node.pid) {
                guard child > 0, seen.insert(child).inserted else { continue }
                visited += 1
                if isNestedAgent(comm: commOf(child)) { count += 1 }
                if visited >= nestedAgentMaxPIDs { return count }
    ...
    /// Nested-agent count for a live pane. Returns 0 without walking when the
    /// pane is remote, has no resolved pid, or its foreground is not an agent
    /// (`agentIcon == nil`) — shells and ordinary programs must not pay for a
    /// descendant walk on the 250ms poll.
    @MainActor
    static func nestedAgentCount(forPane pane: Pane) -> Int {
        nestedAgentCount(forPane: pane, childrenOf: childPIDs, commOf: comm)
    }
    ...
    static func nestedAgentCount(
        ...
        guard !pane.isRemote, pane.agentIcon != nil else { return 0 }
        guard let pid = resolvedForegroundPID(forPane: pane) else { return 0 }
        return nestedAgentCount(rootPID: pid, childrenOf: childrenOf, commOf: commOf)
    }

    /// `AgentIcon.match(processName:)` against a kernel `comm`, with a leading
    /// login-shell `-` stripped. Child matching is comm-only — no argv fallback.
    private static func isNestedAgent(comm: String?) -> Bool {
        guard var name = comm, !name.isEmpty else { return false }
        if name.hasPrefix("-") { name.removeFirst() }
        return AgentIcon.match(processName: name) != nil
    }
