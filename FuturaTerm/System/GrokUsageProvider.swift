...
enum GrokAuthFile {
    ...
    static func url(
        ...
        return URL(fileURLWithPath: home + "/.grok/auth.json")
    }
...
enum GrokBillingParser {
    static func parse(
        ...
        if let config = root["config"] as? [String: Any],
           let percent = jsonDouble(config["creditUsagePercent"])
        {
            let usedFraction = min(1, max(0, percent / 100))
            let endRaw = (config["currentPeriod"] as? [String: Any])?["end"] as? String
        ...
        let used = jsonDouble((root["used"] as? [String: Any])?["val"])
        let limit = jsonDouble((root["monthlyLimit"] as? [String: Any])?["val"])
        if let used, let limit, limit > 0 {
            let usedFraction = min(1, max(0, used / limit))
            let endRaw = root["billingPeriodEnd"] as? String
            return AgentUsageSnapshot(
                kind: kind,
                usedFraction: usedFraction,
                periodEnd: endRaw.flatMap(GrokRFC3339.parse),
