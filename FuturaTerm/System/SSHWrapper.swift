...
/// `ghostty +ssh` CLI action reimplemented natively, surfaced as `futuraterm ssh`
...
/// -- "$@"` in every shell they support. FuturaTerm ships no ghostty CLI, and
...
/// libghostty actually supports (see `FuturaTermConfig.regenerate`).
///
...
/// app target and the `FuturaTermCLI` tool target (the `ControlProtocol`
...
/// same entry for panes whose ssh connection FuturaTerm itself owns; this type
...
enum SSHWrapper {
    ...
    /// the CLI lives at `Contents/Resources/bin/futuraterm` and the DB at its
    ...
    static func cacheFilePath(home: String) -> String {
        home + "/Library/Caches/futuraterm/ssh-terminfo"
    }
