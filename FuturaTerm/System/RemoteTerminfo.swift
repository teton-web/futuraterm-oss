...
/// Installs FuturaTerm's own `xterm-ghostty` terminfo entry on a remote host, so
...
/// ghostty binary involved. FuturaTerm neither ships nor detects a `ghostty`
...
/// produced `SSHWrapper`/`futuraterm ssh`, which serves the ssh the user types
/// into a *local* pane; this type serves panes whose connection FuturaTerm
/// itself owns.)
///
...
/// FuturaTerm's own settings (ghostty-shaped settings belong in the ghostty
/// config).
enum RemoteTerminfo {
...
final class RemoteTerminfoInstaller {
    static let shared = RemoteTerminfoInstaller(
        isEnabled: {
            ...
            // existing flag keeps a ghostty-shaped setting out of FuturaTerm's
            // own preferences.
            ShellIntegrationFeatures.isEnabled(
                "ssh-terminfo", inConfigText: FuturaTermConfig.userGhosttyConfigText()
            )
        },
