...
enum RemoteSpawn {
    ...
    /// FuturaTerm pane command IS the surface command, spawned with no shell in
    ...
    private static func zmxPresenceGuard(zmx: String, fallbackShell: String) -> String? {
        zmx == "zmx"
            ...
            + "{ echo \"futuraterm: zmx not found in PATH on this host ($PATH)\" >&2; \(fallbackShell); }"
            : nil
    }
    ...
    static func paneCommand(remote: ProjectPath, sessionName: String, zmxPath: String? = nil) -> String? {
        ...
        let script = assertSingleQuoteFree(remoteEnvPreamble + remoteTermPreamble + remoteColorPreamble + [
            ...
            "cd \(quotedDir) || "
                + "{ echo \"futuraterm: cannot cd to \(quotedDir)\" >&2; \(fallbackShell); }",
    ...
    /// `run:` capture: resolve every `futuraterm-*` session on the host to its
    ...
    <ZMX> ls 2>/dev/null | while read -r line; do
      ...
      case "$n" in futuraterm-*) ;; *) continue ;; esac
    ...
    /// The zmx label key marking a session as created by a FuturaTerm
    /// installation (`futuraterm.owner=<Preferences.installationID>`). Labels
    ...
    static let ownerLabelKey = "futuraterm.owner"
    ...
    /// silently unimplemented (zmx `src/main.zig` parses only
    ...
    /// argv (for `/usr/bin/env ssh`) running `futuraterm-sunshine-pair` on the
    /// host. Same BatchMode profile as `opArgv`. `pin` must be 4 digits;
    /// `clientName` is reduced to the Sunshine-safe alphabet (no quotes).
    /// Probe Sunshine's local admin port over SSH. Any HTTP response means
    /// the host is streaming; empty/timeout means do not open Moonlight pair.
    static func sunshineProbeArgv(remote: ProjectPath) -> [String]? {
        guard case let .remote(user, host, _) = remote else { return nil }
        let script = assertSingleQuoteFree(
            remoteEnvPreamble
                + "curl -sk -o /dev/null -w %{http_code} https://127.0.0.1:47990",
            onViolation: .failNonZero
        )
        return [
            "-o", "BatchMode=yes",
    ...
    static func sunshinePairArgv(remote: ProjectPath, pin: String, clientName: String) -> [String]? {
        guard case let .remote(user, host, _) = remote else { return nil }
        guard pin.count == 4, pin.allSatisfy(\.isNumber) else { return nil }
        let name = sunshinePairClientName(clientName)
        let script = assertSingleQuoteFree(
            remoteEnvPreamble
                + "P=$HOME/.local/bin/futuraterm-sunshine-pair; "
                + "if [ -x \"$P\" ]; then exec \"$P\" --pin \(pin) --name \(posixDoubleQuote(name)); fi; "
                + "exec futuraterm-sunshine-pair --pin \(pin) --name \(posixDoubleQuote(name))",
            onViolation: .failNonZero
        )
        return [
            "-o", "BatchMode=yes",
    ...
    static func sunshinePairClientName(_ raw: String) -> String {
    ...
    static func assertSingleQuoteFree(
        ...
        let diagnostic = "echo \"futuraterm: remote path contains an unsupported single quote\" >&2; "
        switch fallback {
        case .dropToShell:
