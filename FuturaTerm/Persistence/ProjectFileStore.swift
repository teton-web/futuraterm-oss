...
/// (`~/.config/futuraterm/projects/`). Stateless: every operation re-scans the
...
struct ProjectFileStore {
    ...
    /// `~/.config/futuraterm/projects`. Deliberately shared across debug/release
    ...
    nonisolated static func defaultDirectory() -> URL {
        URL(fileURLWithPath: ProjectPath.currentHome, isDirectory: true)
            .appendingPathComponent(".config/futuraterm/projects", isDirectory: true)
    }
