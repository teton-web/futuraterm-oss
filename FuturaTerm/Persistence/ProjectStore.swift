...
final class ProjectStore {
    ...
    /// Outcome of dropping one folder header onto another.
    enum FolderDropOutcome: Equatable {
        case reordered
        case nested
        case ignored
    }

    /// Same-parent drop reorders to the target's slot (project-on-project
    /// `toOffset` convention). Different parent nests under the target.
    @discardableResult
    func dropFolder(_ groupID: UUID, ontoFolder targetID: UUID, insertAfter: Bool? = nil) -> FolderDropOutcome {
        guard groupID != targetID,
              let dragged = groups.first(where: { $0.id == groupID }),
              let target = groups.first(where: { $0.id == targetID })
        else { return .ignored }
        let draggedParent = resolvedParentID(dragged.parentID)
        let targetParent = resolvedParentID(target.parentID)
        if draggedParent == targetParent {
            let siblings = childGroups(in: draggedParent)
            guard let fromIndex = siblings.firstIndex(where: { $0.id == groupID }),
                  let targetIndex = siblings.firstIndex(where: { $0.id == targetID }),
                  fromIndex != targetIndex
            else { return .ignored }
            let toOffset = if let insertAfter {
                insertAfter ? targetIndex + 1 : targetIndex
            } else {
                fromIndex < targetIndex ? targetIndex + 1 : targetIndex
            }
            if insertAfter == true, fromIndex == targetIndex + 1 { return .ignored }
            if insertAfter == false, fromIndex + 1 == targetIndex { return .ignored }
            reorderGroupsAmongSiblings(groupID: groupID, toOffset: toOffset)
            return .reordered
        }
        let before = draggedParent
        setParent(groupID: groupID, parentID: targetID)
        let after = resolvedParentID(groups.first { $0.id == groupID }?.parentID)
        if after == targetID, before != targetID {
            return .nested
        }
        return .ignored
    }
    ...
    func reorderGroupsAmongSiblings(groupID: UUID, toOffset: Int) {
        ...
        // Keep `groups` in sibling order so List/ForEach sees a collection
        // move, not only in-place sortOrder writes.
        let siblingSet = Set(ordered)
        let rest = groups.filter { !siblingSet.contains($0.id) }
        groups = ordered.compactMap { id in groups.first { $0.id == id } } + rest
        saveGroups()
    }
