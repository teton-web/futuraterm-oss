...
struct PinnedTabSnapshot: Codable {
    ...
    /// Optional MAS folder grant that outlives a removed origin project.
    var originFolderPath: String?
    var originFolderBookmark: Data?
    /// Extra covering folder grants (mixed-cwd merges / dest leftovers).
    ...
    static func snapshotPinned(records: [PinnedTabRecord], workspace: Workspace?) -> [PinnedTabSnapshot] {
        records.map { record in
            ...
            return PinnedTabSnapshot(
                ...
                originFolderPath: record.originFolderPath,
                originFolderBookmark: record.originFolderBookmark,
                coveringGrants: record.coveringGrants.isEmpty ? nil : record.coveringGrants
            )
        }
    }
