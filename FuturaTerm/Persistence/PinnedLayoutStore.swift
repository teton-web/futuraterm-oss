...
// `~/.config/futuraterm/projects/pinned.yaml`. Unlike every other file in that
...
struct PinnedLayoutStore {
    ...
    func write(tabs: [LayoutTab]) throws -> String {
        ...
        let header = """
        # Managed by FuturaTerm — the pinned tabs' layout. Safe to hand-edit:
        # an entry added here appears as a pinned tab on the next launch, and
        # removing an entry unpins its tab (honored at launch). FuturaTerm
