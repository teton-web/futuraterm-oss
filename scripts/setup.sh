...
FORK_REPO="davidsolheim/futuraterm-ghosttykit"
ZMX_REPO="davidsolheim/futuraterm-zmx"
XCFRAMEWORK_DIR="GhosttyKit.xcframework"
# The pinned davidsolheim/futuraterm-ghosttykit release supplying BOTH GhosttyKit
# and the bundled ghostty resources (one release, two assets).
#
# This used to track `latest` on a daily-build host — so every FuturaTerm build,
# tagged releases included, silently picked up whatever libghostty landed that
# morning. Two builds of the same FuturaTerm commit could ship different
# terminal cores, and a release could carry an upstream change nobody had run.
# Bumping the terminal core is now an explicit, reviewable commit instead of a
...
RESOURCES_MARKER="FuturaTerm/Resources/terminfo"
# Prebuilt zmx session multiplexer (session persistence). Downloaded from
# davidsolheim/futuraterm-zmx, mirroring GhosttyKit — never compiled locally (zig).
# Embedded into the bundle at Contents/Resources/zmx/zmx by embed-zmx.sh.
ZMX_BIN="FuturaTerm/Resources/zmx/zmx"
# Does the xcframework rooted at $1 expose the output-activity ABI FuturaTerm's
...
# Stale-artifact warning. A checkout can still be holding a GhosttyKit whose
# UPSTREAM BASE is old (an artifact downloaded before a pin bump). This surfaces
# that here.
#
...
warn_if_ghosttykit_stale() {
  local stale_days="${FUTURATERM_GHOSTTYKIT_MAX_AGE_DAYS:-14}"
  ...
  if (( age_days > stale_days )); then
    ...
    echo "         upstream fixes (this is the class of staleness behind a known crash)." >&2
...
if $need_xcframework; then
  ...
  staging="$(mktemp -d "${TMPDIR:-/tmp}/futuraterm-ghosttykit.XXXXXX")"
  ...
  if ! has_output_activity_action "$staging/$XCFRAMEWORK_DIR"; then
    ...
    echo "The GhosttyKit release must include GHOSTTY_ACTION_OUTPUT_ACTIVITY." >&2
    echo "" >&2
    echo "Note: FuturaTerm has required this ABI since the reliable-activity-detection" >&2
...
if $need_resources; then
  ...
  # FuturaTerm/Resources/ (all gitignored — none committed). Clear any prior
  ...
  rm -rf FuturaTerm/Resources/ghostty FuturaTerm/Resources/terminfo \
    FuturaTerm/Resources/themes FuturaTerm/Resources/shell-integration
  mkdir -p FuturaTerm/Resources
  tar xzf ghostty-resources.tar.gz -C FuturaTerm/Resources
...
if $need_zmx; then
  # Prebuilt universal (arm64+x86_64) zmx binary from davidsolheim/futuraterm-zmx.
  # Universal so it execs on both Apple Silicon and Intel, matching FuturaTerm's
  ...
  # extracted to FuturaTerm/Resources/zmx/zmx (gitignored).
  ...
  rm -rf FuturaTerm/Resources/zmx
  mkdir -p FuturaTerm/Resources/zmx
  tar xzf zmx-universal-macos.tar.gz -C FuturaTerm/Resources/zmx
  chmod +x FuturaTerm/Resources/zmx/zmx
