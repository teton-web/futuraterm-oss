...
Measures FuturaTerm's CPU and memory across an idle-focused sanity baseline plus
two workload states — focused and unfocused, each with busy tabs on screen —
by launching the app with the FUTURATERM_BENCHMARK=1 control hook
(FuturaTerm/App/BenchmarkControl.swift) and driving it with Darwin notifications
...
from _harness import HarnessError, FuturaTermHarness, notify, sh
...
IDLE_STATES = ("focused",)
# Sampled after spawning busy tabs/panes via the bundled `futuraterm` CLI. The
...
def check_alive(harness):
    # Zombie-aware liveness (see FuturaTermHarness.is_alive); dying mid-run is
...
def cmd_run(args):
    # Isolation (throwaway $HOME, FUTURATERM_BENCHMARK_DATA_DIR, ZMX_DIR) and the
    # LaunchServices launch + exact-pid-match recipe live in FuturaTermHarness,
    ...
    harness = FuturaTermHarness(os.path.abspath(args.app), home_prefix="futuraterm-bench-home-")
    try:
        try:
            ...
            # marker — see FuturaTermHarness.open_project).
...
def cmd_report(args):
    ...
    if baseline is not None and workload_missing_baseline:
        lines.append("")
        lines.append(
            "_The `workload-*` states (busy tabs spawned via the `futuraterm` CLI) "
...
def main():
    ...
    run.add_argument("--app", required=True, help="path to the built FuturaTerm.app")
    ...
    run.add_argument(
        ...
        help="also sample workload-* states after spawning TABS busy tabs "
             "(2x2 grid each) via the bundled futuraterm CLI (default: off)",
    )
