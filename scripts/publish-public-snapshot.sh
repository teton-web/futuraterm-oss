#!/usr/bin/env bash
# Publish a history-free snapshot to teton-web/futuraterm-oss.
#
...
# owner/name from a git remote URL. Must not substring-match PUBLIC_REPO
# against PRIVATE_REPO: teton-web/futuraterm is a prefix of
# teton-web/futuraterm-oss.
github_nwo() {
  local url="${1%.git}"
  url="${url%/}"
  url="${url#git@}"
...
ORIGIN_NWO="$(github_nwo "$ORIGIN_URL")"
if [[ "$ORIGIN_NWO" == "$PUBLIC_REPO" ]]; then
  echo "error: this checkout's origin is already ${PUBLIC_REPO}; run from the private teton-web clone" >&2
  exit 1
fi
if [[ "$ORIGIN_NWO" != "$PRIVATE_REPO" ]]; then
  echo "error: origin is '$ORIGIN_URL' (${ORIGIN_NWO}), expected ${PRIVATE_REPO} (refusing to guess)" >&2
  exit 1
fi
...
# notarizing on:release job, the MAS archive/upload job, and website/
# stay on teton-web only.
rm -rf website
rm -f .github/workflows/release.yml
rm -f .github/workflows/mas.yml
rm -f .github/workflows/website.yml
# Root Dependabot still names /website; that directory is gone on the snapshot.
...
for line in text.splitlines(True):
    if line.startswith("  - package-ecosystem: bun"):
        skip = True
        continue
    if skip:
        if line.startswith("  - "):
...
if git ls-tree -r --name-only HEAD | grep -qx '.github/workflows/mas.yml'; then
  echo "error: snapshot contains .github/workflows/mas.yml (MAS archive/upload job must stay private)" >&2
  exit 1
fi
if git ls-tree -r --name-only HEAD | grep -qx '.github/workflows/website.yml'; then
  echo "error: snapshot contains .github/workflows/website.yml (Website CI assumes website/ and must stay private)" >&2
  exit 1
fi
if git ls-tree -r --name-only HEAD | grep -q '^website/'; then
  echo "error: snapshot contains website/ (futuraterm.com must stay private)" >&2
  exit 1
fi
...
if git grep -nF 'FUTURATERM_MAS_SIGNING_CERT_P12' HEAD -- '.github/workflows'; then
  echo "error: snapshot workflows still reference FUTURATERM_MAS_SIGNING_CERT_P12" >&2
  exit 1
fi
...
PUBLIC_REMOTE_NWO="$(github_nwo "$(git remote get-url public)")"
if [[ "$PUBLIC_REMOTE_NWO" == "$PRIVATE_REPO" ]]; then
  echo "error: public remote points at the private repo" >&2
  exit 1
fi
if [[ "$PUBLIC_REMOTE_NWO" != "$PUBLIC_REPO" ]]; then
  echo "error: public remote is '$PUBLIC_REMOTE_NWO', expected ${PUBLIC_REPO}" >&2
  exit 1
fi
