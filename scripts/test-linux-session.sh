...
grep -q 'command -v sunshine' "$ROOT/linux/packaging/install.sh"
grep -q 'linked unit' "$ROOT/linux/packaging/install.sh"
...
grep -q refresh_sunshine_admin "$ROOT/linux/packaging/install.sh"
...
! grep -q 'has_sunshine_username' "$ROOT/linux/packaging/install.sh"
! grep -q 'PIN at https://localhost:47990' "$ROOT/linux/packaging/install.sh"
...
grep -q 'sunshine --creds' "$HELPER"
! grep -E -q 'read -[ap]' "$HELPER"
PAIR_HOME="$(mktemp -d "${TMPDIR:-/tmp}/ft-sunshine-pair.XXXXXX")"
if HOME="$PAIR_HOME" PATH="/usr/bin:/bin" sh "$HELPER" --pin 1234 --name test 2>/dev/null; then
  echo "error: pair helper must fail without Sunshine" >&2
  rm -rf "$PAIR_HOME"
  exit 1
fi
