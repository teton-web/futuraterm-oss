...
if [[ "$(uname -s)" != "Linux" ]]; then
  ...
  echo "The portable session unit test still runs here:" >&2
  cc -O2 -Wall -Wextra -o /tmp/ft-linux-test-session tests/test_session.c src/session.c
  /tmp/ft-linux-test-session
