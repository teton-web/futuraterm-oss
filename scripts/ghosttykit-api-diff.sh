#!/usr/bin/env bash
# Report how a GhosttyKit bump changes the part of ghostty.h that FuturaTerm
# actually uses.
#
...
work="$(mktemp -d "${TMPDIR:-/tmp}/futuraterm-apidiff.XXXXXX")"
...
{
  grep -rhoE '\bghostty_[a-z0-9_]+' --include='*.swift' "$ROOT/FuturaTerm" "$ROOT/CLI" 2>/dev/null || true
  grep -rhoE '\bGHOSTTY_[A-Z0-9_]+' --include='*.swift' "$ROOT/FuturaTerm" "$ROOT/CLI" 2>/dev/null || true
...
if [[ "$used_count" == "0" ]]; then
  echo "error: no ghostty_* symbol from $ROOT/{FuturaTerm,CLI} appears in either header — has the source layout or the header moved?" >&2
  exit 1
fi
...
echo
echo "Filtered to the **${used_count}** \`ghostty_*\`/\`GHOSTTY_*\` symbols FuturaTerm's own sources reference and the header defines."
echo
