#!/bin/sh
# FuturaTerm's stand-in for the ghostty CLI, bundled at
# Contents/Resources/ssh-bridge/ghostty by the "Bundle futuraterm CLI"
# post-build step.
#
# FuturaTermConfig.regenerate points GHOSTTY_BIN_DIR here, and the bundled
...
# `futuraterm ssh`, so those features work with no Ghostty.app installed. It
...
if [ "$1" != "+ssh" ]; then
  echo "this is FuturaTerm's ghostty shim; only +ssh is supported (install Ghostty.app for the real CLI)" >&2
  exit 1
fi
shift
exec "$(dirname "$0")/../bin/futuraterm" ssh "$@"
