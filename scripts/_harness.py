"""Shared harness for scripted, hermetic FuturaTerm instances.
...
- Drive window/project state with Darwin notifications (`notifyutil -p`, the
  FUTURATERM_BENCHMARK=1 hook in FuturaTerm/App/BenchmarkControl.swift) and the
  bundled `futuraterm` CLI over the instance's own control socket. Neither needs
  a TCC grant, so a stock headless CI runner can script the GUI app.
- Isolate the instance: a throwaway $HOME (shell rc files, ~/.config/futuraterm),
  FUTURATERM_BENCHMARK_DATA_DIR (App Support resolves via the user record, NOT
  ...
  FuturaTerm: its `session list` would see their sessions, and its orphan reaper
...
NOTIFY_PREFIX = "com.davidsolheim.futuraterm.bench."
...
class FuturaTermHarness:
    ...
    def __init__(self, app, home_prefix="futuraterm-harness-home-"):
        ...
        self.cli_path = os.path.join(self.app, "Contents", "Resources", "bin", "futuraterm")
        ...
        self.env = {
            "FUTURATERM_BENCHMARK": "1",
            "FUTURATERM_BENCHMARK_DATA_DIR": self.data_dir,
            "HOME": self.home,
    ...
    def wait_for_socket(self, attempts=30):
        ...
        if not os.path.exists(self.cli_path):
            raise HarnessError("bundled futuraterm CLI missing from the app")
        probe = None
        for _ in range(attempts):
    ...
    def kill_sessions(self):
        ...
        could list sessions belonging to another FuturaTerm, and those must
    ...
    def cli(self, *args, check=True, timeout=60):
        """Run the bundled `futuraterm` CLI against this instance's socket.
        ...
        socket discovery, possibly reaching a real FuturaTerm). Use pane_run()
        for that verb.
        """
        if not os.path.exists(self.cli_path):
            raise HarnessError("bundled futuraterm CLI missing from the app")
        ...
        if check and result.returncode != 0:
            raise HarnessError(f"futuraterm {' '.join(args)} failed: {result.stderr.strip()}")
        return result

    def cli_json(self, *args):
        return json.loads(self.cli(*args, "--json").stdout)

    def new_tab(self, run="/bin/sh"):
        """Open a tab whose pane is a POSIX shell.

        Bare `tab new` follows `startGrokInNewTabs` (on by default) and types
        grok — poison for hermetic `pane run` of `/bin/sh -c`. Explicit `--run`
        wins over that pref without changing the user-facing default.
        """
        return self.cli_json("tab", "new", "--run", run)["tabs"][0]
    ...
    def pane_run(self, command, pane=None, session=None):
        ...
        if result.returncode != 0:
            raise HarnessError(f"futuraterm pane run failed: {result.stderr.strip()}")
        return result
