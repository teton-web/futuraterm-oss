...
# App Store CFBundleVersion: one to three period-separated integers. Sparkle's
# 4-component comparison string is rejected at export/upload validation, and
# MAS has no Sparkle feed, so do not reuse sparkle_comparison_version here.
# Stable X.Y.Z and X.Y.Z-beta.N both stamp as X.Y.Z (TestFlight uniqueness
# for a beta of the same marketing version is a VERSION bump).
mas_bundle_version() {
  local version="$1"
  ...
  echo "error: cannot derive MAS CFBundleVersion from '$version' (want X.Y.Z)" >&2
  return 1
}
